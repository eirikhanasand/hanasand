...
- [x] Operate an automated end-to-end collection pipeline continuously and recover safely from restarts and source failures.
...
- [x] Track actor behavior and characterization changes over time.
...
- Production `/v1/intel/evaluation` now measures attempt-level active-source reliability and useful yield, actor coverage, duplicates, corroborated and contradicted claims, and label-backed false-positive rate from durable records. A fresh live read at 2026-08-03T02:29:46Z contains 1,056 label events, all explicitly diagnostic, with zero independently evaluated units; precision, recall, specificity, and F1 are correctly null rather than being inferred from automated source-field parity.
...
- Read-only production measurement at 2026-08-02T23:18:10Z found 2,338 timeliness records: publication-to-collection median 218,920s/p95 16,744,441s (1,143 non-negative samples), collection-to-visibility median 4.19s/p95 119.63s (1,144 samples), report-to-visibility median 3,702.82s/p95 6,916,713.36s (449 samples), and report-to-alert median 21,736.46s/p95 94,511.37s (16 samples). This is useful evidence, but alert coverage remains too small for completion and does not replace the required independently timestamped benchmark corpus.
