...
At 2026-08-09T10:03Z, a fresh cross-authority census found 673 endpoint keys not present in the governed pack or its hashed exclusions, but most were historical endpoints or path variants. Only 19 matched the same service host in a second independent public research authority. The first bounded 12-source approved-proxy batch produced zero parser-positive feeds: one reachable response was parser-empty, nine returned upstream HTTP 500, and two failed before useful metadata could be parsed. Every authority record in that bounded set already marked the service unavailable. None was admitted, excluded, or counted; discovery must continue from the remaining independently referenced frontier without treating inventory volume as source coverage.
