...
The scheduler timeout was fixed in `9955ace1` and the follow-up measurement fix in `c984215a` (canonical `main` at `c984215a`). The default dashboard request now asks for two source rows: this remains bounded while preserving measured operational and qualification totals; explicit paginated requests retain their requested page size. The canonical scraper was rebuilt and recreated from that checkout (container `268421c7c30355060ae3c20c9bc4e22e152585109f0e6ad677bc7c4ccb8c69a6`, image digest `sha256:d2839018788d56e8d64eb108c72fd88f93c7d1e4c2719e658515f251a86c7156`); the authenticated global scheduler endpoint returned 58 sources with `measurementState: measured`, 14 qualifying clear-web sources, 8 qualifying public Telegram sources, and zero qualifying lawful dark-web sources. The scraper is healthy with storage `ok:true`, PostgreSQL backend, migration `036_source_operations_runtime_indexes`, and `pendingWrites:0`. This closes the timeout and hidden-metrics defects; it does not satisfy the separate 6,100 qualifying-feed requirement.
...
On 2026-08-03, the follow-up measurement fix was deployed from canonical `main` at `fcd604c9`. After normal scraper hydration, an authenticated global `limit=1` request returned HTTP 200 with `measurementState: measured` in about 4.1 seconds, reporting 1,235 sources, 58 executable, 52 ever-useful, 8 currently useful, and qualification counts of 14 clear-web, 0 lawful dark-web, and 8 public Telegram (22 total). The scraper was healthy with PostgreSQL storage available, zero pending writes, and restart count 0. This confirms the earlier homepage `0/0` issue and the source-operations hidden-metrics issue are separate; neither is being represented as a false zero. The 6,100-source requirement remains open.
...
Read-only repository audit at 2026-08-03 found no `darkwebIndexFixtureRecords`, `sampleDwmProductSnapshot`, or `ti/scraper/src/pipeline/evaluation.ts` in the production tree; evaluation metrics code itself has no `@ts-nocheck`. The STIX, frontier, planner, canary, in-memory storage, source-quality, public-channel, intelligence-profile, search-index, evidence-store, canary-operation, and memory-store suppressions were removed and type-check cleanly in commits `f533b7f7`, `fd879b64`, `b1bca714`, `13006652`, `cc0777ca`, `b0514c44`, `ebc84cc1`, `19d063df`, `0f5fa68a`, `44c3f816`, `660a7e11`, and `c04aeaa0`. The remaining legacy suppression count is now zero; focused storage/collection tests and scraper typecheck pass. This closes only the suppression sub-item; the broader reachability and live acceptance requirements remain open.
