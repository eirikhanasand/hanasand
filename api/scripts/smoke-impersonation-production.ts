...
const checks: Array<() => void> = [
    ...
    () => assertIncludes('api/src/utils/auth/tokenWrapper.ts', 'isImpersonationScopeAllowed', 'request-time impersonation scope guard'),
    () => assertIncludes('api/src/utils/auth/tokenWrapper.ts', 'impersonation.scope_denied', 'structured impersonation scope denial audit'),
    ...
    () => assertIncludes('api/src/handlers/impersonation.ts', 'validateSupportSessionForImpersonation', 'support session bound impersonation start'),
    () => assertIncludes('api/src/handlers/impersonation.ts', 'support_session_duration_exceeds_scope', 'impersonation expiry cannot exceed support session'),
    ...
    () => assertIncludes('frontend/src/app/api/backend/[...path]/route.ts', 'x-impersonation-token', 'web same-origin impersonation proxy'),
    () => assertNotIncludes('frontend/src/utils/impersonation/client.ts', 'getCookie(\'impersonation_token\')', 'client-readable impersonation token access'),
    () => assertIncludes('frontend/src/app/dashboard/system/impersonation/page.tsx', '/admin/audit-events', 'web structured audit page fetch'),
