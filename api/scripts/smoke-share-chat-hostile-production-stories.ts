...
const stories: Story[] = [
    ...
    { id: 225, title: 'Image Transcoding Worker With Validation', kind: 'worker', prompt: 'Create an image video transcoding worker queue with enqueue API worker status retry dead-letter counts event logs Redis production seam and no fake success.', mustMention: [/events/i, /dead/i, /retrying/i, /worker-status/i, /redis/i] },
    ...
    { id: 229, title: 'Incident Status Page With Subscribers', kind: 'website', prompt: 'Create an incident observability status page with service health incident timeline subscriber notice SLO evidence postmortems delivery notes accessible Docker.', mustMention: [/Service health/i, /Incident timeline/i, /Subscriber notice/i, /SLO evidence/i, /Postmortems/i] },
    ...
    { id: 232, title: 'Restaurant Allergy Safety Site', kind: 'website', prompt: 'Create a restaurant allergy safety mobile menu site with allergens dietary booking reservations hours private dining guest validation location update delivery Docker.', mustMention: [/allergen/i, /Reservations/i, /Opening hours/i, /Location/i, /Docker/i] },
    ...
    { id: 234, title: 'SEO Migration Landing Page', kind: 'website', prompt: 'Build an SEO migration landing page for a marketer angry about destroyed search traffic. Include metadata validation pricing FAQ redirects checklist lead capture accessible Docker.', mustMention: [/metadata/i, /Validation/i, /Pricing/i, /FAQ/i, /lead capture/i] },
    ...
    { id: 236, title: 'Data Retention Dashboard', kind: 'website', prompt: 'Build a data retention dashboard for privacy officer with retention rules export delete requests consent status audit trail ownership boundaries accessible responsive Docker.', mustMention: [/Retention rules/i, /Export request/i, /Delete request/i, /Audit trail/i, /Privacy and data paths documented/i] },
    { id: 237, title: 'Rate-Limited Public API', kind: 'api', prompt: 'Create a rate limited public API that implements rate limits with shaped 429 responses health readiness scoped records token gate Docker delivery.', mustMention: [/rateLimit/i, /Limit reached/i, /429/i, /ownerId/i, /ready/i] },
    { id: 238, title: 'Deployment Delivery Site', kind: 'website', prompt: 'Build a deployment delivery launch site for a founder burned by lock-in. Include export self-hosting env variables DNS SSL checklist rollback notes verification Docker.', mustMention: [/No platform lock-in/i, /Docker Compose/i, /Verification/i, /delivery|release notes|verification/i, /env/i] },
    ...
    { id: 240, title: 'Customer Portal API', kind: 'api', prompt: 'Build a customer portal API with account scoped records token gated writes validation idempotency rate limiting health readiness shaped errors production delivery.', mustMention: [/ownerId/i, /idempotency/i, /rateLimit/i, /API_TOKEN/i, /durable audit logs/i] },
]
...
async function verify(story: Story, target: string, files: ToolFile[]) {
    ...
    const checks: Record<string, boolean> = {
        ...
        delivery: /delivery|release notes|verification|docker compose|durable audit logs/i.test(await read(path.join(target, 'README.md'))),
    ...
    if (story.kind === 'website') {
        ...
        checks.productionTasks = /Verify forms, limits, and ownership rules|privacy|delivery|release notes/i.test(allContent)
    }
    if (story.kind === 'api') {
