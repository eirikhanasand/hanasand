...
const unitTasks: TestTask[] = (await readdir(path.join(scriptDir, '../tests')))
    ...
    .map(file => ({
        ...
        ...(['automation-history.test.mjs', 'monitoring-issues-postgres.test.ts', 'monitoring-message-history-postgres.test.ts', 'monitoring-case-workflow-postgres.test.ts', 'monitoring-vm-access-postgres.test.ts', 'case-development-postgres.test.ts', 'monitor-backlog-postgres.test.ts', 'personal-automations-postgres.test.ts'].includes(file) ? { requires: 'database' as const } : {}),
    }))
