...
assert.match(ws, /connectResumableWorker/, 'browser proxy should use a recoverable worker connection')
assert.match(readFileSync(new URL('../src/utils/ws/resumableWorker.ts', import.meta.url), 'utf8'), /await connectBrowserWorkerSocket/, 'browser proxy should wait for the worker websocket before failing the session')
