...
assert.match(ws, /NanoCpus:\s*2_000_000_000/, 'session worker should cap at the benchmarked two-core ceiling')
...
if (existsSync(composeUrl)) {
    ...
    assert.match(serviceBlock('browser-worker'), /cpus:\s*"2\.0"/, 'shared browser worker should use the benchmarked two-core ceiling')
