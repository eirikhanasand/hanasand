...
try {
    ...
    await queryOnce(`INSERT INTO agent_automations
        ...
        VALUES ('monitor-ti-delivery-reports',$1,'TI Delivery: missing reports',
          ...
          'interval',1,'active','agent_prompt','Europe/Oslo',15,4,$2,$3,'failure',false,NOW(),$4)
        ON CONFLICT (id) DO NOTHING`, [existing.owner_id, existing.model_name, existing.notification_destinations, existing.organization_id])
