...
assert.equal(app.videoPlaying, false, 'Show the branded loader while playback is paused')
...
assert.match(html, /video::-webkit-media-controls-overlay-play-button \{ display:none !important; \}/)
assert.doesNotMatch(html, /<video[^>]*visibility/, 'Autoplay video must remain visible while connecting')
assert.doesNotMatch(html, /video\.video\s*\{[^}]*visibility:\s*hidden/)
assert.match(html, /class="loading" v-if="!videoPlaying"/)
assert.doesNotMatch(html, /video::[^\n]+,/, 'Keep vendor selectors separate so an unsupported one cannot invalidate the others')
