...
const destination = await queryOnce(`SELECT owner_id, model_name, notification_destinations, organization_id FROM agent_automations
...
for (const [id, name, path, prompt] of [
    ...
    await queryOnce(`INSERT INTO agent_automations
        ...
        VALUES ($1,$2,$3,$4,$5,'fetch','interval',1,'active','agent_prompt','Europe/Oslo',5,4,$6,$7,'failure',false,NOW(),$8)
        ON CONFLICT (id) DO UPDATE SET target_url=EXCLUDED.target_url, status='active', next_run_at=NOW(),
          ...
          notify_on='failure', notify_warnings=false, timeout_seconds=5, retry_count=4, updated_at=NOW()`,
    [id, existing.owner_id, name, prompt, `https://api.hanasand.com/api/ai/health/${path}`, existing.model_name, existing.notification_destinations, existing.organization_id])
