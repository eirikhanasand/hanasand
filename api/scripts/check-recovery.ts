...
import { recoveryReadOnly, recoveryRequestAllowed } from '../src/utils/recovery.ts'
const root = mkdtempSync(join(tmpdir(), 'hanasand-recovery-'))
process.env.RECOVERY_STATE_FILE = join(root, 'state.json')
try {
    assert.equal(recoveryReadOnly(), true, 'Missing state must fail closed')
    writeFileSync(process.env.RECOVERY_STATE_FILE, JSON.stringify({ readOnly: true, updatedAt: new Date().toISOString() }))
    ...
    writeFileSync(process.env.RECOVERY_STATE_FILE, JSON.stringify({ readOnly: false, updatedAt: new Date(0).toISOString() }))
    await Bun.sleep(1010)
    assert.equal(recoveryReadOnly(), true, 'Stale state must fail closed')
    writeFileSync(process.env.RECOVERY_STATE_FILE, JSON.stringify({ readOnly: false, updatedAt: new Date().toISOString() }))
    await Bun.sleep(1010)
...
} finally { delete process.env.RECOVERY_STATE_FILE; delete process.env.RECOVERY_ESSENTIAL_ONLY; rmSync(root, { recursive: true }) }
