...
try {
    ...
    const template = (await queryOnce(`SELECT owner_id, model_name, notification_destinations FROM agent_automations
    ...
    for (const [site, name] of [['inspur', 'Inspur'], ['ovh', 'OVH']]) {
        const result = await queryOnce(`INSERT INTO agent_automations
            (id, owner_id, name, prompt, target_url, monitoring_type, json_rule, schedule_kind, interval_minutes,
             ...
             notify_on, notify_warnings, next_run_at)
            VALUES ($1,$2,$3,$4,$5,'json',$6::jsonb,'interval',1,'active','agent_prompt','Europe/Oslo',5,4,$7,$8,'failure',false,NOW())
        ...
        [`monitor-mail-relay-${site}`, template.owner_id, `Mail relay — ${name}`,
            ...
            template.model_name, template.notification_destinations])
