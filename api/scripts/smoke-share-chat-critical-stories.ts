...
const stories: Story[] = [
    ...
    { id: 210, title: 'Multi-Tenant Admin Dashboard', kind: 'website', prompt: 'Build a multi-tenant admin dashboard for AgencyScope with tenant metrics risk flags task queues empty states and future real data integration notes.', mustMention: [/metrics/i, /risks/i, /require a separate implementation/i, /docker/i] },
    ...
    { id: 212, title: 'Privacy-Sensitive Portfolio', kind: 'website', prompt: 'Build a privacy-sensitive portfolio for an artist worried about scraping and lock-in. Include case studies contact replacement guidance export and no external image dependency.', mustMention: [/COPY --from=builder/i, /contact/i, /export/i, /docker/i] },
    ...
    { id: 214, title: 'Image Review Queue', kind: 'website', prompt: 'Build an image gallery review queue for a photographer with keep reject later collections export summary counters accessible controls and no immediate deletion.', mustMention: [/Review queue/i, /Reject later/i, /Export summary/i, /:focus-visible/i] },
    ...
    { id: 219, title: 'Observability Status Page', kind: 'website', prompt: 'Create an observability status page with uptime validation incident timeline service cards customer messaging operational release notes and Docker export.', mustMention: [/validation|status|incident/i, /Verification/i, /Docker/i, /repeat\(auto-fit/i] },
