...
try {
    await withTransaction(async query => {
        ...
        `)
        const empty = (await query(sql, [], 'backlog-fixture')).rows[0]
        ...
        `)
        assert.deepEqual((await query(sql, [], 'backlog-fixture')).rows[0], {
        ...
        assert.equal((await query(sql, [], 'backlog-fixture')).rows[0].stale_reviews, 1)
    })
