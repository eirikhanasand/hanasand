...
const { default: ensureIssues } = await import('../src/utils/db/monitoringIssuesSchema.ts')
...
test('history pagination retains timestamp precision and recovery releases orphaned claims', async () => {
    ...
    const pageTwo = await loadRuns('a', 'owner', false, { page: '2' })
    const pageThree = await loadRuns('a', 'owner', false, { page: '3' })
    expect(pageTwo.nextPage).toBe(3)
    expect(pageThree.nextPage).toBeNull()
    expect(new Set([...first.runs, ...pageTwo.runs, ...pageThree.runs].map(run => run.id)).size).toBe(125)
