...
try {
    ...
    await query("UPDATE mill_rules SET enabled=FALSE WHERE organization_id='platform'")
    assert.equal(await analyzeAccess({ ...sample, key: 'disabled' }), false)
    await schemaInstall()
    assert.equal((await query('SELECT enabled FROM mill_rules')).rows[0].enabled, false, 'Restart cannot undo Disable')
    ...
    await query('UPDATE mill_rules SET definition=$1::jsonb', [JSON.stringify(accessDefinition)])
