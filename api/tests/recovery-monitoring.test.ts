...
test('replication and backup failures remain separate even when services respond', () => {
    const checks = recoveryChecks(state, now)
    ...
    expect(recoveryCheckMessage(checks, rule, false)).toBe(checks.ovh_replica.message)
    expect(recoveryCheckMessage(checks, rule, true)).toBeNull()
})

test('missing or stale observations never report recovery', () => {
    expect(() => recoveryChecks({ ...state, sampledAt: (now - 61000) / 1000 }, now)).toThrow('Recovery monitoring is unavailable.')
    expect(recoveryChecks({ ...state, database: {} }, now).ovh_replica.failed).toBe(true)
    expect(recoveryChecks({ ...state, database: {}, backups: { restoreSlots: ['hanasand_ovh_standby'] } }, now).ovh_replica.message).toContain('WAL replication lost')
})
...
test('recovery requires an active caught-up slot and a successful recent backup', () => {
    ...
    expect(recoveryChecks(recovered, now).ovh_replica.failed).toBe(false)
    expect(recoveryChecks(recovered, now).backup.failed).toBe(false)
    expect(recoveryChecks({ ...recovered, backupJob: { status: 'failed' } }, now).backup.failed).toBe(true)
})

test('changing failure detail keeps the same case and cooldown', () => {
    const monitor = { target_url: 'system:recovery', monitoring_type: 'json' as const, json_rule: rule }
...
test('recovery monitoring is restricted to administrators', () => {
    expect(needsSystemAutomationAccess({ actionType: 'agent_prompt', targetUrl: 'system:recovery', organizationId: null, modelName: null })).toBe(true)
    expect(automationReadScope('a', '$1', '$2')).toContain('a.target_url IS DISTINCT FROM \'system:recovery\'')
})
...
test('brief replica lag gets one minute, while sustained lag and outages fail', () => {
    ...
    const check = (overrides = {}, elapsed = 0) => recoveryChecks({ sampledAt: (now + elapsed) / 1000,
    ...
    expect(recoveryChecks({ sampledAt: now / 1000, database: { slots: [slot] },
