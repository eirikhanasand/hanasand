...
const { cdnRefreshRuleId } = await import('../src/utils/mill/analyzeCdnRefresh.ts')
...
test('only enabled Drop rules admit receipts; suspicious or incomplete events never reach the database', async () => {
    for (const enabled of [false, true]) {
        ...
        const query: any = async (sql: string, params: unknown[]) => {
            statements.push(sql)
            if (sql.includes('WHERE organization_id = $1')) return { rows: [] }
            if (sql.includes('FROM mill_rules')) {
                ...
                return { rows: enabled ? [{ organization_id: 'platform', version: '1' }] : [] }
            }
