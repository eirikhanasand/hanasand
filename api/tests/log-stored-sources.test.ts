...
let recentId = '100', watermark = '200', historyRows = 1, reads: Array<{ sql: string, values: unknown[] }>
...
const query = async (sql: string, values: unknown[] = []) => {
    ...
    if (sql.startsWith('SELECT *')) {
        ...
        return { rows: Array.from({ length: values[1] === watermark ? 1 : Math.min(historyRows, Number(values[2] || 1000)) }, (_, index) => ({ id: values[1] === watermark ? String(BigInt(recentId) + 1n) : String(index + 1), created_at: '2026-09-19T12:00:00Z', status: 'success' })) }
    }
...
beforeEach(() => { checkpoints = []; skipped = []; busy = null; recentId = '100'; watermark = '200'; historyRows = 1; reads = [] })
...
test('throttled history keeps every recent stream and advances only the evaluated historical rows', async () => {
    ...
    expect(reads.slice(0, 3).every(read => read.sql.includes('LIMIT 1000'))).toBe(true)
