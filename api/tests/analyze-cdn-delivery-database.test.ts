...
test.skipIf(!process.env.POSTGRES_FILTER_TEST_PORT)('real CDN ingestion keeps suspicious evidence and respects Keep, Disable, replay and rollback', async () => {
    ...
    const namespace = `cdn_delivery_${process.pid}_${Date.now()}`
    ...
    try {
        ...
        const entry = () => {
            ...
            log.metadata.structured.access.timestamp = log.timestamp
        ...
        for (const policy of [{ ...cdnDeliveryDefinition, conditions: [{ path: 'host', operator: 'equals', value: 'another-host' }] },
            { ...cdnDeliveryDefinition, conditions: [{ path: 'metadata.structured.access.ip', operator: 'equals', value: '127.0.0.1' }] }]) {
            await query('UPDATE mill_rules SET definition=$2::jsonb WHERE rule_id=$1', [cdnDeliveryRuleId, JSON.stringify(policy)])
        ...
        await query('UPDATE mill_rules SET definition=$2::jsonb WHERE rule_id=$1', [cdnDeliveryRuleId, JSON.stringify(cdnDeliveryDefinition)])
        for (const mode of ['disable', 'keep', 'custom-keep']) {
            if (mode === 'disable') await query('UPDATE mill_rules SET enabled=false WHERE rule_id=$1', [cdnDeliveryRuleId])
            if (mode === 'keep') await query("UPDATE mill_rules SET enabled=true,definition=jsonb_set(definition,'{action}','\"keep\"') WHERE rule_id=$1", [cdnDeliveryRuleId])
            if (mode === 'custom-keep') {
                await query('UPDATE mill_rules SET enabled=true,definition=$2::jsonb WHERE rule_id=$1', [cdnDeliveryRuleId, JSON.stringify(cdnDeliveryDefinition)])
