...
export default async function userHandler(req: FastifyRequest, res: FastifyReply) {
    ...
    try {
        const userResult = await run('SELECT id, name, avatar, active, deactivated_at, deactivated_by, deletion_requested_at, deletion_scheduled_at FROM users WHERE id = $1', [id])
