...
export function needsSystemAutomationAccess(input: AccessInput) {
    return input.actionType === 'mail_health_check' || input.targetUrl?.startsWith('system:cron:') || ['system:metrics', 'system:ti-delivery', 'system:ti-collection', 'system:ti-enrichment', 'system:recovery'].includes(input.targetUrl || '')
