...
export async function correlationKey(query: typeof queryOnce, a: AutomationRow, fingerprint: string, kind: string, message: string) {
    ...
    const restricted = ['system:metrics', 'system:ti-delivery', 'system:ti-collection', 'system:ti-enrichment', 'system:resilience'].includes(a.target_url || '') || a.action_type === 'mail_health_check'
