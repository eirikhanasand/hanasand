...
export function resilienceChecks(state: State, now = Date.now()): Record<string, Check> {
...
export function readResilienceChecks() {
    let state: State
    try { state = JSON.parse(readFileSync(process.env.RESILIENCE_MONITOR_STATE_FILE || '/resilience/state.json', 'utf8')) }
    catch { throw new Error('Recovery monitoring is unavailable.') }
    return resilienceChecks(state)
}

export function resilienceCheckMessage(payload: unknown, rule: JsonRule, inverted: boolean) {
