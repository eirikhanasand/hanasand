...
// Keep enough precision to distinguish readings immediately around an alert limit.
...
export function hostCheckMessage(rule: JsonRule, observed: unknown, failed: boolean, inverted = false): string | null {
    ...
    if (name && rule.operator === 'gt' && rule.aggregate === 'max' && typeof rule.value === 'number') {
        return `${name} usage is ${failed ? 'high' : 'normal'}: ${number(observed)}% used (alert at ${number(rule.value)}%).`
    }
    ...
    if (sensor && rule.operator === 'lt' && rule.aggregate === 'min' && rule.value === 0) {
        ...
        return `${sensor[0]} is ${failed ? 'high' : 'normal'}: ${number(Math.abs(observed))}${sensor[1]} ${observed < 0 ? 'above' : 'below'} the alert limit.`
    }
    return null
}
