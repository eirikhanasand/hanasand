...
export default async function ensureLogAnalyzeSchema() {
    ...
    await run(`WITH installed AS (
        ...
        SELECT gen_random_uuid()::text,o.id,$2,'1',$3,'HTTP','high',$4,$5::jsonb,'hanasand',TRUE
        ...
        FROM installed`, [process.env.PLATFORM_LOG_ORGANIZATION_ID || null, accessRule.id, accessRule.name, accessRule.explanation, JSON.stringify(accessDefinition)])
}
