...
export default async function ensureMonitoringIssuesSchema() {
    await run(`CREATE TABLE IF NOT EXISTS monitoring_issues (
        ...
        automation_id TEXT NOT NULL REFERENCES agent_automations(id),
    ...
    await run(`CREATE TABLE IF NOT EXISTS monitoring_issue_notifications (
        issue_id BIGINT NOT NULL REFERENCES monitoring_issues(id),
    ...
    await run('ALTER TABLE agent_automation_runs ADD COLUMN IF NOT EXISTS issue_id BIGINT REFERENCES monitoring_issues(id)')
