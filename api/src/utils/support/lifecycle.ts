...
export async function setSupportStatus(id: string, status: 'open' | 'closed', agentId: string) {
    return withTransaction(async query => {
        const ticket = (await query('SELECT * FROM support_tickets WHERE id=$1 FOR UPDATE', [id])).rows[0]
        if (!ticket) throw new SupportStateError('Support chat not found.', 404)
        if (ticket.status === status) return
        await query(`UPDATE support_tickets SET status=$2, updated_at=NOW(), ai_pending_id=NULL, ai_pending_at=NULL,
        ...
        await query(`INSERT INTO support_messages(id,ticket_id,sender_id,sender_kind,event,body)
            VALUES($1,$2,$3,'system',$4,$5)`, [randomUUID(), id, agentId, status === 'closed' ? 'resolved' : 'reopened',
            status === 'closed' ? 'Chat resolved. How was your experience? Please rate your support from 1 to 5 stars.' : 'Support reopened this chat. You can send messages again.'])
    })
}
