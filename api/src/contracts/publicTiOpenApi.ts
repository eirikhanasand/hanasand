...
const schemas: Record<string, Schema> = {
    ...
    SearchResult: object({
        query: string(), queryKind: string({ enum: ['actor', 'domain', 'cve', 'indicator', 'organization', 'free_text'] }), generatedAt: dateTime(), mode: string({ enum: ['scraper', 'seeded', 'live_search', 'unavailable'] }), cacheStatus: string({ enum: ['hit', 'miss'] }), status: string({ enum: ['queued', 'searching', 'partial', 'ready', 'metadata_review', 'blocked_unsafe_target', 'needs_source_activation'] }), runId: string(), refreshAfterSeconds: integer({ minimum: 0 }), summary: string(), confidence: confidence(), lastSeen: string(), aliases: stringArray(), recentActivity: { type: 'array', items: ref('Activity') }, targets: { type: 'array', items: ref('Target') }, ttps: { type: 'array', items: ref('Ttp') }, datasets: { type: 'array', items: ref('Dataset') }, sources: { type: 'array', items: ref('SearchSource') }, notes: stringArray(), actorIntelligence: ref('ActorIntelligence'), actionability: ref('Actionability'),
