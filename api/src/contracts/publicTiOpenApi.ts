...
const schemas: Record<string, Schema> = {
    ...
    Activity: object({
        date: string(), title: string(), detail: string(), confidence: confidence(), sourceIds: stringArray(), url: string({ format: 'uri' }), claimType: string(), victimName: string(), affectedSectors: stringArray(), countries: stringArray(), impact: string(), firstReportedAt: dateTime(), lastReportedAt: dateTime(), publisherCount: integer({ minimum: 0 }), corroboratingSourceIds: stringArray(), contradictingSourceIds: stringArray(), assertionKind: string(), reviewState: string(), corroborationState: string(), observationSummary: string(),
    ...
    Ttp: object({ name: string(), attackId: string(), tactic: string(), detail: string(), confidence: confidence(), extractionMethod: string({ maxLength: 120 }), extractorVersion: string({ maxLength: 120 }) }, ['name', 'tactic', 'detail', 'confidence']),
    ...
    SearchResult: object({
        query: string(), queryKind: string({ enum: ['actor', 'domain', 'cve', 'indicator', 'organization', 'free_text'] }), generatedAt: dateTime(), mode: string({ enum: ['scraper', 'seeded', 'live_search', 'unavailable'] }), cacheStatus: string({ enum: ['hit', 'miss'] }), status: string({ enum: ['queued', 'searching', 'partial', 'ready', 'metadata_review', 'blocked_unsafe_target', 'needs_source_activation', 'unavailable'] }), runId: string(), refreshAfterSeconds: integer({ minimum: 0 }), summary: string(), confidence: confidence(), lastSeen: string(), aliases: stringArray(), recentActivity: { type: 'array', items: ref('Activity') }, targets: { type: 'array', items: ref('Target') }, ttps: { type: 'array', items: ref('Ttp') }, datasets: { type: 'array', items: ref('Dataset') }, sources: { type: 'array', items: ref('SearchSource') }, notes: stringArray(), actorIntelligence: ref('ActorIntelligence'), actionability: ref('Actionability'),
    }, ['query', 'generatedAt', 'mode', 'summary', 'confidence', 'aliases', 'recentActivity', 'targets', 'ttps', 'datasets', 'sources', 'notes']),
