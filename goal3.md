...
Status: the old homepage `0/0` display is **resolved and closed**, not active work. A fresh production read returned `status: live` and `total: 2,781` with current exposure items. The current UI represents timeout/unavailable states as checking or unavailable and does not fabricate a zero count. Reopen this item only if a new live check reproduces the failure.
...
The evaluation endpoint was rechecked after the production scraper restart on 2026-08-03: the test split returned a 128,565-byte JSON response in 7.05 seconds with `quality.status=diagnostic_only`, `evaluatedUnitCount=0`, and no independent metrics. A request-scoped cache for repeated capture/source/reference lookups is committed and tested in `550217b6`; its first production image was rolled back after an unexpectedly long cold start, so it is not being claimed as deployed. The previously healthy scraper image was restored; PostgreSQL storage is healthy with `pendingWrites=0`. Independent evaluation remains open.
