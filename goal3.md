...
The evaluation endpoint was rechecked after the production scraper restart on 2026-08-03: the test split returned a 128,565-byte JSON response in 7.05 seconds with `quality.status=diagnostic_only`, `evaluatedUnitCount=0`, and no independent metrics. A request-scoped cache for repeated capture/source/reference lookups is committed and tested in `550217b6`, then deployed successfully in scraper image `sha256:95210e4b…` (container `442b0c7770f1`) after normal PostgreSQL hydration. The live endpoint now returns in 1.94 seconds; storage is healthy with `pendingWrites=0` and restart count 0. Independent evaluation remains open.
