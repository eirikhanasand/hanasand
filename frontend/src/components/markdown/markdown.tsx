...
const components = {
    ...
    a: ({ children, href }: { children: ReactNode, href?: string }) => <a className={href?.startsWith('#ti-reference-') ? 'markdown-render_citation' : 'markdown-render_a'} href={href}>{children}</a>
}

export default function MarkdownRender({ MDstr }: { MDstr: string }) {
    return (
        // @ts-expect-error The a type has a very nested error.
