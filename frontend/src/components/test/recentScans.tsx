...
export default function RecentScans({ title, empty, scans, mine = false, className = '', listClassName = '' }: RecentScansProps) {
    return (
        <section className={`grid min-h-0 min-w-0 grid-rows-[auto_minmax(0,1fr)] gap-3 rounded-lg border border-[#dfe5ee] bg-white p-4 shadow-sm ${className}`}>
            ...
            {!scans.length && (
                <div className='grid h-full min-h-0 place-items-center rounded-lg border border-dashed border-[#cfd7e4] bg-[#f8fafc] text-sm text-[#667085]'>
                    {empty}
                </div>
            )}
            {scans.length > 0 && (
                <div className={`grid min-h-0 gap-2 overflow-y-auto pr-1 ${listClassName}`}>
                    {scans.map((scan) => (
                        <Link
                            key={scan.id}
                            href={`/test/${scan.id}`}
                            className='grid min-w-0 gap-2 rounded-lg border border-[#e0e5ed] bg-[#f8fafc] px-3 py-3 transition hover:border-[#c9d2df] hover:bg-white'
                        >
                            <div className='flex min-w-0 items-center justify-between gap-3'>
                                <div className='min-w-0'>
                                    <div className='truncate text-sm font-medium text-[#171a21]'>{scan.url}</div>
                                    <div className='mt-1 flex min-w-0 flex-wrap items-center gap-3 text-xs text-[#667085]'>
                            ...
                            <div className='text-xs text-[#8c95a5]'>{prettyDate(scan.created_at)}</div>
                        </Link>
                    ))}
                </div>
            )}
...
function ScanStats({ scan }: { scan: Test }) {
    ...
    return (
        <div className='mt-2 flex min-w-0 flex-wrap gap-2 text-[11px] text-[#667085]'>
