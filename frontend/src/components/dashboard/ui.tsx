...
export function DashboardPage({ children, className = '' }: DashboardPageProps) {
    return <div className={`mx-auto grid w-full max-w-[1320px] gap-3 py-3 sm:gap-4 sm:py-4 md:py-6 ${className}`.trim()}>{children}</div>
}
...
export function DashboardHeader({
    ...
    return (
        <section className={`${dashboardPanelClass} overflow-hidden px-4 py-4 md:px-6 md:py-5`}>
            <div className='flex flex-col gap-3 md:flex-row md:items-end md:justify-between'>
                <div className='min-w-0'>
                    ...
                    {description && (
                        <p className='mt-1.5 max-w-3xl text-[13px] leading-6 text-bright/52 sm:text-sm'>{description}</p>
                    )}
                </div>
