...
export default function DeleteAccountButton({ name, onDelete, label, alwaysConfirm = false }: { name: string, onDelete: () => Promise<void>, label?: string, alwaysConfirm?: boolean }) {
    ...
    async function remove() {
        ...
        try {
            await onDelete()
            if (skip && !alwaysConfirm) { try { sessionStorage.setItem(storageKey(), 'skip') } catch { /* Storage may be disabled; keep asking for confirmation. */ } }
    ...
    return <>
        <button type='button' aria-label={`Delete ${name}`} disabled={pending}
            ...
            onClick={(event) => {
                ...
                let direct = false
                try { direct = !alwaysConfirm && sessionStorage.getItem(storageKey()) === 'skip' } catch { /* Storage may be disabled; keep asking for confirmation. */ }
