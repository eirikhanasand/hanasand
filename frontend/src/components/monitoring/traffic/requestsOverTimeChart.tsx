export default function RequestsOverTimeChart({ data }: { data: { key: string, count: number }[] }) {
    if (!data || data.length === 0) {
        return <div className='p-4 text-sm text-muted-foreground'>Traffic meter updates as requests arrive.</div>
    }
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className='w-full h-auto overflow-visible'>
            <defs>
                <linearGradient id='chartGradient' x1='0' y1='0' x2='0' y2='1'>
                    <stop offset='0%' stopColor='var(--ui-warning)' stopOpacity='0.28' />
                    <stop offset='100%' stopColor='var(--ui-warning)' stopOpacity='0' />
                </linearGradient>
            </defs>
            ...
            {data.length < 30 && data.map((d, i) => (
                <circle key={i} cx={xScale(i)} cy={yScale(d.count)} r={3} fill='var(--ui-warning)' />
            ))}
