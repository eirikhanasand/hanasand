...
export default function NewRequest({
    ...
    return (
        <div className='grid h-full min-h-0 gap-2 overflow-auto pr-1'>
            <form onSubmit={send} className='grid gap-2 rounded-lg border border-ui-border bg-ui-canvas/12 p-2'>
                <div className='grid gap-2 sm:grid-cols-[86px_minmax(0,1fr)_74px]'>
                    <label className='relative min-w-0'>
                        <span className='sr-only'>Method</span>
                        <select value={method} onChange={(e) => setMethod(e.target.value)} className='h-10 w-full cursor-pointer appearance-none rounded-full border border-ui-border bg-ui-panel/[0.045] px-3 pr-8 text-xs font-semibold text-ui-text outline-none focus:border-ui-warning/45'>
                            {METHODS.map((item) => <option key={item} value={item} className='bg-background'>{item}</option>)}
                        </select>
                        <ChevronDown className='pointer-events-none absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-ui-text/45' />
                    </label>
                    <input
                        ref={inputRef}
                        className='h-10 min-w-0 rounded-full border border-ui-border bg-ui-panel/[0.045] px-3 text-sm text-ui-text outline-none focus:border-ui-warning/45'
                        ...
                        required
                    />
                    <button type='submit' className='flex h-10 cursor-pointer items-center justify-center gap-1.5 rounded-full bg-ui-warning px-3 text-[11px] font-semibold text-background hover:bg-ui-warning'>
                        <Play className='h-3.5 w-3.5' />
                        Run
                    </button>
                </div>

                <div className='min-w-0 rounded-lg border border-ui-border bg-ui-canvas/18 px-3 py-2 font-mono text-[11px] leading-4 text-ui-text/62'>
                    <span className='mr-2 text-ui-text/32'>URL</span>
                    <span className='mr-2 text-ui-text/45'>{method}</span>
                    <span className='break-all' title={resolvedUrl}>{resolvedUrl || 'Enter a request URL.'}</span>
                    {resolvedUrl !== url ? (
                        <span className='mt-1 block text-ui-text/35' title={url}>Template: {url}</span>
                    ) : null}
                </div>

                <div className='flex flex-wrap items-center justify-between gap-2 px-1'>
                    <div className='flex flex-wrap gap-2 text-xs font-medium text-ui-text/70'>
                        {(['headers', 'body', 'vars', 'ai'] as const).map((item) => (
                            <button key={item} type='button' onClick={() => setTab(item)} className={`cursor-pointer rounded-full px-2.5 py-1 text-[11px] capitalize ${tab === item ? 'bg-ui-panel/12 text-ui-text' : 'text-ui-text/50 hover:bg-ui-panel/7 hover:text-ui-text/75'}`}>
                                {item === 'ai' ? 'AI' : item === 'vars' ? 'Vars' : item}
                            </button>
                        ))}
                    </div>
                    <div className='inline-flex items-center gap-2 rounded-full border border-ui-border px-3 py-1 text-[11px] text-ui-text/55'>
            ...
            <section className='grid min-h-0 gap-2 rounded-lg border border-ui-border bg-ui-canvas/12 p-2'>
                <div className='flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border border-ui-border bg-ui-canvas/16 px-3 py-2 text-[11px] text-ui-text/50'>
                    ...
                    <span className='inline-flex min-w-0 items-center gap-1.5'>
                        <span>Source</span>
                        <strong className='truncate font-semibold text-ui-text/82'>{executionMode}</strong>
                    </span>
                    <span className='inline-flex min-w-0 items-center gap-1.5'>
                        <Clock3 className='h-3 w-3 text-ui-text/35' />
                        <strong className='truncate font-semibold text-ui-text/82'>{response?.elapsed_ms !== undefined ? `${response.elapsed_ms} ms` : activeRun?.loading ? 'Running' : 'Pending'}</strong>
                    </span>
                </div>
                ...
                <div className='flex min-w-0 flex-wrap items-center justify-between gap-2'>
                    <div className='flex flex-wrap gap-2 text-xs font-medium text-ui-text/70'>
                        {(['response', 'raw', 'tree', 'preview', 'headers', 'request', 'curl', 'http'] as const).map((item) => (
                            <button key={item} type='button' onClick={() => setResponseTab(item)} className={`cursor-pointer rounded-full px-2.5 py-1 text-[11px] capitalize ${responseTab === item ? 'bg-ui-panel/12 text-ui-text' : 'text-ui-text/50 hover:bg-ui-panel/7 hover:text-ui-text/75'}`}>
                    ...
                    {response?.warnings?.length ? (
                        <span className='max-w-full truncate rounded-full bg-ui-warning/10 px-2.5 py-1 text-[11px] text-ui-warning/72' title={response.warnings.join('\n')}>
                            {response.warnings.length} warning{response.warnings.length === 1 ? '' : 's'}
                        </span>
                    ) : null}
                </div>

                <div className='grid min-h-44 overflow-hidden rounded-lg bg-ui-canvas/22'>
                    {responseTab === 'response' && (
                        <pre className='min-h-32 overflow-auto whitespace-pre-wrap wrap-break-word p-4 text-xs leading-5 text-ui-text/80'>
                            {activeRun?.loading ? 'Request is running...' : response ? formatPrettyResponseBody(response) : 'Run a request to stream the formatted response.'}
                        </pre>
                    )}

                    {responseTab === 'raw' && (
                        <pre className='min-h-32 overflow-auto whitespace-pre-wrap wrap-break-word p-4 text-xs leading-5 text-ui-text/80'>
                            {activeRun?.loading ? 'Request is running...' : response ? formatRawResponseBody(response) : 'Run a request to stream the raw response.'}
                        </pre>
                    )}

                    {responseTab === 'tree' && (
                        <div className='grid min-h-32 grid-rows-[auto_minmax(0,1fr)] overflow-hidden'>
                            <label className='flex h-10 items-center gap-2 border-b border-ui-border px-3 text-ui-text/45'>
                                ...
                                <input
                                    ...
                                    className='min-w-0 flex-1 bg-transparent text-xs text-ui-text/78 outline-none placeholder:text-ui-text/32'
                                />
                            </label>
                            <pre className='min-h-0 overflow-auto whitespace-pre-wrap wrap-break-word p-4 text-xs leading-5 text-ui-text/80'>
                                {activeRun?.loading ? 'Request is running...' : response ? formatJsonTree(response, treeFilter) : 'Run a request to inspect JSON paths.'}
                            </pre>
                        </div>
                    )}

                    {responseTab === 'preview' && (
                        <div className='grid min-h-32 place-items-center overflow-auto p-4 text-xs text-ui-text/55'>
                            {previewUrl ? (
                    ...
                    {responseTab === 'headers' && (
                        <div className='grid min-h-32 grid-rows-[auto_minmax(0,1fr)] overflow-hidden'>
                            ...
                            <pre className='min-h-0 overflow-auto whitespace-pre-wrap wrap-break-word p-4 text-xs leading-5 text-ui-text/75'>
                                {response?.headers ? formatFilteredHeaders(response.headers, headerFilter) : 'Run a request to inspect response headers.'}
                            </pre>
                        </div>
                    )}

                    {responseTab === 'request' && (
                        <div className='grid min-h-32 gap-3 overflow-auto p-4 text-xs text-ui-text/80'>
                            <div className='grid gap-1'>
                                <span className='text-[11px] uppercase tracking-[0.24em] text-ui-text/40'>Request line</span>
                                <pre className='whitespace-pre-wrap wrap-break-word rounded-lg bg-ui-panel/5 p-3'>{formatRequestLine(response?.request)}</pre>
                            </div>
                            <div className='grid gap-1'>
                                <span className='text-[11px] uppercase tracking-[0.24em] text-ui-text/40'>Request headers</span>
                                <pre className='whitespace-pre-wrap wrap-break-word rounded-lg bg-ui-panel/5 p-3'>{formatHeaders(response?.request?.headers)}</pre>
                            </div>
                            <div className='grid gap-1'>
                                <span className='text-[11px] uppercase tracking-[0.24em] text-ui-text/40'>Request body</span>
                                <pre className='whitespace-pre-wrap wrap-break-word rounded-lg bg-ui-panel/5 p-3'>{response?.request?.body || 'No request body.'}</pre>
                            </div>
                        </div>
                    )}

                    {responseTab === 'curl' && (
                        <pre className='min-h-32 overflow-auto whitespace-pre-wrap wrap-break-word p-4 text-xs leading-5 text-ui-text/78'>
            ...
            <section className='grid min-h-0 gap-2 rounded-lg border border-ui-border bg-ui-canvas/12 p-2'>
                {tab === 'headers' && (
                    <div className='grid min-h-0 content-start gap-2'>
                        ...
                        <button type='button' onClick={() => setHeaders((prev) => [...normalizeHeaderRows(prev), { key: '', value: '' }])} className='flex w-fit cursor-pointer items-center gap-2 rounded-full px-3 py-1.5 text-[11px] text-ui-text/60 hover:bg-ui-panel/8 hover:text-ui-text/80'>
                            <Plus className='h-3.5 w-3.5' />
                            Add header
                        </button>
                        {templateWarnings.length || headerPlan.warnings.length ? (
                            <div className='rounded-lg border border-ui-warning/12 bg-ui-warning/8 px-3 py-2 text-[11px] leading-4 text-ui-warning/72'>
                ...
                {tab === 'body' && (
                    <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder='JSON, text or form body' className='min-h-32 rounded-lg border border-ui-border bg-ui-panel/[0.025] p-3 font-mono text-sm text-ui-text outline-none' />
                )}
                ...
                {tab === 'vars' && (
                    <div className='grid min-h-0 content-start gap-2'>
                        ...
                        <button type='button' onClick={() => setAndSaveVariables([...normalizedVariables, { key: '', value: '' }])} className='flex w-fit cursor-pointer items-center gap-2 rounded-full px-3 py-1.5 text-[11px] text-ui-text/60 hover:bg-ui-panel/8 hover:text-ui-text/80'>
                            <Plus className='h-3.5 w-3.5' />
                            Add variable
                        </button>
                        <div className='rounded-lg border border-ui-border bg-ui-canvas/14 px-3 py-2 text-[11px] leading-4 text-ui-text/48'>
                ...
                {tab === 'ai' && (
                    <div className='grid min-h-0 gap-3 lg:grid-rows-[auto_auto_minmax(0,1fr)]'>
                        <textarea value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} className='min-h-24 rounded-lg border border-ui-border bg-ui-panel/[0.025] p-3 text-sm text-ui-text outline-none' />
                        <button type='button' onClick={askAi} disabled={aiLoading} className='flex w-fit cursor-pointer items-center gap-2 rounded-full bg-ui-primary/12 px-3 py-1.5 text-[11px] font-semibold text-ui-primary hover:bg-ui-primary/20 disabled:cursor-not-allowed disabled:opacity-60'>
                            <Bot className='h-3.5 w-3.5' />
                            {aiLoading ? 'Asking...' : 'Ask AI'}
                        </button>
                        <pre className='max-h-32 min-h-0 overflow-auto whitespace-pre-wrap wrap-break-word rounded-lg bg-ui-canvas/20 p-3 text-sm text-ui-text/70'>
                            {aiLoading ? 'Thinking through the request and response...' : aiResponse || 'Run analysis to review this request.'}
                        </pre>
                    </div>
                )}
            </section>

            <div className='flex max-w-full gap-1.5 overflow-x-auto rounded-lg border border-ui-border bg-ui-canvas/12 p-2'>
                {runs.length ? runs.map((run) => (
                    <button
                        key={run.id}
                        type='button'
                        onClick={() => setActiveRunId(run.id)}
                        className={`grid w-40 shrink-0 gap-1 rounded-lg border px-2.5 py-2 text-left transition ${activeRun?.id === run.id ? 'border-ui-warning/35 bg-ui-warning/8' : 'border-ui-border/8 bg-ui-panel/3 hover:bg-ui-panel/6'}`}
                    >
                        <span className='flex min-w-0 items-center gap-2 text-[11px] text-ui-text/78'>
                            <span className='font-semibold'>{run.method}</span>
                            <span className='truncate' title={run.url}>{shortUrlLabel(run.url)}</span>
                        </span>
                        <span className='truncate text-[10px] text-ui-text/45'>{run.loading ? 'Running' : run.response?.status || (run.response?.error ? 'Error' : 'Done')}</span>
                    </button>
                )) : (
                    <div className='rounded-lg border border-dashed border-ui-border px-3 py-2 text-xs text-ui-text/45'>
