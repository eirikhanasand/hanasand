    ...
    return (
        <DashboardPanel className='grid min-h-42 content-start gap-3 p-4'>
            <div className='flex items-start justify-between gap-3'>
                <div>
                    <h2 className='text-base font-semibold text-[#171a21]'>Customer handoffs</h2>
                    <p className='mt-1 text-sm text-[#596170]'>
                        {typeof shares === 'string' ? 'Unavailable' : `${shares.length} handoff link${shares.length === 1 ? '' : 's'}`}
                    </p>
                </div>
            ...
            <div className='grid gap-1'>
                {typeof shares === 'string'
                    ...
                    : (shares as Share[]).length
                        ...
                        : <p className='rounded-lg border border-dashed border-[#d8dee9] p-4 text-sm text-[#596170]'>No customer handoffs yet. Create one when a monitoring result needs to be reviewed outside the console.</p>}
            </div>
        </DashboardPanel>
    )
}
