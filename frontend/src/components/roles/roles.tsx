...
export default async function Roles({ roles }: { roles: Role[] }) {
    ...
    return <RoleList roles={roles} canManage={assigned.some(role => ['administrator', 'user_admin'].includes(role.role_id))} highestPriority={Math.min(...ownRoles.map(role => role.priority))} />
}
