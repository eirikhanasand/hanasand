import { readFile } from 'node:fs/promises'
import type { DwmDeliveryItem } from '@/app/dashboard/operatorConsoleModel'

export type ProductWebhookDeliveryProofLedger = {
    schemaVersion: 'product.webhook_delivery_proof_ledger.v1'
    generatedAt?: string
    tenantId?: string
    organizationId?: string
    source?: string
    ledgerPath?: string
    deliveries: DwmDeliveryItem[]
}

const INLINE_LEDGER_KEYS = [
    'PRODUCT_PROGRESS_WEBHOOK_DELIVERY_PROOF_JSON',
    'HANASAND_PRODUCT_PROGRESS_WEBHOOK_DELIVERY_PROOF_JSON',
] as const

const FILE_LEDGER_KEYS = [
    'PRODUCT_PROGRESS_WEBHOOK_DELIVERY_PROOF_PATH',
    'HANASAND_PRODUCT_PROGRESS_WEBHOOK_DELIVERY_PROOF_PATH',
] as const

export async function loadProductWebhookDeliveryProofLedger(scope: {
    tenantId: string
    organizationId?: string
}, env: Record<string, string | undefined> = process.env): Promise<ProductWebhookDeliveryProofLedger | undefined> {
    const inlineLedger = parseJsonLedger(firstValue(INLINE_LEDGER_KEYS, env), scope)
    if (inlineLedger) return inlineLedger

    const filePath = firstValue(FILE_LEDGER_KEYS, env)
    if (!filePath) return undefined

    try {
        const parsedFile = parseJsonLedger(await readFile(filePath, 'utf8'), scope)
        return parsedFile ? { ...parsedFile, ledgerPath: parsedFile.ledgerPath || filePath } : undefined
    } catch {
        return undefined
    }
}

export function webhookDeliveryPayloadFromLedger(ledger: ProductWebhookDeliveryProofLedger) {
    return {
        deliveries: ledger.deliveries,
        proofLedger: {
            schemaVersion: ledger.schemaVersion,
            generatedAt: ledger.generatedAt,
            source: ledger.source,
            ledgerPath: ledger.ledgerPath,
        },
    }
}

function parseJsonLedger(input: string | undefined, scope: { tenantId: string, organizationId?: string }) {
    if (!input?.trim()) return undefined
    try {
        return parseProductWebhookDeliveryProofLedger(JSON.parse(input), scope)
    } catch {
        return undefined
    }
}

export function parseProductWebhookDeliveryProofLedger(input: unknown, scope: {
    tenantId: string
    organizationId?: string
}): ProductWebhookDeliveryProofLedger | undefined {
    if (!isRecord(input)) return undefined
    if (input.schemaVersion !== 'product.webhook_delivery_proof_ledger.v1') return undefined

    const ledgerTenantId = stringOrUndefined(input.tenantId)
    if (ledgerTenantId && ledgerTenantId !== scope.tenantId) return undefined
    const ledgerOrganizationId = stringOrUndefined(input.organizationId)
    if (ledgerOrganizationId && scope.organizationId && ledgerOrganizationId !== scope.organizationId) return undefined
    if (!Array.isArray(input.deliveries)) return undefined

    const deliveries = input.deliveries.filter(isDwmDeliveryItem)
    if (deliveries.length === 0) return undefined
    if (!deliveries.some(delivery => delivery.status !== 'failed' && delivery.status !== 'skipped')) return undefined

    return {
        schemaVersion: 'product.webhook_delivery_proof_ledger.v1',
        generatedAt: stringOrUndefined(input.generatedAt),
        tenantId: ledgerTenantId,
        organizationId: ledgerOrganizationId,
        source: stringOrUndefined(input.source),
        ledgerPath: stringOrUndefined(input.ledgerPath),
        deliveries,
    }
}

function isDwmDeliveryItem(input: unknown): input is DwmDeliveryItem {
    if (!isRecord(input)) return false
    return Boolean(
        stringOrUndefined(input.id)
        && stringOrUndefined(input.alertId)
        && stringOrUndefined(input.watchlistId)
        && stringOrUndefined(input.endpointHash)
        && stringOrUndefined(input.attemptedAt)
        && stringOrUndefined(input.payloadHash)
        && stringOrUndefined(input.status),
    )
}

function isRecord(input: unknown): input is Record<string, unknown> {
    return Boolean(input && typeof input === 'object' && !Array.isArray(input))
}

function stringOrUndefined(input: unknown) {
    return typeof input === 'string' && input.trim() ? input.trim() : undefined
}

function firstValue(keys: readonly string[], env: Record<string, string | undefined>) {
    for (const key of keys) {
        const value = env[key]
        if (value?.trim()) return value.trim()
    }
    return undefined
}
