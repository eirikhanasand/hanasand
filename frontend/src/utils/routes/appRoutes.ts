...
export function hasAppSidebar(path: string) {
    return isInternalAppPath(path) || matches(path, '/profile') || matches(path, '/ti') || path === '/api' || path === '/api/openapi' || matches(path, '/browser') || path.startsWith('/solutions/') || matches(path, '/pwned') || matches(path, '/test')
}
