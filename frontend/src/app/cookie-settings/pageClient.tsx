    ...
    return (
        <main className='min-h-[calc(100vh-4.5rem)] bg-ui-canvas text-ui-text'>
            ...
            <section className='mx-auto grid max-w-5xl gap-4 px-4 py-10 md:px-8'>
                <div className='rounded-lg border border-ui-border bg-ui-panel p-4 shadow-sm'>
                    <div className='flex flex-wrap items-center justify-between gap-3'>
                        <div className='flex items-center gap-3'>
                            ...
                            <div>
                                <p className='text-sm font-semibold text-ui-text'>{consentLabel}</p>
                                <p className='text-sm text-ui-muted'>{snapshot.consentUpdatedAt ? `Updated ${formatDate(snapshot.consentUpdatedAt)}` : 'This browser has not saved a consent choice yet.'}</p>
                            </div>
                        </div>
                        <p className='rounded-full border border-ui-border bg-ui-raised px-3 py-1.5 text-xs font-semibold text-ui-muted'>{message}</p>
                    </div>
                </div>
                ...
                <div className='grid gap-4 lg:grid-cols-[1fr_0.8fr]'>
                    <section className='rounded-lg border border-ui-border bg-ui-panel p-5 shadow-sm'>
                        <div className='flex items-start justify-between gap-3'>
                            ...
                            <ShieldCheck className='h-5 w-5 text-ui-success' />
                        </div>
                    ...
                    <section className='rounded-lg border border-ui-border bg-ui-panel p-5 shadow-sm'>
                        ...
                        <Link
                            href='/logout'
                            className='mt-4 inline-flex h-10 items-center gap-2 rounded-md border border-ui-border bg-ui-raised px-3 text-sm font-semibold text-ui-text shadow-sm transition hover:border-ui-primary'
                        >
                ...
                <section className='rounded-lg border border-ui-border bg-ui-panel p-5 text-sm leading-7 text-ui-muted shadow-sm'>
    ...
    return (
        <button
            ...
            className={`rounded-lg border p-3 text-left transition ${
                active
                    ? 'border-ui-primary bg-ui-primary text-ui-canvas shadow-sm'
        ...
        >
            ...
            <span className={`mt-1 block text-xs ${active ? 'text-ui-canvas/85' : 'text-ui-muted'}`}>{detail}</span>
        </button>
    )
}
...
function StatusRow({ label, present }: { label: string, present: boolean }) {
    return (
        <div className='flex items-center justify-between gap-3 rounded-md border border-ui-border bg-ui-raised px-3 py-2 text-sm'>
            <span className='font-medium text-ui-text'>{label}</span>
            <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${present ? 'border-ui-success text-ui-success' : 'border-ui-border text-ui-muted'}`}>
    ...
    return (
        <div className='rounded-md border border-ui-border bg-ui-raised p-3'>
            <div className='flex items-center justify-between gap-3'>
                ...
                <span className='inline-flex items-center gap-1 rounded-full border border-ui-border bg-ui-panel px-2.5 py-1 text-xs font-semibold text-ui-text'>
            ...
            <div className='mt-3 flex flex-wrap gap-2'>
                {keys.length ? keys.map((key) => (
                    <span key={key} className='rounded-full border border-ui-border bg-ui-panel px-2.5 py-1 text-xs font-medium text-ui-muted'>{friendlyKey(key)}</span>
                )) : (
                    <span className='text-sm text-ui-muted'>Nothing product-local stored here.</span>
                )}
            </div>
        </div>
    )
