...
import { reviewStatus, type CodeInventory, type CodeItem, type ReviewEvent } from '@/utils/codeReviewTypes'
...
function StatusIcon({ item }: { item: CodeItem }) {
    ...
    return <span className='code-status' data-status={status} title={statusNames[status]}>{status === 'approved' ? <Check size={12} /> : status === 'changed' ? <RotateCcw size={12} /> : <Circle size={10} />}</span>
}
...
export default function CodeReview() {
    ...
    const load = useCallback(async() => {
        ...
        try {
            const response = await fetch('/api/thesis/code', { cache: 'no-store' }), result = await response.json()
            if (!response.ok) throw new Error(result.error)
            setData(result)
    ...
    const filtered = useMemo(() => scoped.filter(item => (item.title + ' ' + item.file).toLowerCase().includes(query.toLowerCase()) && (status === 'all' || reviewStatus(item) === status)), [scoped, query, status])
    ...
    return <section className='code-workspace' aria-label='Code review workspace'>
        ...
        {error && <p role='alert'>{error}</p>}{loading && <p role='status'>Loading inventory…</p>}
        {data && <>
            ...
            <p className='code-caption'>{scoped.filter(node => reviewStatus(node) === 'approved').length} of {scoped.length} items approved. Review each item individually; approving a page does not approve its files.</p>
