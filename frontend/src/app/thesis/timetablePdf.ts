...
import { expectedHoursText, hoursText, isoWeek, type ActivityLog, type timetable } from './timetableData'
...
export async function exportTimetablePdf(title: string, model: ReturnType<typeof timetable>, log: ActivityLog) {
    ...
    pdf.setFontSize(9)
    pdf.text(`${log.startYear}-${model.weeks.at(-1)?.year || log.startYear} | ${hoursText(total)} logged / ${expectedHoursText(model.expectedHours)} expected hours | Exported ${new Date().toISOString().slice(0, 10)}`, 14, 28)
    ...
    if (!body.length) body.push([{ content: 'No activity.', colSpan: 5 }])
    let reportEnd = 34
    ...
    const notes = [...(model.weeks.some(week => week.legacy) ? ['* Includes earlier hours without activity entries.'] : []), ...model.notes]
    if (notes.length) {
        autoTable(pdf, { startY: reportEnd + 8, margin: { top: 15, bottom: 18, left: 14, right: 14 }, styles, headStyles, head: [['Notes']], body: notes.map(note => [note]) })
    }
