...
export async function GET() {
    try {
        const response = await fetch(process.env.RECOVERY_STATUS_URL || 'https://api.hanasand.com/api/recovery/status', { cache: 'no-store', signal: AbortSignal.timeout(3000) })
        ...
        if (process.env.RECOVERY_STATE_FILE && await canViewHostMetrics()) {
            try {
                const privateState = JSON.parse(await readFile(process.env.RECOVERY_STATE_FILE, 'utf8'))
