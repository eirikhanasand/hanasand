...
import { Archive, BellRing, Building2, CheckCircle2, CircleAlert, ExternalLink, Loader2, Pause, Pencil, Play, RefreshCw, Settings, ShieldCheck, Trash2, UserPlus, Users, Webhook } from 'lucide-react'
                            ...
                            <div className='grid gap-5'>
                                ...
                                <section className='grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]'>
                                    ...
                                    <div className='grid gap-5 content-start'>
                                        <InvitePanel emails={inviteEmails} setEmails={setInviteEmails} role={inviteRole} setRole={setInviteRole} invites={bundle.invites} canManage={canManage} busy={busy} onInvite={() => void sendInvite()} onInviteAction={(invite, action) => void inviteAction(invite, action)} />
...
function SettingsPanel({ settingsDraft, setSettingsDraft, canManage, busy, onSave }: { settingsDraft: OrganizationSettings, setSettingsDraft: (next: OrganizationSettings) => void, canManage: boolean, busy: string, onSave: () => void }) {
    return (
        <section className='rounded-lg border border-[#dfe5ee] bg-white p-4 shadow-sm dark:border-[#273345] dark:bg-[#111927]'>
            <SectionTitle icon={<Settings className='h-4 w-4' />} title='Organization settings' detail={canManage ? 'Owners and admins can update workspace policies.' : 'Your role can read settings.'} />
...
function InvitePanel({ emails, setEmails, role, setRole, invites, canManage, busy, onInvite, onInviteAction }: { emails: string, setEmails: (value: string) => void, role: OrganizationRole, setRole: (value: OrganizationRole) => void, invites: OrganizationInvite[], canManage: boolean, busy: string, onInvite: () => void, onInviteAction: (invite: OrganizationInvite, action: 'revoke' | 'resend') => void }) {
    return (
        <section className='rounded-lg border border-[#dfe5ee] bg-white p-4 shadow-sm dark:border-[#273345] dark:bg-[#111927]'>
            <SectionTitle icon={<UserPlus className='h-4 w-4' />} title='Invites' detail={canManage ? 'Invite teammates and recover pending access.' : 'Invite management requires owner or admin.'} />
            <div className='mt-4 grid gap-3'>
                <label className='grid gap-1 text-sm font-medium text-[#344054] dark:text-[#cbd5e1]'>
                    Emails
            ...
            <div className='mt-5 grid gap-2'>
                ...
                {invites.map(invite => (
                    <div key={invite.id} className='grid gap-2 rounded-lg border border-[#e6ebf2] p-3 dark:border-[#26344a]'>
                        <div className='flex items-start justify-between gap-3'>
                            <div className='min-w-0'>
                                <p className='truncate text-sm font-semibold text-[#171a21] dark:text-white'>{invite.email}</p>
                                <p className='text-xs text-[#667085] dark:text-[#a8b3c5]'>{invite.role} · {invite.status}</p>
                            </div>
                            <RoleBadge role={invite.role} />
                        </div>
                        {canManage && (
                            <div className='grid grid-cols-2 gap-2'>
                                <button type='button' className={secondaryButtonClass} disabled={Boolean(busy)} onClick={() => onInviteAction(invite, 'resend')}>
                                    <RefreshCw className='h-4 w-4' />
                                    Resend
                                </button>
                                <button type='button' className={dangerButtonClass} disabled={Boolean(busy)} onClick={() => onInviteAction(invite, 'revoke')}>
...
function MemberPanel({ members, canManage, busy, onRoleChange, onRemove }: { members: OrganizationMember[], canManage: boolean, busy: string, onRoleChange: (member: OrganizationMember, role: OrganizationRole) => void, onRemove: (member: OrganizationMember) => void }) {
    return (
        <section className='rounded-lg border border-[#dfe5ee] bg-white p-4 shadow-sm dark:border-[#273345] dark:bg-[#111927]'>
            <SectionTitle icon={<Users className='h-4 w-4' />} title='Members' detail='Active users with organization-scoped access.' />
            <div className='mt-4 grid gap-2'>
                {members.length === 0 && <EmptyLine text='No active members returned.' />}
                {members.map(member => (
                    <div key={member.userId} className='grid gap-3 rounded-lg border border-[#e6ebf2] p-3 dark:border-[#26344a]'>
                        <div className='flex items-start justify-between gap-3'>
                            <div className='min-w-0'>
                                <p className='truncate text-sm font-semibold text-[#171a21] dark:text-white'>{member.name || member.userId}</p>
                        ...
                        {canManage && member.role !== 'owner' && (
                            <div className='grid gap-2 sm:grid-cols-[1fr_auto]'>
                                <select className={inputClass} value={member.role} disabled={Boolean(busy)} onChange={event => onRoleChange(member, event.target.value as OrganizationRole)}>
                                    {roleOptions.map(option => <option key={option} value={option}>{option}</option>)}
                                </select>
                                <button type='button' className={dangerButtonClass} disabled={Boolean(busy)} onClick={() => onRemove(member)}>
                                    <Trash2 className='h-4 w-4' />
                                    Remove
                                </button>
                            </div>
...
function ActivityPanel({ activity }: { activity: ActivityItem[] }) {
    return (
        <section className='rounded-lg border border-[#dfe5ee] bg-white p-4 shadow-sm dark:border-[#273345] dark:bg-[#111927]'>
            <SectionTitle icon={<CheckCircle2 className='h-4 w-4' />} title='Action history' detail='Recent organization changes from this workspace.' />
            <div className='mt-4 grid gap-2'>
...
const dangerButtonClass = 'inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-[#fecdca] bg-[#fffbfa] px-3 text-sm font-semibold text-[#b42318] transition hover:bg-[#fff1f0] disabled:cursor-not-allowed disabled:opacity-55 dark:border-[#7f1d1d] dark:bg-[#2a1010] dark:text-[#fecaca] dark:hover:bg-[#3b1414]'
