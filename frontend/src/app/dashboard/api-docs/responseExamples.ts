...
export const responseExamples: Record<string, ResponseExample> = {
    ...
    'GET /browser/runs': example({ runs: [{ id: 'run_example', target: 'https://vendor.example.com', network: 'regular', status: 'ended', startedAt: at, checkCount: 1, title: 'Vendor documentation', providerResults: {} }], quota: { plan: 'browser', limit: null, used: 4, remaining: null, resetsAt: null, identityKind: 'user', paid: true, advancedAnalysis: true, sessionSeconds: 1800, concurrentLimit: 3, active: 0 } }),
    ...
    'GET /ti/enrichment': example({ ok: false, error: 'api_ti_enrichment_retired', canonicalRoute: '/v1/intel/search' }, 410),
    'POST /ti/enrichment/run': example({ ok: false, error: 'api_ti_enrichment_retired', canonicalRoute: '/v1/intel/search' }, 409),
    ...
    'GET /billing/subscription': example({ subscription: { planId: 'browser', status: 'active', currentPeriodStart: '2026-06-01T00:00:00Z', currentPeriodEnd: '2026-07-01T00:00:00Z', cancelAtPeriodEnd: false }, entitlements: [{ planId: 'browser', quotas: {}, features: ['30-minute browser sessions', '3 simultaneous browsers', 'automated analysis profiles', 'evidence capture and run history'] }], quotas: [] }),
