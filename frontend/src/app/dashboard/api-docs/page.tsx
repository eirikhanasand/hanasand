...
export default async function ApiDocsPage() {
    ...
    if (!contract) {
        return <DashboardPage><DashboardHeader eyebrow='Developer tools' title='API docs' description='The API contract could not be loaded.' /><DashboardPanel className='p-4 text-sm text-ui-danger'>The live API contract is temporarily unavailable. Try again shortly.</DashboardPanel></DashboardPage>
    }
    ...
    return (
        <DashboardPage>
            <DashboardHeader
                ...
                actions={<Link href='/developers' target='_blank' rel='noopener noreferrer' className='inline-flex h-9 items-center gap-2 rounded-md border border-ui-border bg-ui-raised px-3 text-xs font-semibold text-ui-text transition hover:border-ui-primary'>Public guide <ExternalLink className='h-3.5 w-3.5' /></Link>}
            />
