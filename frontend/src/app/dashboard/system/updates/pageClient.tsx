...
export default function AptUpdatesClient() {
    ...
    return <div className='grid gap-3'>
        <DashboardPanel className='grid gap-4 p-4'>
            <div className='flex flex-wrap items-start justify-between gap-3'>
                ...
                <div className='ml-auto flex flex-wrap items-center justify-end gap-2'>
                    <span className='whitespace-nowrap text-sm text-ui-muted' title={formatDate(status?.checked_at)}>Checked {formatLastCheck(status?.checked_at)}</span>
                    <div className='flex shrink-0 items-center gap-2'><select aria-label='Host' value={host} onChange={event => { request.current?.abort(); setStatus(null); setHistory([]); setError(''); setLoading(true); setHost(event.target.value as UpdateHost) }} className='h-9 rounded-lg border border-ui-border bg-ui-raised px-3 text-sm font-semibold'><option value='inspur'>Inspur</option><option value='ovhcloud'>OVH</option></select><button disabled={loading} onClick={() => void load()} className='inline-flex h-9 items-center gap-2 rounded-lg border border-ui-border bg-ui-raised px-3 text-sm font-semibold hover:border-ui-primary disabled:opacity-60'><RefreshCcw className='h-4 w-4' />{loading ? 'Checking' : 'Refresh'}</button></div>
                </div>
            </div>
            ...
            <div className='grid gap-2 lg:grid-cols-3'>
                ...
                <Detail label='Policy' value={`Security updates immediately; other Ubuntu updates after ${delayHours} hours.`} />
        ...
        <DashboardPanel className='grid gap-3 p-4'><h2 className='text-base font-semibold'>History</h2>{history.length ? <div className='grid gap-2'>{history.map(item => <div key={item.run_id} className='grid items-center gap-1 rounded-lg border border-ui-border bg-ui-raised p-3 text-sm md:grid-cols-[180px_24px_minmax(0,1fr)]'><span className='text-ui-muted'>{item.is_today ? 'Today' : new Date(item.occurred_at).toLocaleDateString(undefined, { timeZone: 'UTC' })}</span><span className='flex items-center'><HistoryStatus item={item} /></span><div><p>{item.packages?.length ? item.packages.join(', ') : item.is_today ? 'Nothing installed yet' : 'No packages installed.'}</p>{item.error ? <p className='text-ui-danger'>{formatUpdateError(item.error)}</p> : null}</div></div>)}</div> : <p className='text-sm text-ui-muted'>No completed days yet.</p>}</DashboardPanel>
    </div>
}
