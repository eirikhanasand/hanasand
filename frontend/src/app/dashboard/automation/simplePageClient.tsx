...
export default function AutomationsClient({ setup, initial, mode = 'health' }: { setup?: 'dwm', initial: InitialAutomationData, mode?: 'health' | 'cron' }) {
    ...
    const newDraft = (): AutomationPayload => mode === 'cron' ? { ...defaultDraft(), name: 'Scheduled reminder', prompt: '', actionType: 'system_alert', notifyOn: 'always' } : defaultDraft()
    ...
    async function load(selectedId = selected?.id) {
        setBusy('load')
        try {
            const result = await fetchAutomations(scope)
            setCanManageSystem(Boolean(result.canManageSystem))
            setLoadError('')
    ...
    useEffect(() => {
        ...
        const refresh = window.setInterval(() => {
            void fetchAutomations(scope).then(result => {
                setCanManageSystem(Boolean(result.canManageSystem))
                setAutomations(result.automations)
            ...
            <div className={`grid gap-4 ${detailsOpen ? 'xl:grid-cols-[minmax(0,1fr)_minmax(22rem,0.72fr)]' : ''}`}>
                <section className='rounded-xl border border-ui-border bg-ui-panel shadow-sm'>
                    <div className='flex flex-wrap items-center justify-between gap-3 border-b border-ui-border p-4'>
                        <div><h2 className='text-lg font-semibold text-ui-text'>{mode === 'cron' ? 'Personal jobs' : 'Health checks'}</h2><p className='mt-1 text-sm text-ui-muted'>{automations.length} check{automations.length === 1 ? '' : 's'} configured</p></div>
                        ...
                        <div className='flex max-w-full flex-wrap items-center gap-2'>
                            {searchOpen && <input ref={searchInput} id='health-check-search' aria-label='Find a case or monitor' className={`${inputClass} max-w-56`} type='search' placeholder='HA-123 or monitor name' value={caseSearch} onChange={event => setCaseSearch(event.target.value)} onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); closeSearch() } }} />}
                ...
                <section id='health-check-details' hidden={!detailsOpen} className='rounded-xl border border-ui-border bg-ui-panel p-4 shadow-sm'>
                    {editing ? <AutomationForm canManageSystem={canManageSystem} draft={draft} selected={selected} busy={busy} message={message} onChange={setDraft} onSave={() => void save()} onCancel={() => { setEditing(false); setMessage('') }} /> : selected ? <AutomationDetails automation={selected} runs={history.runs} history={history} historyFrom={historyFrom} historyTo={historyTo} onHistoryFromChange={setHistoryFrom} onHistoryToChange={setHistoryTo} showHistoryFilters={showHistoryFilters} onShowHistoryFiltersChange={setShowHistoryFilters} busy={busy} message={message} onRun={() => void runNow()} onEdit={() => setEditing(true)} onDelete={() => void remove()} onRefresh={history.refresh} /> : <WelcomeState onCreate={beginCreate} compact />}
                </section>
            </div>
        </div>
    )
...
function AutomationForm({ canManageSystem, draft, selected, busy, message, onChange, onSave, onCancel }: { canManageSystem: boolean, draft: AutomationPayload, selected: AgentAutomation | null, busy: string, message: string, onChange: (draft: AutomationPayload) => void, onSave: () => void, onCancel: () => void }) {
    const validation = validateDraft(draft)
    return <div className='grid gap-4'><div><h2 className='text-xl font-semibold text-ui-text'>{selected?.name || 'Create an automation'}</h2><p className='mt-1 text-sm text-ui-muted'>Define a monitoring job and where to send its alerts.</p></div>
        ...
        <label className='grid gap-1.5'><span className='text-xs font-semibold text-ui-muted'>Automation type</span><select className={inputClass} value={draft.actionType} onChange={event => onChange({ ...draft, actionType: event.target.value as AutomationPayload['actionType'] })}><option value='agent_prompt'>Monitoring</option>{canManageSystem && <option value='mail_health_check'>Mail health</option>}<option value='system_alert'>Scheduled message</option><option value='echo'>Record a reminder in job history</option></select></label>
        {draft.actionType === 'agent_prompt' ? <>
            ...
            {draft.monitoringType === 'json' && <JsonRuleForm canManageSystem={canManageSystem} draft={draft} onChange={onChange} inputClass={inputClass} />}
        ...
        <label className='grid gap-1.5'><span className='text-xs font-semibold text-ui-muted'>Schedule</span><select className={inputClass} value={draft.scheduleKind} onChange={event => onChange({ ...draft, scheduleKind: event.target.value as AutomationPayload['scheduleKind'] })}><option value='interval'>Repeating</option><option value='once'>Once</option></select></label>
        {validation?.field === 'schedule' && <span className='text-sm text-ui-danger'>{validation.message}</span>}
        {draft.scheduleKind === 'once' && <label className='grid gap-1.5'><span className='text-xs font-semibold text-ui-muted'>Run at</span><input type='datetime-local' required className={inputClass} value={draft.runAt ? new Date(new Date(draft.runAt).getTime() - new Date(draft.runAt).getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ''} onChange={event => onChange({ ...draft, runAt: event.target.value ? new Date(event.target.value).toISOString() : '' })} /></label>}
        <div className='grid gap-3 sm:grid-cols-2'><label className={draft.scheduleKind === 'once' ? 'hidden' : 'grid gap-1.5'}><span className='text-xs font-semibold text-ui-muted'>Run every</span><div className='flex items-center gap-2'><input className={inputClass} type='number' min={1} value={draft.intervalMinutes || ''} onChange={event => onChange({ ...draft, intervalMinutes: Number(event.target.value) || null })} /><span className='text-sm text-ui-muted'>{draft.intervalMinutes === 1 ? 'minute' : 'minutes'}</span></div></label><label className='grid gap-1.5'><span className='text-xs font-semibold text-ui-muted'>Notify me</span><select className={inputClass} value={draft.notifyOn || 'failure'} onChange={event => onChange({ ...draft, notifyOn: event.target.value as AutomationPayload['notifyOn'] })}><option value='failure'>When something fails</option><option value='always'>After every check</option><option value='never'>Never</option></select></label></div>
    ...
    return <div className='grid gap-4'><div className='flex flex-wrap items-start justify-between gap-3'><div><h2 className='text-xl font-semibold text-ui-text'>{automation.name}</h2><p className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${failed ? 'bg-ui-danger/10 text-ui-danger' : warning ? 'bg-ui-warning/10 text-ui-warning' : automation.status === 'active' ? 'bg-ui-success/10 text-ui-success' : 'bg-ui-raised text-ui-muted'}`}>{state}</p></div><div className='flex gap-2'><button type='button' onClick={onRun} disabled={busy === 'run' || missingUrl} className='inline-flex h-9 items-center gap-2 rounded-lg bg-ui-primary px-3 text-sm font-semibold text-ui-canvas hover:opacity-90 disabled:opacity-50' title={missingUrl ? 'Add a URL before checking' : undefined}><Play className='h-4 w-4' />Check now</button><button type='button' onClick={onEdit} className='h-9 rounded-lg border border-ui-border px-3 text-sm font-semibold text-ui-text'>Edit</button></div></div>
        ...
        <section aria-label='Monitoring issues' className='grid gap-2'>
            ...
            <div className='grid max-h-80 gap-2 overflow-y-auto'>{history.issues.map(issue => <details key={issue.id} id={issue.caseNumber} className='rounded-lg border border-ui-border bg-ui-raised p-3'>
                <summary className='cursor-pointer text-sm text-ui-text'><Link href={`/cases/${encodeURIComponent(issue.caseNumber)}`} className='font-semibold text-ui-primary underline'>{issue.caseNumber}</Link> · {issue.resolvedAt ? 'Resolved' : 'Open'} · {issue.occurrences} occurrences</summary>
...
function validateDraft(draft: AutomationPayload) {
    ...
    if (draft.scheduleKind === 'once' && (!draft.runAt || !Number.isFinite(Date.parse(draft.runAt)) || Date.parse(draft.runAt) <= Date.now())) return { field: 'schedule' as const, message: 'Choose a future run time.' }
    if (draft.scheduleKind === 'interval' && (!draft.intervalMinutes || draft.intervalMinutes < 1 || draft.intervalMinutes > 43200)) return { field: 'schedule' as const, message: 'Choose an interval between 1 and 43200 minutes.' }
