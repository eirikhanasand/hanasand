...
import { GitCommitHorizontal, GitPullRequest, RefreshCw, Plus } from 'lucide-react'
import config from '@/config'
import { CaseCommitPicker, type CaseRepository as Repository } from './case-commit-picker'
...
export function CaseDevelopment({ caseId, organizationId }: { caseId: string, organizationId?: string }) {
    ...
    useEffect(() => { setSetup(null); setPickerOpen(false) }, [caseId, query])
    useEffect(() => {
    ...
    async function disconnect(repository: Repository) {
        ...
        try { await request(`${base}/repositories/${repository.id}?${new URLSearchParams(repository.organization_id ? { organizationId: repository.organization_id } : {})}`, { method: 'DELETE' }); setRevision(value => value + 1); setSetup(null) }
    ...
    return <section aria-label='Development' className='grid min-w-0 gap-3 rounded-lg border border-ui-border bg-ui-canvas p-4 text-ui-text'>
        <div className='flex items-center justify-between gap-3'><h2 className='text-lg font-semibold'>Development</h2><div className='flex gap-2'>
            <button type='button' className={control} aria-label='Refresh development links' title='Refresh development links' disabled={loading} onClick={() => setRevision(value => value + 1)}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button>
            <button type='button' className={control} aria-label='Link a commit' title='Link a commit' aria-expanded={pickerOpen} aria-controls='case-commit-picker' onClick={() => setPickerOpen(value => !value)}><Plus className='h-4 w-4' /></button>
        </div></div>
        {error && <p role='alert' className='text-sm text-ui-danger'>{error}</p>}
        <CaseCommitPicker caseId={caseId} query={query} repositories={repositories} open={pickerOpen} revision={revision} onLinked={() => { setPickerOpen(false); setRevision(value => value + 1) }} />
        {entries.length > 0 && <ul className='grid gap-2'>{entries.map(entry => <li key={`${entry.repository_id}:${entry.kind}:${entry.external_id}`} className='flex min-w-0 gap-3 rounded-lg border border-ui-border p-3'>
        ...
        {(pickerOpen || entries.length > 0) && <details className='border-t border-ui-border pt-3'><summary className='cursor-pointer text-sm font-medium'>Repository connections ({repositories.length})</summary>
            <div className='mt-3 grid gap-3'>
                <p className='text-sm text-ui-muted'>{organizationId ? 'Organization connections are shared with active members. Personal connections stay private to your account.' : 'These connections and repository details are private to your account.'}</p>
