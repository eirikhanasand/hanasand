...
import { MonitoringCaseDetail } from '../monitoring-case-detail'
...
export default async function DwmCaseDetailPage({
    ...
    return (
        <DashboardPage className='gap-2 sm:gap-3'>
            {/^HA-[1-9]\d*$/.test(id) ? <MonitoringCaseDetail key={`${tenantId}:${id}`} caseId={id} organizationId={organizationId} /> : <DwmCaseDetailClient
                key={`${tenantId}:${id}`}
                caseId={id}
                tenantId={tenantId}
