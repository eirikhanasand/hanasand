                ...
                <DashboardPanel className='min-h-0 overflow-hidden border-ui-border bg-ui-panel p-0'>
                    ...
                    <div className='max-h-[calc(100vh-19rem)] min-h-72 overflow-auto p-2'>
                        <div className='grid gap-1.5'>
                            ...
                            {!failedEvents.length ? <p className='rounded-md border border-dashed border-ui-border p-3 text-xs text-ui-muted'>{audit.available ? 'No failed events on this page.' : 'Audit events could not be loaded.'}</p> : null}
                        </div>
                    </div>
                </DashboardPanel>
            </div>
...
async function getAuditPage(params: AuditSearchParams): Promise<AuditPage> {
    ...
    for (const key of ['service', 'actor', 'action', 'target', 'outcome', 'from', 'to', 'page']) {
...
function withPage(params: AuditSearchParams, page: number) {
    ...
    return `/management/audit?${query.toString()}`
}
