...
export default function OrganizationList({ organizations }: { organizations: Organization[] }) {
    ...
    return <div className='p-4'>
        ...
        <p className='mb-3 text-xs text-ui-muted'>Last active shows the latest recorded organization change or API-key use.</p>
        <div className='overflow-x-auto'>
            <table className='w-full text-left text-sm'>
                <thead className='border-b border-ui-border text-ui-muted'><tr>{['Organization', 'Slug', 'Status', 'Members', 'Created', 'Last active (UTC)'].map(label => <th key={label} scope='col' className='px-3 py-3 font-medium'>{label}</th>)}</tr></thead>
