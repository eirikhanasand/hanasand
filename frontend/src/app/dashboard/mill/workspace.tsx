...
export default function MillWorkspace({ view = 'overview' }: { view?: 'overview' | 'rules' }) {
        ...
        <DashboardPage className='!gap-6 !p-4 lg:!p-6'>
            <div className='flex flex-wrap items-center justify-between gap-4'>
                ...
                <div className='flex max-w-full flex-wrap items-center gap-3'>
                    ...
                    <Link href={`${view === 'rules' ? '/dashboard/mill' : '/dashboard/mill/rules'}${organizationId ? `?organizationId=${encodeURIComponent(organizationId)}` : ''}`} className='rounded-lg border border-ui-border px-4 py-2 text-sm font-semibold text-ui-primary hover:bg-ui-raised'>{view === 'rules' ? 'Back to overview' : 'Detection rules'}</Link>
                </div>
            </div>
            ...
            {view === 'overview' && <section className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>
                <Metric label='Open findings' value={String(openCount)} icon={<ShieldAlert className='h-4 w-4' />} />
                <Metric label='Recent events (up to 80)' value={String(events.length)} icon={<Radio className='h-4 w-4' />} />
