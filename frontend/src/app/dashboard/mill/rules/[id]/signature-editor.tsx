...
export default function SignatureEditor({ rule, disabled, onChange }: { rule: MillRule, disabled: boolean, onChange: (definition: Definition) => void }) {
    ...
    const configurableAnalysis = ['model.verified_discovery_probes', 'postgresql.readiness_audit', 'http.duplicate_ingestion_records', 'proxy.redundant_connections', 'cdn.successful_cache_refresh', 'cdn.telemetry_deliveries', 'collector.routine_executions', 'system.completed_telemetry_cycles', 'ssh.completed_session_windows', 'ssh.transport_debug', 'postgresql.readiness_sessions', 'http.routine_access', 'mongodb.cashflow_connections'].includes(rule.id.replace(/\.v\d+$/, ''))
    ...
    function selectors(key: 'conditions' | 'failureConditions', title: string, required: string) {
        ...
        return <section className='grid min-w-0 content-start gap-3 rounded-lg border border-ui-border p-4'>
            ...
            {items.map((condition, index) => <div key={index} className='grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_7rem_minmax(0,1fr)_auto]'>
                ...
                <label className='flex items-center gap-2 text-xs sm:col-span-4'><input type='checkbox' aria-label={`${title} ${index + 1} match case`} checked={condition.caseSensitive === true} onChange={event => onChange({ ...definition, [key]: items.map((item, i) => i === index ? { ...item, caseSensitive: event.target.checked } : item) })} />Match case</label>
            </div>)}
