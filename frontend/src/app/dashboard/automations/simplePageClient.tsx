...
export default function AutomationsClient({ setup }: { setup?: 'dwm' }) {
    ...
    useEffect(() => {
        ...
        const refresh = window.setInterval(() => {
            void fetchAutomation(selected.id).then(result => {
        ...
        <div className='grid gap-4'>
            ...
            <div className='grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(22rem,0.72fr)]'>
                ...
                <section className='rounded-xl border border-ui-border bg-ui-panel p-4 shadow-sm'>
                    {editing ? <AutomationForm draft={draft} selected={selected} busy={busy} message={message} onChange={setDraft} onSave={() => void save()} onCancel={() => { setEditing(false); setMessage('') }} /> : selected ? <AutomationDetails automation={selected} runs={runs} historyFrom={historyFrom} historyTo={historyTo} onHistoryFromChange={setHistoryFrom} onHistoryToChange={setHistoryTo} busy={busy} message={message} onRun={() => void runNow()} onEdit={() => setEditing(true)} onDelete={() => void remove()} onRefresh={() => void select(selected, false)} /> : <WelcomeState onCreate={beginCreate} compact />}
                </section>
            </div>
        </div>
    )
...
function AutomationRow({ automation, selected, onClick }: { automation: AgentAutomation, selected: boolean, onClick: () => void }) {
    ...
    return <button type='button' onClick={onClick} className={`grid w-full grid-cols-2 items-center gap-3 border-t border-ui-border px-4 py-3 text-left transition hover:bg-ui-raised md:grid-cols-[minmax(9rem,1.3fr)_minmax(7rem,0.8fr)_minmax(6rem,0.7fr)_minmax(8rem,1fr)_minmax(5rem,0.6fr)_minmax(5rem,0.6fr)] ${selected ? 'bg-ui-primary/5' : ''}`}>
        ...
        <span className='flex items-center gap-1 text-xs text-ui-muted'>{automation.certificateStatus === 'not_applicable' || automation.monitoringType === 'tcp' || automation.monitoringType === 'ssh' ? null : automation.certificateStatus === 'valid' ? <ShieldCheck className='h-4 w-4 text-ui-success' /> : automation.certificateStatus === 'invalid' ? <ShieldX className='h-4 w-4 text-ui-danger' /> : automation.certificateStatus === 'expiring' ? <ShieldX className='h-4 w-4 text-ui-warning' /> : <span>—</span>}{certLabel(automation)}</span>
...
function historyBars(automation: AgentAutomation) {
    const color = automation.lastStatus === 'failed' ? 'bg-ui-danger' : automation.lastStatus === 'warning' ? 'bg-ui-warning' : automation.lastStatus === 'completed' ? 'bg-ui-success' : 'bg-ui-muted/40'
...
function uptimeLabel(automation: AgentAutomation) {
    ...
    return automation.lastStatus === 'failed' ? '0%' : automation.lastStatus === 'warning' ? 'Degraded' : '100%'
}

function certLabel(automation: AgentAutomation) {
    if (automation.actionType !== 'agent_prompt') return ''
    if (automation.certificateStatus === 'not_applicable' || automation.monitoringType === 'tcp' || automation.monitoringType === 'ssh') return ''
...
function AutomationDetails({ automation, runs, historyFrom, historyTo, onHistoryFromChange, onHistoryToChange, busy, message, onRun, onEdit, onDelete, onRefresh }: { automation: AgentAutomation, runs: AgentAutomationRun[], historyFrom: string, historyTo: string, onHistoryFromChange: (value: string) => void, onHistoryToChange: (value: string) => void, busy: string, message: string, onRun: () => void, onEdit: () => void, onDelete: () => void, onRefresh: () => void }) {
    const missingUrl = automation.actionType === 'agent_prompt' && !automation.targetUrl
    const failed = Boolean(automation.consecutiveFailures || automation.lastStatus === 'failed' || missingUrl)
    const warning = automation.lastStatus === 'warning'
    const state = missingUrl ? 'Missing URL' : failed ? 'Needs attention' : warning ? 'Warning' : automation.expectedDown || automation.upsideDown ? 'Maintenance' : automation.status === 'active' ? 'Healthy' : 'Paused'
    const visibleRuns = runs.filter(run => (!historyFrom || new Date(run.startedAt) >= new Date(historyFrom)) && (!historyTo || new Date(run.startedAt) <= new Date(historyTo)))
    return <div className='grid gap-4'><div className='flex flex-wrap items-start justify-between gap-3'><div><p className='text-xs font-semibold uppercase tracking-wide text-ui-primary'>Automation</p><h2 className='mt-1 text-xl font-semibold text-ui-text'>{automation.name}</h2><p className={`mt-2 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${failed ? 'bg-ui-danger/10 text-ui-danger' : warning ? 'bg-ui-warning/10 text-ui-warning' : automation.status === 'active' ? 'bg-ui-success/10 text-ui-success' : 'bg-ui-raised text-ui-muted'}`}>{state}</p></div><div className='flex gap-2'><button type='button' onClick={onRun} disabled={busy === 'run' || missingUrl} className='inline-flex h-9 items-center gap-2 rounded-lg bg-ui-primary px-3 text-sm font-semibold text-ui-canvas hover:opacity-90 disabled:opacity-50' title={missingUrl ? 'Add a URL before checking' : undefined}><Play className='h-4 w-4' />Check now</button><button type='button' onClick={onEdit} className='h-9 rounded-lg border border-ui-border px-3 text-sm font-semibold text-ui-text'>Edit</button></div></div>
        ...
        <div><div className='mb-2 flex flex-wrap items-center justify-between gap-2'><h3 className='text-sm font-semibold text-ui-text'>Recent checks <span className='font-normal text-ui-muted'>({visibleRuns.length} of {runs.length})</span></h3><button type='button' onClick={onRefresh} className='text-ui-muted hover:text-ui-text' aria-label='Refresh checks'><RefreshCw className='h-4 w-4' /></button></div><div className='mb-2 grid gap-2 sm:grid-cols-2'><label className='grid gap-1 text-xs text-ui-muted'><span>From</span><input type='datetime-local' value={historyFrom} onChange={event => onHistoryFromChange(event.target.value)} className={inputClass} /></label><label className='grid gap-1 text-xs text-ui-muted'><span>To</span><input type='datetime-local' value={historyTo} onChange={event => onHistoryToChange(event.target.value)} className={inputClass} /></label></div><div className='grid max-h-[32rem] gap-2 overflow-y-auto pr-1'>{visibleRuns.map(run => <div key={run.id} className='rounded-lg border border-ui-border bg-ui-raised p-3'><div className='flex justify-between gap-2 text-xs text-ui-muted'><span>{formatDate(run.startedAt)}</span><span className={run.status === 'failed' ? 'font-semibold text-ui-danger' : run.warning ? 'font-semibold text-ui-warning' : 'font-semibold text-ui-success'}>{run.warning ? 'warning' : run.status}</span></div>{run.status === 'failed' ? <ErrorNotice compact className='mt-2' message={run.error || 'Check failed.'} /> : <p className='mt-2 whitespace-pre-wrap text-sm leading-6 text-ui-muted'>{run.result || 'Check completed.'}</p>}</div>)}{!visibleRuns.length ? <p className='rounded-lg border border-dashed border-ui-border p-4 text-sm text-ui-muted'>{runs.length ? 'No checks match this time range.' : 'No checks yet. Run it now or wait for the next check.'}</p> : null}</div></div>
