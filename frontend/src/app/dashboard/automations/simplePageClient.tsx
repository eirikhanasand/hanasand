...
function StatusSummary({ automations }: { automations: AgentAutomation[] }) {
    const counts = automations.reduce((result, automation) => {
        const key = automation.status === 'paused' ? 'maintenance' : !automation.lastStatus || automation.lastStatus === 'running' ? 'pending' : automation.lastStatus === 'failed' ? 'down' : automation.lastStatus === 'warning' ? 'warning' : 'up'
        result[key] += 1
        return result
...
function AutomationRow({ automation, selected, onClick }: { automation: AgentAutomation, selected: boolean, onClick: () => void }) {
    ...
    const state = missingUrl ? 'Missing URL' : failed ? 'Needs attention' : warning ? 'Warning' : automation.status === 'active' ? 'Healthy' : 'Paused'
