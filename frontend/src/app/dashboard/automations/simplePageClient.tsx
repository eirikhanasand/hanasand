...
import { AlertTriangle, Check, Clock3, Filter, Play, Plus, RefreshCw, ShieldCheck, ShieldX, Trash2 } from 'lucide-react'
import {
    createAutomation,
    deleteAutomation,
    fetchAutomation,
...
function AutomationDetails({ automation, runs, historyFrom, historyTo, onHistoryFromChange, onHistoryToChange, showHistoryFilters, onShowHistoryFiltersChange, busy, message, onRun, onEdit, onDelete, onRefresh }: { automation: AgentAutomation, runs: AgentAutomationRun[], historyFrom: string, historyTo: string, onHistoryFromChange: (value: string) => void, onHistoryToChange: (value: string) => void, showHistoryFilters: boolean, onShowHistoryFiltersChange: (value: boolean) => void, busy: string, message: string, onRun: () => void, onEdit: () => void, onDelete: () => void, onRefresh: () => void }) {
    const missingUrl = automation.actionType === 'agent_prompt' && !automation.targetUrl
    const failed = Boolean(automation.consecutiveFailures || automation.lastStatus === 'failed' || missingUrl)
    const warning = automation.lastStatus === 'warning'
    const state = missingUrl ? 'Missing URL' : failed ? 'Needs attention' : warning ? 'Warning' : automation.expectedDown || automation.upsideDown ? 'Maintenance' : automation.status === 'active' ? 'Healthy' : 'Paused'
