...
export default function LogCatchupProgress({ progress, catchingUp, now, stalled }: { progress?: CatchupProgress | null, catchingUp: boolean, now: string, stalled?: boolean }) {
    ...
    const eta = stale ? 'Waiting for a progress update' : progress.remaining === 0 ? 'Finishing catch-up…' : progress.estimated_seconds === null ? (progress.processed > 0 ? 'Waiting for processing to resume' : 'Estimating time remaining…') : duration(progress.estimated_seconds)
    return <section aria-label='Historical log catch-up' className='rounded-xl border border-ui-primary/25 bg-ui-primary/5 p-4 sm:p-5'>
        <div className='flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2'>
            <div><h2 className='text-sm font-semibold text-ui-text'>Checking historical logs</h2><p className='mt-1 text-xl font-semibold tabular-nums text-ui-primary'>{progress ? `${progress.remaining.toLocaleString()} logs remaining` : 'Counting remaining logs…'}</p></div>
            <p className='text-sm text-ui-muted'>{eta}</p>
        </div>
        <div role='progressbar' aria-label='Historical logs checked' aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress ? Math.round(percent) : undefined} aria-valuetext={progress ? `${progress.remaining.toLocaleString()} logs remaining${stale ? ', awaiting update' : ''}` : 'Counting logs'} className='mt-4 h-2.5 overflow-hidden rounded-full bg-ui-primary/10'>
        ...
        <div className='mt-2 flex flex-wrap justify-between gap-2 text-xs text-ui-muted'><span>Results and counters will update as logs are checked.</span>{progress && <span className='tabular-nums'>{percent.toFixed(1)}%{stale ? ' · Last measured' : ''}</span>}</div>
    </section>
}
