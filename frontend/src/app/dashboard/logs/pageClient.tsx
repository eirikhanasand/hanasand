...
export default function LogsPageClient({ initialServices, initialErrors, initialServiceFilter = 'all' }: { initialServices: LogService[], initialErrors: ErrorEventsResponse, initialServiceFilter?: string }) {
    ...
    return <div className='grid min-w-0 gap-4'>
        <header className='flex flex-wrap items-center justify-between gap-3'>
            <div><h1 className='text-2xl font-semibold'>{view === 'dashboard' ? 'Logs' : view === 'realtime' ? 'Realtime' : view === 'errors' ? 'Errors' : 'Search logs'}</h1>{(view === 'realtime' || view === 'errors') && <p className='mt-1 text-sm text-ui-muted'>{view === 'realtime' ? 'High and critical events checked by Mill. Expand an event to investigate.' : view === 'errors' ? 'Application errors, response codes, and request details.' : ''}</p>}</div>
        ...
        {view === 'errors' ? <><div className='flex items-center justify-between gap-3 text-xs text-ui-muted'><span role='status'>{busy ? 'Refreshing…' : copied ? 'Event copied' : 'Recent application errors'}</span><button type='button' className={fieldClass} disabled={busy} onClick={() => setRefresh(value => value + 1)}>Refresh errors</button></div><ErrorsPanel events={errors} expanded={expanded} onToggle={toggle} onCopy={event => void copy(event)} /></> : <>
            ...
            {view === 'dashboard' ? <>
                ...
                <details open className={`${dashboardPanelClass} group overflow-hidden`}><summary className='flex cursor-pointer list-none items-center justify-between border-b border-ui-border bg-ui-raised px-4 py-3 text-sm font-semibold [&::-webkit-details-marker]:hidden'>Operational counters<ChevronDown size={18} aria-hidden className='-rotate-90 text-ui-muted transition-transform group-open:rotate-0' /></summary><div className='p-4'><p className='text-xs text-ui-muted'>Most active services in the selected time range</p><dl className='mt-3 grid gap-2'>{data?.services.map(item => <div key={item.service} className='flex justify-between gap-3 text-sm'><dt>{item.service}</dt><dd>{item.count.toLocaleString()}</dd></div>)}</dl></div></details>
