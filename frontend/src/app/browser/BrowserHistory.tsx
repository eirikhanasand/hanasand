...
export default function BrowserHistory({ clientId }: { clientId: string }) {
    ...
    return <>
        <button type='button' onClick={() => { dialog.current?.showModal(); void load() }} disabled={!clientId} className='fixed bottom-24 right-4 z-40 inline-flex items-center gap-2 rounded-md border border-ui-border bg-ui-panel px-4 py-2 text-sm font-semibold text-ui-text shadow-lg hover:border-ui-primary' aria-haspopup='dialog'>
