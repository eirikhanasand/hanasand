...
const publicExposureRouteSource = readFileSync(new URL('../src/app/api/public/exposure-queue/route.ts', import.meta.url), 'utf8')
...
assert(homeClientSource.includes('/api/public/exposure-queue'))
assert(homeClientSource.includes('root: viewportRef.current'))
assert(homeClientSource.includes('REFRESH_MS = 300_000'))
assert(publicExposureRouteSource.includes('max-age=5, stale-while-revalidate=30'))
assert(!homeClientSource.includes('Latest activity is refreshing'))
