...
assert(actionability.orgRelevance.handoffRows.every(row => row.sourceFamily && Array.isArray(row.provenanceRefs) && Array.isArray(row.blockers)), 'Every org relevance row should carry source family, provenance refs, and typed blockers.')
