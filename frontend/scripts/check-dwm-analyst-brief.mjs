...
for (const token of [
    ...
    'Exposure operations',
    'No cases yet. Matching monitoring events create cases automatically.',
...
assert.ok(caseDetailSource.includes('const DWM_CASE_REPORT_MAX_EVIDENCE = 25'), 'DWM case detail should cap report evidence.')
assert.ok(caseDetailSource.includes('<CollapsiblePanel title=\'Case and evidence report\''), 'DWM case detail should keep secondary report controls collapsed by default.')
...
for (const token of [
    ...
    'No persisted watchlist terms. Add terms owned by this tenant before collecting or checking events.',
    'validEvidenceUrl',
    ...
    'Enter an HTTPS webhook URL or open an organization with a saved delivery destination before sending queued events.',
