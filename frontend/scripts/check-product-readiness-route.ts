...
try {
    ...
    assert.ok(payload?.deployGate.blockingProofRows.every(row => row.proofAgeSeconds >= 0 && typeof row.proofStale === 'boolean'), 'Every blocker proof row needs computed freshness metadata.')
    assert.ok(payload?.deployGate.blockingProofRows.every(row => row.proofDrilldowns.length >= 4), 'Every blocker proof row needs proof drilldowns.')
    ...
    assert.ok(payload?.rows.every(row => row.proofAgeSeconds >= 0 && typeof row.proofStale === 'boolean'), 'Every readiness row needs computed freshness metadata.')
    assert.ok(payload?.rows.every(row => row.proofDrilldowns.length >= 4), 'Every readiness row needs proof drilldowns.')
