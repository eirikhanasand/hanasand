...
try {
    ...
    await expect(catalog.locator('button[aria-pressed]')).toHaveCount(50)
    ...
    await catalog.getByLabel('Search icons', { exact: true }).fill('Database')
    await catalog.getByRole('button', { name: 'Database icon', exact: true }).click()
    ...
    await page.getByRole('button', { name: 'Edit Support', exact: true }).click()
    await expect(priority).toHaveValue('10')
    await expect(page.getByRole('button', { name: 'Choose role icon' }).locator('[data-role-icon]')).toHaveAttribute('data-role-icon', 'database')
    await page.getByRole('button', { name: 'Choose role icon' }).click()
    ...
    console.log('Role editor passed: fixed admin priority, 50 presets, distinct defaults, search/categories, persistence, audit failure recovery, keyboard, read-only access and 360–1440px layouts.')
