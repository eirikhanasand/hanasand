...
writeFileSync(entry, `import React from ${JSON.stringify(resolve('node_modules/react'))}; import { createRoot } from ${JSON.stringify(resolve('node_modules/react-dom/client'))}; import SystemDashboard from ${JSON.stringify(resolve('src/app/dashboard/system/clientPage.tsx'))}; import Panel, { RecoveryBanner } from ${JSON.stringify(resolve('src/components/system/recovery.tsx'))}; createRoot(document.getElementById('app')).render(React.createElement(React.Fragment, null, React.createElement(RecoveryBanner), React.createElement(Panel), React.createElement(SystemDashboard, { id: 'fixture', token: 'fixture', systemTelemetry: null, dockerTelemetry: [], vms: [{ name: 'vm-' + 'c'.repeat(128), owner: 'owner-' + 'd'.repeat(128), status: 'running' }], vmMetrics: [] })));`)
...
const server = Bun.serve({ port: 0, fetch(request) {
    ...
    if (path === '/api/recovery') return Response.json(state)
...
try {
    ...
    for (const width of [320, 375, 390, 540, 640, 768, 820, 1024, 1279, 1280, 1440, 1920]) {
        ...
        assert.equal(await page.locator('[data-recovery-cards]').isVisible(), width < 1280)
        assert.equal(await page.locator('[data-recovery-table]').isVisible(), width >= 1280)
        assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), `Page overflow at ${width}px`)
        if (width < 1280) assert(await page.locator('[data-recovery-cards] article').evaluateAll(elements => elements.every(el => el.scrollWidth <= el.clientWidth)), `Card overflow at ${width}px`)
    }
