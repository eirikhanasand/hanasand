...
try {
    ...
    for (const failure of ['unavailable', 'empty-end', 'launch-failed']) {
    ...
    assert.equal(await page.getByText('Run failed', { exact: true }).count(), 0, 'A transient transport loss must reconnect')
