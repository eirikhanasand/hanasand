...
assert(source.includes('const workspaceMeta = sanitizeOrganizationDisplayCopy(organization.status || organization.slug || organization.id) || \'Active workspace\''), 'Organization summary should sanitize workspace metadata before rendering.')
assert(!source.includes('const workspaceMeta = sanitizeOrganizationDisplayCopy(organization.tenantId || organization.slug || organization.id) || \'Default workspace\''), 'Organization summary should not use raw tenant ids as workspace metadata.')
