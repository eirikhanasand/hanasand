...
try {
    ...
    assert.equal(await page.getByRole('button', { name: 'Continue with Apple', exact: true }).isDisabled(), true)
    assert.equal(await page.getByText('Awaiting provider setup', { exact: true }).count(), 2)
