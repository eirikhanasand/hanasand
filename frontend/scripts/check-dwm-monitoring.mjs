...
try {
    ...
    assert(caseLink.includes('/cases/case-one') && caseLink.includes('org=org-one'))
    ...
    const actorSearch = page.getByLabel('Search actors')
    await page.keyboard.press('Meta+j')
    assert(await actorSearch.evaluate(element => element === document.activeElement), 'Cmd+J focuses actor search')
    await page.getByRole('heading', { name: 'Actors', exact: true }).click()
    await page.keyboard.press('Control+j')
    assert(await actorSearch.evaluate(element => element === document.activeElement), 'Ctrl+J focuses actor search')
    const searchBox = await actorSearch.boundingBox()
    const countBox = await page.getByText('25 actor profiles', { exact: true }).boundingBox()
    assert(Math.abs(searchBox.y - countBox.y) < 20, 'Search belongs beside the profile count')
    for (const width of [390, 768, 1440]) {
        await page.setViewportSize({ width, height: 1000 })
        assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), `Actors overflow at ${width}`)
    }
