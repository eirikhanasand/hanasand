...
assert(pageClientSource.includes('const TI_SELECTED_CONTINUITY_GAP_ROWS = 3'), 'Public TI first-viewport continuity gaps should stay compact.')
...
assert(pageClientSource.includes('selected.evidence.slice(0, TI_SELECTED_DETAIL_LIST_ROWS).map'), 'Public TI expanded selected evidence should use the detail preview cap.')
assert(pageClientSource.includes('selected.nextActions.slice(0, TI_SELECTED_DETAIL_LIST_ROWS).map'), 'Public TI expanded selected actions should use the detail preview cap.')
assert(!pageClientSource.includes('selected.evidence.map(line => <li key={line}>{line}</li>)'), 'Public TI selected evidence should not render every evidence row by default.')
assert(!pageClientSource.includes('selected.nextActions.map(line => <li key={line}>{displayRequirementText(line)}</li>)'), 'Public TI selected next actions should not render every action row by default.')
...
assert(pageClientSource.includes('filteredWorkItems.slice(0, TI_EVIDENCE_QUEUE_PREVIEW_ROWS)'), 'Public TI default queue should use the named preview cap before expansion.')
...
assert(pageClientSource.includes('continuityRefs.slice(0, TI_SELECTED_CONTINUITY_REF_ROWS)'), 'Public TI continuity refs should use the named preview cap.')
...
assert(pageClientSource.includes('continuityGaps.slice(0, TI_SELECTED_CONTINUITY_GAP_ROWS)'), 'Public TI continuity gaps should use the named preview cap.')
...
assert(pageClientSource.includes('capture needed'), 'Public TI selected action rail should show operational capture requirements instead of attach/zero labels.')
assert(pageClientSource.includes('watchlist needed'), 'Public TI selected action rail should show operational watchlist requirements instead of empty alert labels.')
assert(pageClientSource.includes('case source needed'), 'Public TI selected action rail should show operational case requirements instead of empty case labels.')
assert(pageClientSource.includes('watch term needed'), 'Public TI selected action rail should show explicit watch-term requirements.')
assert(pageClientSource.includes('destination needed'), 'Public TI selected action rail should show explicit delivery destination requirements.')
