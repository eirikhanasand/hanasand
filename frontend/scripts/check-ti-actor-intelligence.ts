...
assert(profile.actorClass === 'Named threat actor or activity cluster', 'Actor class should stay generic when no sourced class is supplied.')
assert(profile.malwareTools.length === 0, 'Actor tooling must not be inferred from the actor name.')
assert(profile.campaigns.some(item => /Microsoft disclosed/i.test(item)), 'Campaign context should come from captured activity.')
...
assert(profile.techniqueCoverage.some(item => item.attackId === 'T1078.004' && item.sourceIds.length === 0 && item.missing.includes('sourceId') && item.missing.includes('captureId')), 'Technique coverage should expose missing evidence links instead of borrowing unrelated profile sources.')
assert(profile.techniqueCoverage.every(item => item.name && item.tactic && item.detail && item.freshness === 'review'), 'Unlinked technique coverage should remain review-only.')
assert(profile.targetSectors.some(item => /Government/i.test(item)), 'APT29 should include government targeting context.')
assert(profile.sourceProvenance.length >= 1, 'APT29 should expose source provenance.')
...
assert(!profile.sourceCoverage.sourceFamilies.some(item => item.family === 'government_advisory'), 'Actor coverage must not invent a government source that is absent from the response.')
...
assert(!fullPublicActionability.geographyHandoffs.some(item => item.code === 'DE'), 'Actor geography must not add countries absent from captured evidence.')
assert(!actionability.geographyHandoffs.some(item => item.role === 'operator'), 'Operator geography requires an explicit attributed profile and linked provenance.')
...
assert(!geo.points.some(item => item.role === 'operator'), 'Actor names and summary prose must not create operator geography.')
...
assert(actionability.sourceClusters.every(item => !item.watchlistTerm), 'Publisher domains must not become customer watchlist terms.')
...
assert(!artifacts.some(item => item.kind === 'tool'), 'Artifact workbench must not invent malware/tool artifacts.')
assert(artifacts.some(item => item.kind === 'campaign' && /Microsoft disclosed/i.test(item.label)), 'Artifact workbench should expose captured campaign artifacts.')
...
assert(lockbitProfile.actorClass === 'Named threat actor or activity cluster', 'Actor class must stay generic without a sourced classification.')
assert(lockbitProfile.malwareTools.length === 0, 'Aliases must not be converted into malware/tool facts.')
assert(lockbitProfile.infrastructure.length === 0, 'Source names must not be converted into actor infrastructure.')
assert(lockbitProfile.campaigns.length > 0, 'Captured activity should remain available as campaign context.')
assert(lockbitProfile.confidenceReasoning.some(item => /source records/i.test(item)), 'Generic fallback should explain source-backed confidence.')
...
assert(pageClientSource.includes('const TI_EVIDENCE_QUEUE_PREVIEW_ROWS = 2'), 'Public TI evidence queue should keep the first viewport focused before expansion.')
assert(pageClientSource.includes('const TI_SELECTED_CONTEXT_ROWS = 2'), 'Public TI selected source context should stay capped before expansion.')
assert(pageClientSource.includes('const TI_SELECTED_CONTINUITY_REF_ROWS = 2'), 'Public TI first-viewport continuity refs should stay compact.')
...
assert(pageClientSource.includes('artifact.watchlistTerms.slice(0, TI_SELECTED_DETAIL_LIST_ROWS).map'), 'Public TI selected artifact watchlist terms should use the detail preview cap.')
assert(pageClientSource.includes('artifact.enrichmentTasks.slice(0, TI_SELECTED_DETAIL_LIST_ROWS).map'), 'Public TI selected artifact source questions should use the detail preview cap.')
assert(!pageClientSource.includes('artifact.watchlistTerms.map(term => <li'), 'Public TI selected artifact watchlist terms should not render every row by default.')
assert(!pageClientSource.includes('artifact.enrichmentTasks.map(task => <li'), 'Public TI selected artifact source questions should not render every row by default.')
...
assert(pageClientSource.includes('const TI_SOURCE_REFERENCE_ROWS = 2'), 'Public TI source reference panel should stay compact.')
...
assert(pageClientSource.includes('const TI_DOSSIER_SOURCE_FAMILY_ROWS = 3'), 'Public TI actor dossier should cap source-family chips.')
...
assert(pageClientSource.includes('coverage.sourceFamilies.slice(0, TI_DOSSIER_SOURCE_FAMILY_ROWS).map'), 'Public TI actor dossier source-family chips should use the named preview cap.')
assert(!pageClientSource.includes('actor.confidenceReasoning.map(item => <li'), 'Public TI actor dossier should not render every confidence reason by default.')
assert(!pageClientSource.includes('coverage.sourceFamilies.map(item =>'), 'Public TI actor dossier should not render every source-family chip by default.')
assert(!pageClientSource.includes('data-ti-right-rail'), 'Public TI default actor page should not render a secondary right rail.')
...
assert(pageClientSource.includes('Action packages'), 'Public TI page should expose validated action packages.')
assert(!pageClientSource.includes('>Action exports<'), 'Public TI page should not render implementation-shaped action export copy.')
assert(pageClientSource.includes('Action packages for authenticated review'), 'Public TI page should explain copy-only action packages professionally.')
...
assert(pageClientSource.includes('ti.public_actor.selected_delivery_readiness.v1'), 'Selected evidence should export versioned delivery readiness.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Delivery status\''), 'Selected delivery readiness should expose copyable status.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Watchlist plan\''), 'Selected watchlist plan should expose a copyable request.')
...
assert(pageClientSource.includes('ti.public_actor.selected_enrichment_triage.v1'), 'Selected evidence should export versioned enrichment triage.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Review alert\''), 'Selected enrichment triage should expose a copyable alert.')
...
assert(!pageClientSource.includes(').slice(0, 8)) {'), 'Public TI workbench generators should not reintroduce broad eight-row visible loops.')
assert(pageClientSource.includes('data-ti-actor-info=\'true\''), 'Public TI first viewport should expose actor information.')
assert(pageClientSource.includes('data-ti-map=\'true\''), 'Public TI first viewport should expose the actor map.')
...
assert(!pageClientSource.includes('capture needed'), 'Public TI should not show capture labels as manual user actions.')
assert(!pageClientSource.includes('watchlist needed'), 'Public TI should not show watchlist labels as manual user actions.')
assert(!pageClientSource.includes('data-ti-selected-brief=\'true\''), 'Public TI should not render a generic analyst brief for unsupported activity.')
assert(!/label:\s*'Alerts'[\s\S]{0,500}:\s*'none'/.test(pageClientSource), 'Public TI selected action rail should not render empty alert continuity as none.')
assert(!/label:\s*'Cases'[\s\S]{0,500}:\s*'none'/.test(pageClientSource), 'Public TI selected action rail should not render empty case continuity as none.')
assert(!/label:\s*'Watch'[\s\S]{0,500}:\s*'term needed'/.test(pageClientSource), 'Public TI selected action rail should not render vague watch continuity labels.')
assert(!/label:\s*'Destinations'[\s\S]{0,500}:\s*'choose'/.test(pageClientSource), 'Public TI selected action rail should not render vague delivery continuity labels.')
assert(!pageClientSource.includes('Add source name, report date, and provenance before using geography for customer routing.'), 'Customer geography fallback should not expose internal source-routing instructions.')
assert(pageClientSource.includes('MapCoverageFallback regions={regionalAreas} actor={actor} compact={compact}'), 'Geography fallback should remain customer-facing in the first viewport.')
...
assert(pageClientSource.includes('data-ti-selected-case-provenance-fingerprints'), 'Selected case create request should render provenance fingerprint evidence.')
...
assert(pageClientSource.includes('countLinkedLabel(actionability.readiness.backedIds.alertIds.length'), 'Public TI handoff evidence matrix should summarize alert refs without raw IDs.')
assert(pageClientSource.includes('countLinkedLabel(actionability.readiness.backedIds.captureIds.length'), 'Public TI handoff evidence matrix should summarize capture refs without raw IDs.')
assert(pageClientSource.includes('capture linked'), 'Public TI handoff evidence matrix should show linked capture state without raw capture IDs.')
assert(pageClientSource.includes('link needed'), 'Public TI handoff evidence matrix should use link-needed copy instead of ID-needed copy.')
assert(!pageClientSource.includes('ID needed'), 'Public TI handoff evidence matrix should not expose ID-needed copy.')
...
assert(pageClientSource.includes('Linked records'), 'Public TI page should expose backed workflow state in plain language.')
assert(pageClientSource.includes('Org records, follow-up fields, and next action'), 'Public TI page should describe backed status without prompt-shaped language.')
...
assert(pageClientSource.includes('source reference linked'), 'Public TI source/provenance rows should show customer-safe source reference labels.')
assert(!pageClientSource.includes('`source ${sourceId}`'), 'Public TI source/provenance rows should not render raw source IDs.')
assert(!pageClientSource.includes('`capture ${row.captureId}`'), 'Public TI source/provenance rows should not render raw capture IDs.')
assert(pageClientSource.includes('<span className=\'h-2.5 w-2.5 rounded-full bg-ui-primary\' />Origin'), 'Public TI map legend should use customer-facing origin language.')
assert(pageClientSource.includes('<span className=\'h-2.5 w-2.5 rounded-full bg-ui-danger\' />Target'), 'Public TI map legend should use customer-facing target language.')
assert(!pageClientSource.includes('Source review task'), 'Public TI map should not expose internal source-review tasks.')
...
assert(!pageClientSource.includes('data-ti-selected-brief'), 'Selected evidence should not render a generic analyst brief for unsupported activity.')
...
assert(pageClientSource.includes('source: task.sourceFamily ? formatLabel(task.sourceFamily) : \'Analyst review\''), 'Public TI source-gap rows should use analyst workflow source labels.')
assert(pageClientSource.includes('Analysts can stage review, but source changes require authenticated approval.'), 'Public TI source activation copy should avoid internal product-lane naming.')
assert(!pageClientSource.includes('source: task.sourceFamily ? formatLabel(task.sourceFamily) : \'Public TI\''), 'Public TI source-gap rows should not expose internal product-lane source labels.')
assert(!pageClientSource.includes('Public TI can stage review'), 'Public TI source activation copy should not expose internal product-lane naming.')
...
assert(!pageClientSource.includes('capture needed'), 'Selected source drilldown should not show capture labels as manual user actions.')
...
assert(pageClientSource.includes('data-ti-evidence-priority'), 'Selected evidence should expose backed priority evidence.')
