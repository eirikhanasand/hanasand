...
assert(pageClientSource.includes('{stage.request.method} {stage.request.path}'), 'Public TI consumer readiness should show backed request method and path.')
assert(pageClientSource.includes('provenance row'), 'Public TI consumer readiness should show stage provenance counts.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Delivery readiness\''), 'Selected delivery readiness should expose a copyable packet.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Enrichment triage\''), 'Selected enrichment triage should expose a copyable packet.')
...
assert(pageClientSource.includes('ti.public_actor.freshness_review.v1'), 'Freshness gate should export a versioned source freshness review payload.')
assert(pageClientSource.includes('data-ti-freshness-review-export'), 'Freshness gate should expose a stable render hook for the review export.')
assert(pageClientSource.includes('CopyPayloadButton label=\'Freshness review\''), 'Freshness gate should expose copyable source freshness review packets.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Selected artifact\''), 'Selected artifact workbench should expose a copyable artifact payload.')
...
assert(pageClientSource.includes('stagedHandoffFor'), 'Staged handoffs should combine review, source drilldown, case draft, ownership, case create request, and action trail payloads.')
...
assert(pageClientSource.includes('disabled={!reviewHandoff || !caseDraft || !caseOwnership || !caseCreateRequest || !caseActionTrail}'), 'Stage handoff should require the full case create request, ownership, and action trail.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Consumer readiness\''), 'Source-health UI should expose copyable consumer readiness payloads.')
...
assert(pageClientSource.includes('CopyPayloadButton label=\'Enrichment request\''), 'Source-health rows should expose copyable enrichment requests.')
...
assert(pageClientSource.includes('Source drilldown'), 'Selected evidence should label source drilldown in analyst language.')
