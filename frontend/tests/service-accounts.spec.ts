...
test('service account creation and mandatory deletion confirmation', async ({ browser, baseURL }) => {
    ...
    const createDialog = page.getByRole('dialog',{name:'Add service account',exact:true,includeHidden:true})
    await expect(createDialog.getByLabel('Name',{exact:true})).toBeHidden()
    ...
    await createDialog.getByLabel('Name',{exact:true}).fill('Database check')
...
test('names and descriptions persist, editing is beside delete, and long text opens in an accessible popup', async ({ browser, baseURL }) => {
    ...
    await context.route(url=>url.pathname.startsWith('/api/service-accounts'),async route=>{
        ...
        if (req.method()==='POST') {
            ...
            account.name = req.postDataJSON().name
            account.description = req.postDataJSON().description
            await route.fulfill({json:{id:account.id,name:account.name,description:account.description}})
        } else await route.fulfill({json:{accounts,endpoints}})
    })
    const page = await context.newPage()
    ...
    await page.getByRole('button',{name:'Add service account',exact:true}).click()
    await page.getByRole('dialog',{name:'Add service account',exact:true}).getByLabel('Name',{exact:true}).fill('Database monitor')
    ...
    await search.fill('')
    const existingRow = page.getByRole('row').filter({has:page.getByRole('cell',{name:'Existing monitor',exact:true})})
    const actions = existingRow.getByRole('cell').last()
    await expect(actions.getByRole('button',{name:'Edit Existing monitor',exact:true})).toBeVisible()
    await expect(actions.getByRole('button',{name:'Delete Existing monitor',exact:true})).toBeVisible()
    await expect(existingRow.getByRole('cell').nth(1).getByRole('button')).toHaveCount(0)
    await actions.getByRole('button',{name:'Edit Existing monitor',exact:true}).click()
    const dialog = page.getByRole('dialog',{name:/^Edit /})
    const name = dialog.getByLabel('Name',{exact:true})
    await expect(name).toBeFocused()
    await expect(name).toHaveValue('Existing monitor')
    ...
    await page.screenshot({path:'/tmp/service-account-edit-desktop.png',fullPage:true})
    await dialog.getByRole('button',{name:'Save changes'}).click()
    await expect(dialog.getByRole('alert')).toHaveText('Unable to save; try again.')
    await expect(name).toHaveValue('  Alpha monitor  ')
    await expect(dialog.getByLabel('Description',{exact:true})).toHaveValue('Updated checks')
    await dialog.getByRole('button',{name:'Save changes'}).click()
    await expect(dialog).toBeHidden()
    await expect(page.locator('tbody tr > td:first-child')).toHaveText(['Alpha monitor','Database monitor'])
    await search.fill('Alpha')
    await expect(page.locator('tbody tr > td:first-child')).toHaveText(['Alpha monitor'])
    await search.fill('')
    await page.getByRole('button',{name:'Delete Alpha monitor',exact:true}).click()
    await expect(page.getByRole('dialog',{name:'Delete Alpha monitor?'})).toBeVisible()
    await page.getByRole('dialog',{name:'Delete Alpha monitor?'}).getByRole('button',{name:'Cancel',exact:true}).click()
    await page.reload()
    await expect(page.locator('tbody tr > td:first-child')).toHaveText(['Alpha monitor','Database monitor'])
    await expect(page.locator('span').filter({hasText:/^Updated checks$/})).toBeVisible()
    await page.getByRole('button',{name:'Edit Alpha monitor',exact:true}).click()
    ...
    await page.setViewportSize({width:390,height:844})
    await page.getByRole('button',{name:'Edit Alpha monitor',exact:true}).click()
    const editBox = await dialog.boundingBox()
    assert(editBox && editBox.x >= 0 && editBox.x + editBox.width <= 390 && editBox.height <= 844)
    await page.screenshot({path:'/tmp/service-account-edit-mobile.png',fullPage:true})
    await page.keyboard.press('Escape')
    await expect(dialog).toBeHidden()
    await info.click()
    await expect(popup).toHaveText(longDescription)
