...
test('organization workspace exposes destination lifecycle controls safely', () => {
    ...
    assert.match(organizationWorkspaceSource, /Retention days must be between 30 and 2555\./, 'retention policy should have bounded validation.')
    ...
    assert.match(organizationWorkspaceSource, /Testing watchlist destination|Test and save a destination/, 'watchlist destination setup should confirm the saved scope.')
    assert.match(organizationWorkspaceSource, /No durable delivery result was returned for \$\{fallback\}\./, 'delivery actions must fail explicitly when the backend returns no durable row.')
    assert.doesNotMatch(organizationWorkspaceSource, /\$\{destination\.name \|\| destination\.id\} tested\./, 'saved destination tests must not claim success without a durable row.')
    assert.doesNotMatch(organizationWorkspaceSource, /Delivery replay requested\./, 'delivery replay must not claim success without a durable row.')
    ...
    assert.match(organizationWorkspaceSource, /\/dwm\/cases\/\$\{encodeURIComponent\(delivery\.caseId\)\}/, 'delivery history should link case context.')
