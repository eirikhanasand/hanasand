...
test('organization workspace exposes destination lifecycle controls safely', () => {
    ...
    assert.match(organizationWorkspaceSource, /destinationSearchText\(destination, latestDelivery\)/, 'saved destinations should be searchable by destination and latest delivery context.')
    ...
    assert.match(organizationWorkspaceSource, /No destinations match this view\./, 'saved destination filters should expose an empty result state.')
