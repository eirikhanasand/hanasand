...
test('DWM case detail exposes webhook delivery traceability', () => {
    ...
    assert.match(source, /receipt\.idempotencyKey === latestDelivery\?\.idempotencyKey/)
    assert.match(source, /deliveries\.some\(delivery => delivery\.id === receipt\.deliveryId && delivery\.status === 'delivered'\)/)
    assert.doesNotMatch(source, /receipt\.reportExportChecksum === latestDelivery\?\.reportExportChecksum/)
