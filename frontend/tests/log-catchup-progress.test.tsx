...
test('missing, paused and stale measurements do not invent an ETA', () => {
    ...
    expect(render({ ...progress, rate: null, estimated_seconds: null })).toContain('Waiting for processing to resume')
    const stale = render({ ...progress, updated_at: '2026-09-19T11:57:00Z' })
    expect(stale).toContain('Waiting for a progress update')
