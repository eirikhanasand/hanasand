...
assert(apt29.orgRelevance.watchlistIntersections.some(item =>
    ...
    && item.casePaths.includes('/cases/case_apt29_microsoft')
...
assert(alertOnlyCaseContinuity.relatedCases.some(item =>
    ...
    && item.path === '/cases/case_apt29_microsoft'
), 'Actionability should derive related cases from related alert case context.')
assert(alertOnlyCaseContinuity.readiness.backedIds.casePaths.includes('/cases/case_apt29_microsoft'), 'Derived related cases should feed readiness case paths.')
...
assert(sanitizedApt29Result.actionability?.watchlistMatches?.some(match =>
    ...
    && match.casePath === '/cases/case_apt29_microsoft'
    ...
    return {
        ...
        actionability: {
            ...
            relatedCases: [{
                ...
                status: 'open',
                priority: 'high',
                path: '/cases/case_apt29_microsoft',
            }],
            sourceProvenance: [{
                sourceId: 'src_microsoft_midnight_blizzard',
