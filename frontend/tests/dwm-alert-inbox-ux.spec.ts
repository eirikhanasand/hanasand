...
test('DWM alert inbox keeps triage actions primary and collapses evidence noise', async () => {
    ...
    expect(page).toContain('data-dwm-alert-delivery-setup')
    expect(page).toContain('function deliverySetupHref')
    expect(page).toContain('data-dwm-alert-result-action')
    expect(page).toContain('No delivery was attempted. Configure or test a destination before sending.')
