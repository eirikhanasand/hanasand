...
test('traffic dashboard summary lanes use shared theme tokens', async () => {
    ...
    expect(client).toContain('data-traffic-user-agent-disclosure')
    expect(client).toContain('data-traffic-request-log-disclosure')
    expect(client.indexOf('data-traffic-primary-flow')).toBeLessThan(client.indexOf('data-traffic-throughput-disclosure'))
    expect(client.indexOf('data-traffic-primary-flow')).toBeLessThan(client.indexOf('Blocklist and recent activity'))
