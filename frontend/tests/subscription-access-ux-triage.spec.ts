...
test('subscription page makes access upgrade and enterprise review paths explicit', async () => {
    ...
    expect(page).toContain('data-subscription-utility-entitlements')
    expect(page).toContain('Utility tools')
    expect(page).toContain('Separate from monitoring plans')
    expect(page).toContain('label: \'Service checks\', value: \'Utility\'')
    expect(page).toContain('href: \'/dashboard/load-testing\'')
    expect(page).not.toContain('checks: \'5 load checks / month\'')
    expect(page).not.toContain('checks: \'50 load checks / month\'')
    expect(page).not.toContain('checks: \'250 load checks / month\'')
