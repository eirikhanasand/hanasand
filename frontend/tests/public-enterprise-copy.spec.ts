...
test('public buyer navigation keeps hash utility out of the primary monitoring journey', async () => {
    ...
    expect(header).not.toContain('Password exposure utility')
    expect(header).not.toContain('Bloom Hash Exposure Lookup')
    expect(header).not.toContain('Bloom hash lookup')
    expect(header).not.toMatch(/href: '\/pwned'/)
    expect(footer).not.toContain('Password Utility')
    expect(footer).not.toContain('Bloom Hash Lookup')
    expect(footer).not.toMatch(/href: '\/pwned'/)
    expect(sitemap).not.toMatch(/'\/pwned'/)
