...
test('cron jobs dashboard renders unified scheduled operations and controls', async ({ context, page, baseURL }) => {
    ...
    await expect(page.getByText('Public TI collection loop')).toBeVisible()
    await expect(page.getByText('Exposure parser bridge')).toBeVisible()
    await expect(page.getByText('DWM alert generation readiness')).toBeVisible()
    await expect(page.getByText('Vulnerability image scanner')).toBeVisible()
    await expect(page.getByText('Docker socket is unavailable at /var/run/docker.sock', { exact: true })).toBeVisible()
    await expect(page.getByText('blocked', { exact: true })).toBeVisible()
    await expect(page.getByText('DWM exposure queue collection')).toBeVisible()
    await expect(page.getByText('Forgejo standby sync')).toBeVisible()
