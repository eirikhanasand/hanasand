...
test('initial send and resend failures stay visible and both can be retried', async ({ page }) => {
    ...
    await expect(page.getByText('We’ll email you a six-digit code', { exact: false })).toHaveCount(0)
    const form = page.locator('form[action="/api/auth/register"]')
    await form.getByLabel('Username', { exact: true }).fill('signup-example')
    await form.getByLabel('Name', { exact: true }).fill('Signup Example')
    await form.getByLabel('Email', { exact: true }).fill('signup@example.test')
...
test('signup uses its own route and survives refresh with the destination intact', async ({ page }) => {
    await page.goto('/login?path=%2Forganizations&error=Account+pending+deletion.')
    await expect(page.getByText('Sign in to the console.', { exact: true })).toHaveCount(0)
    await page.getByRole('button', { name: 'Sign up', exact: true }).click()
    await expect(page).toHaveURL(/\/signup\?path=%2Forganizations$/)
    await page.reload()
    await expect(page.locator('form[action="/api/auth/register"]')).toBeVisible()
    await expect(page.locator('input[name="redirectPath"]')).toHaveValue('/organizations')
    await expect(page.getByLabel('Email', { exact: true })).toHaveAttribute('placeholder', 'example@mail.com')
    await page.getByRole('button', { name: 'Log in', exact: true }).click()
    await expect(page).toHaveURL(/\/login\?path=%2Forganizations$/)
})
