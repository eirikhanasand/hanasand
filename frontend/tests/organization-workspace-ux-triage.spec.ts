...
test('organization workspace keeps launch workflow primary and admin controls disclosed', async () => {
    ...
    expect(page).toContain('data-org-setup-blocked-count')
    expect(page).toContain('data-org-setup-step-reason')
    expect(page).toContain('{lockedCount} needs access')
    ...
    expect(page).toContain('Matched exposure records will appear here after collection.')
    ...
    expect(page).toContain('const workspaceMeta = sanitizeOrganizationDisplayCopy(organization.tenantId || organization.slug || organization.id) || \'Default workspace\'')
