...
test('organization workspace keeps launch workflow primary and admin controls disclosed', async () => {
    ...
    expect(page).toContain('<ActionAnchor href=\'#destinations\' icon={<Webhook className=\'h-4 w-4\' />} label=\'Test destination\' disabled={!hasWatchlists} disabledReason=\'Add a watchlist term before testing delivery.\' />')
    expect(page).toContain('href: \'#destinations\'')
    expect(page).toContain('disabledReason=\'Add a watchlist term before testing delivery.\'')
    expect(page).toContain('disabledReason=\'Admin access is required to invite team members.\'')
    ...
    expect(page).toContain('const visibleRows = rows.filter(row => row.ready || !row.blocked)')
...
test('organization workspace renders searchable shared watchlists', async ({ context, page, baseURL }, testInfo) => {
    ...
    await expect(page.locator('[data-org-section-nav-item="destinations"]')).toContainText('1 ready')
    ...
    await expect(page.locator('[data-org-health-row="delivery"]')).toContainText('15 delivery events')
