...
test('organization workspace keeps launch workflow primary and admin controls disclosed', async () => {
    ...
    expect(page).toContain('data-org-pilot-metrics')
    ...
    expect(page).toContain('/ti/workbench?organizationId=${organizationId}&watchlistId=${watchlistId}')
    expect(page).toContain('/dwm/cases/${caseId}?organizationId=${organizationId}')
...
test('organization workspace renders searchable shared watchlists', async ({ context, page, baseURL }, testInfo) => {
    ...
    await expect(page.locator('[data-org-pilot-metrics="true"]')).toBeVisible()
    await expect(page.locator('[data-org-pilot-metric="coverage"]')).toContainText('1 active term')
    await expect(page.locator('[data-org-pilot-metric="precision"]')).toContainText('Not measured')
    await expect(page.locator('[data-org-pilot-metric="triage"]')).toContainText('Not measured')
