...
test('command queue reports capped counts and clears its delay warning after processing', async ({ page }) => {
    ...
    await expect(notice).toHaveCount(0)
    await expect(page.getByText('Commands awaiting checks:', { exact: false })).toHaveCount(0)
    ...
    await expect(page.getByText(/Commands awaiting checks:/)).toHaveCount(0)
...
test('dashboard lands on severity counts and keeps active services inside operational counters', async ({ page }) => {
    ...
    await expect(page.getByRole('region', { name: 'Events by severity' }).getByRole('link')).toHaveCount(5)
    await expect(page.getByRole('navigation', { name: 'Log pages' }).getByRole('link')).toHaveText(['Realtime', 'Search', 'Errors', 'Traffic'])
    await expect(page.getByRole('navigation', { name: 'Log pages' }).getByRole('link', { name: 'Dashboard', exact: true })).toHaveCount(0)
