...
test('ti control keeps search and work queue primary while telemetry is disclosed', async () => {
    ...
    expect(source).toContain('discoveredAt: stringValue(task.discoveredAt) || stringValue(task.createdAt) || \'\'')
    expect(source).toContain('if (!value) return \'Time unavailable\'')
    expect(source).not.toContain('snapshot?.generatedAt || new Date().toISOString()')
