...
### 2026-08-09T17:08Z production recovery and remaining blockers

- Candidate commit `40e2e6a16701b3bdaee8abc1ed768ac3e4979bdc` added the independently verified `avleonovcom` and `maldevcc` global public-preview sources. The first deployment imported exactly those two rows. The measured canonical fleet is now 65 registered: 26 active, 30 candidate, 2 rejected, and 7 retired; strict qualification remains 17 because neither new row has two useful scheduled retained-capture cycles.
- The scraper was configured for `http://ai-parser-bridge:18181`, but the declared Compose dependency was only in `Created` state and absent from the runtime network. This prevented the hosted exposure parser and delayed the evidence-bound review path. Starting the existing declared service restored a healthy bridge at 2026-08-09T17:01:51Z; its health response reported `ready`, upstream HTTP 200, and the configured model available. No source policy or qualification flag was bypassed.
- A concurrent shared release interrupted the first post-import global run. Its container exited 137 after the graceful-stop window with the run still `running`, zero persisted source-health rows, and its queued writes uncommitted. Current-main startup reconciled that exact run to failed rather than manufacturing completion.
- Deployed commit `998f10ef974be6d11a099e2a4953c99e7ff30dd2` and scraper image `sha256:fe6110fc545379eeda4cc0726199be669bff1c31d93b86ca2da4645b6c7d4bf6` then completed native global run `canary-run_969bd48e03470a1a`: 42/42 tasks, 0 failures, 9 inserted captures, 73 duplicates, and 0 retries. `src_zhassulan_maldevcc_telegram` persisted HTTP 200, 16 parsed items, 2 captures, zero parser warnings, and its first useful cycle. `src_alexander_leonov_english_telegram` persisted HTTP 200, 20 parsed items, 2 captures, and zero parser warnings, but correctly remained non-useful pending automatic source review. DCiber and REDSEG recorded healthy duplicate-only second checks and remain candidates.
- The four new immutable objects match their recorded SHA-256 values. Re-running the shipped Telegram sanitizer over every object found 0 changes, 0 idempotence failures, and 0 residual email, bot-token, credential, or phone matches; the four persisted safe excerpts also contain 0 email, bot-token, or credential residuals.
- Restart preserved the exact Telegram IDs and global scopes, but shared bootstrap telemetry reported `importedSourceCount=7` even though the database source total stayed 1,766 and none of those rows had a new `created_at`. Treat that counter as unresolved restart telemetry, not proof of seven new sources. The smallest shared fix is to make bootstrap apply reporting distinguish a memory rehydrate/upsert of an existing durable identity from a new database insert.
- The next review/collection pass is currently delayed by continuous shared full-JSON capture-search reads. Production repeatedly has four to five active `SELECT capture.record ... position(term in lower(capture.record::text || source.record::text)) ... LIMIT` scans lasting more than a minute; pending writes rose from 0 to more than 1,500 while no second run record could cross the durable boundary. This is outside the Telegram-owned adapter/pack/policy paths. The shared search owner must make the database read genuinely bounded or cancel it when the 12-second client request aborts, then deploy the matching API caller change; do not work around it by weakening Telegram review or manually promoting rows.

The next accepted evidence is a naturally scheduled cycle after the shared read pressure clears. MalDevCC needs one more useful retained cycle plus evidence-bound approval; Leonov needs approval followed by two useful retained cycles. Duplicate-only health does not advance DCiber or REDSEG.
