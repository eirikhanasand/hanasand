...
export type ApplyPlanPayload = {
  applyPlan?: {
    ...
    connectorCertifications: Array<{
      scenario: string;
      metadataOnly: boolean;
      safeForApi: boolean;
      dryRunOnly: boolean;
      noLeakSerialization: { passed: boolean };
      guarantees: { noContact: boolean; noDownload: boolean };
    }>;
    killSwitchDrills: DrillProof;
    emergencyStopCertification: EmergencyStopProof;
...
type IsolationHarnessProof = {
  ...
  packets: Array<{
    ...
    complianceEvidence: { legalReviewReady: boolean; securityReviewReady: boolean };
    proof: Record<string, boolean>;
    noLeakSerialization: { passed: boolean };
  }>;
};
