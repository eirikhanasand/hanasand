...
import type {
  CutoverMountedRouteProof,
  CutoverPromotionPacketInput,
  CutoverRehearsalInput,
  CutoverResourceBudget
} from "../../src/ops/liveSearch.ts";

export async function buildLiveInput(): Promise<CutoverPromotionPacketInput> {
  const queries = queryList();
  const randomActorQuery = process.env.CUTOVER_RANDOM_ACTOR_QUERY ?? queries.at(-1) ?? "MuddyWater";
  const [livePublicProofs, apiSearchProofs, healthEndpoints] = await Promise.all([
    ...
    Promise.all(queries.map(async (query) => ({ query, ...(await fetchJsonOrText(apiBase(), postQuery(query)) }))),
    Promise.all([
  ...
  return promotionInput(rehearsalInput(deploymentDrift, livePublicProofs, apiSearchProofs, randomActorQuery));
}

export function mountedRouteProofsFromRecord(value: unknown): CutoverMountedRouteProof[] | undefined {
...
function rehearsalInput(
  deploymentDrift: CutoverRehearsalInput["deploymentDrift"],
  livePublicProofs: CutoverRehearsalInput["livePublicProofs"],
  apiSearchProofs: CutoverRehearsalInput["apiSearchProofs"],
  ...
  return {
    ...
    apiSearchProofs,
    randomActorQuery,
    resourceBudget: readResourceBudget(),
...
function promotionInput(input: CutoverRehearsalInput): CutoverPromotionPacketInput {
  return {
...
function queryList(): string[] {
  return (process.env.CUTOVER_QUERIES ?? "APT29,Scattered Spider,Volt Typhoon,Turla,Akira,MuddyWater")
    .split(",")
    .map((query) => query.trim())
    .filter(Boolean);
}

function workstreams(): CutoverRehearsalInput["workstreams"] {
  return [
    ["source_readiness", "source", "bun test src/tests/sourceSeeds.test.ts"],
    ["scheduler_readiness", "scheduler", "bun test src/tests/schedulerProduction.test.ts"],
    ["clear_web_promotion", "clear_web", "bun test src/tests/adapterFixtures.test.ts"],
    ["public_channel_readiness", "channels", "bun test src/tests/telegramPublic.test.ts"],
    ["restricted_metadata_readiness", "restricted_metadata", "bun test src/tests/darknetMetadata.test.ts"],
    ["evidence_readiness", "evidence", "bun test src/tests/storageCutover.test.ts"],
    ["extraction_quality", "extraction", "bun test src/tests/pipeline.test.ts"],
    ["graph_readiness", "graph", "bun test src/tests/graphViews.test.ts"],
    ["api_readiness", "api", "bun test src/tests/api.test.ts"]
...
async function fetchHealth(name: string, url: string) {
  const result = await fetchJsonOrText(url);
  const json = isRecord(result.json) ? result.json : undefined;
  return { name, ...result, ok: json?.ok === true || json?.status === "ok" || result.body?.toLowerCase().includes("ok") };
}
...
function postQuery(query: string): RequestInit {
  return { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ query }) };
}

function publicBase(): string {
  return process.env.PUBLIC_TI_BASE_URL ?? "https://hanasand.com/ti";
}

function apiBase(): string {
  return process.env.PUBLIC_TI_API_BASE_URL ?? "https://api.hanasand.com/api/ti/search";
}
