...
const checks = [
  ...
  apifyStoreActor.publishedBuildVersion === "0.6.7",
  apifyStoreActor.outputContract === "safe_metadata_only.v1",
  stringArray(apifyStoreActor.categories).join(",") === "SECURITY,MONITORING",
  apifyLatestBuild.buildVersion === "0.6.7",
  apifyLatestProofRun.runId === "OThlfd0uzSCNnedAO",
  apifyLatestProofRun.datasetId === "LSen2fYtwFTtOr7vK",
  apifyLatestProofRun.rowCount === 10,
  ...
  stringArray(apifyStoreReadinessPacket.localPaidPresetProof?.productionBlockers).includes("hosted_100_name_apify_run_not_yet_verified"),
  ...
  ["listing_copy", "sample_rows", "production_sellable_rows", "pricing_shape", "marketplace_telemetry", "payout_setup", "fake_traction_guards", "no_leak_sample_proof"].every((id) => apifyRevenueChecklistRows.some((row) => row.id === id)),
  ...
  isRecord(apifyPricingProof.paidDailyMonitoringShape) && apifyPricingProof.paidDailyMonitoringShape.minimumSellableRows === 100 && apifyPricingProof.paidDailyMonitoringShape.minimumSellableRowRate === 0.25,
  ...
  stringArray(apifyDefaultInput.queries).length === 100,
  apifyDefaultInput.maxRowsPerQuery === 25,
  apifyDefaultInput.includeDatasets === false,
  apifyDefaultInput.includeCoverageGaps === false,
