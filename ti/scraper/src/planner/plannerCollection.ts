...
import { allowedSource, novelty, rankedSources, sourceFreshness, sourceMatchesScope, target } from "./plannerSources.ts";
...
export function createCollectionPlan(input, sources, frontier = new FocusedFrontier()): any {
  ...
  for (const source of rankedSources(sources, queryTerms, createdAt)) {
    ...
    if (tasks.length + reviewRequired.length >= request.maxTasks) { explanations.push(decision(source, "skipped", "request task budget exhausted", undefined, budget.class, queryTerms)); continue; }
    ...
    seen.add(key);
    const freshness = sourceFreshness(source, createdAt), waiting = source.crawlState?.backoffUntil && Date.parse(source.crawlState.backoffUntil) > Date.parse(createdAt);
    ...
    explanations.push(decision(source, waiting ? "waiting-for-backoff" : availableAt ? "delayed" : "selected", waiting ? "source backoff is active; task delayed" : availableAt ? "queued as deeper background work" : "selected by relevance, freshness, trust, and budget", tasks.at(-1), budget.class, queryTerms, availableAt));
  }
