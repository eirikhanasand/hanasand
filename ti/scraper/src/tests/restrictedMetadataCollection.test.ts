...
describe("restricted metadata collection", () => {
    ...
    const candidate = source({
      ...
      metadata: {
        ...
        sourcePortfolioVerification: {
          ...
          legalBasisVerifiedAt: "2026-07-01T09:00:00.000Z",
          outcome: "content_parsed",
          observedItemCount: 1
        }
      }
  ...
  test("promotes an admitted legacy Tor candidate after two productive cycles", async () => {
    const store = new InMemoryScraperStore();
    let victim = "Northwind Health";
    store.saveSource(source({
      id: "src_restricted_legacy_candidate",
      type: "tor_metadata",
      url: `http://${"f".repeat(56)}.onion/`,
      accessMethod: "approved_proxy",
      status: "candidate",
      risk: "restricted",
      crawlFrequencySeconds: 900,
      legalNotes: "Approved public victim-listing metadata only; no interaction or raw content retention.",
      governance: { approvalRequired: true, approvalState: "approved", metadataOnly: true, approvedAt: "2026-07-22T09:00:00.000Z", approvedBy: "reviewer" },
      metadata: { actorName: "Legacy candidate", restrictedMetadataCandidate: true, productionCollection: false, countsAsCoverage: false }
    }));
    const boundary = new TorMetadataHttpBoundary({
      proxyUrl: "http://onion-tor:8118",
      fetcher: async () => new Response(`<title>Legacy notices</title><div class="post-title">${victim}</div>`, { headers: { "content-type": "text/html" } })
    });

    await runRestrictedMetadataCollectionCycle({ store, boundary, now: () => "2026-07-22T10:00:00.000Z" });
    expect(store.getSource("src_restricted_legacy_candidate")).toMatchObject({
      status: "candidate",
      metadata: { productionCollection: false, sourcePortfolioProductiveCheckCount: 1 }
    });

    victim = "Contoso Manufacturing";
    await runRestrictedMetadataCollectionCycle({ store, boundary, now: () => "2026-07-22T10:15:00.000Z" });
    expect(store.getSource("src_restricted_legacy_candidate")).toMatchObject({
      status: "active",
