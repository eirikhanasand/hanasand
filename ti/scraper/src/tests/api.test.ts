...
describe("api v1", () => {
  ...
  test("exposes live product SLO dashboard for Agent 10 revenue monitoring", async () => {
    ...
    expect((response.releaseDecision as {
      ...
      productionSellableRowFloor: number;
      usefulCaveatedRows: number;
      rowsBlockedFromBilling: number;
      oneRepairAwayRows: number;
      projectedSellableRowsFromAcceptedRepairs: number;
      projectedSellableRowsAfterAcceptedRepairs: number;
      topBlocker: string;
      revenueTruth: { paidTrafficAllowed: boolean; apifyAnalyticsExternal: boolean; payoutEvidenceExternal: boolean; revenueEvidenceExternal: boolean; proofSizedRunsMayCompleteShapeSafetyOnly: boolean };
    ...
    const hostedPaidReadinessProof = (response.paidReleaseTruthBoard as {
      hostedPaidReadinessProof: {
        ...
        hostedProofOperatorChecklist: {
          ...
          gateEffects: {
            hosted100: { state: string; unlocks: boolean };
            hosted300: { state: string; unlocks: boolean };
            hosted500: { state: string; unlocks: boolean };
            marketplacePromotion: { state: string; unlocks: boolean };
          };
          validationExamples: Array<{ name: string; unlockSummary: string; expectedStatus: string }>;
        };
    ...
    expect(hostedPaidReadinessProof.hostedProofOperatorChecklist.missingFields).toEqual(expect.arrayContaining(["runId", "datasetId", "sellableRows", "pricingModel", "payoutEnabled", "observedAt"]));
    expect(hostedPaidReadinessProof.hostedProofOperatorChecklist.validationExamples.map((example) => example.name)).toEqual(expect.arrayContaining(["missing_proof", "sample_proof_rejected_for_promotion", "valid_hosted100_hosted300_hold", "valid_hosted300_hosted500_hold", "valid_hosted500_marketplace_hold", "invalid_unsafe_no_leak_proof"]));
    expect(hostedPaidReadinessProof.hostedProofOperatorChecklist.validationExamples.find((example) => example.name === "sample_proof_rejected_for_promotion")).toMatchObject({ expectedStatus: "accepted_sample_no_unlock", unlockSummary: "none" });
    ...
    expect((response.parserToSellableRepairPacket as {
      schemaVersion: string;
      owner: string;
      routeVisibleOn: string[];
      baselineRunId: string;
      ...
      candidates: Array<{ dryRunDecision: string; requiresSourceCorroboration: boolean; provenanceHash: string; nextBuyerSearches: string[]; noLeak: boolean }>;
      rejectedRepairs: Array<{ blockedReason: string; projectedRows: number; doesNotCountToward100Floor: boolean; noLeak: boolean }>;
      ...
      parserFieldsUnlocking: expect.arrayContaining(["victim", "sector", "country", "dataset_or_impact", "ttp_tool", "first_seen", "last_seen", "confidence", "source_family_support", "provenance_hash", "next_buyer_search"]) as unknown as string[],
      sourceFamilyGaps: expect.arrayContaining(["public_report", "government_advisory", "vendor_report"]) as unknown as string[],
      graphPivotGaps: expect.arrayContaining(["claim:victim", "sector:healthcare", "ttp:T1078"]) as unknown as string[],
      suppressionChecks: expect.arrayContaining(["stale_only", "single_source_without_caveat", "contradicted", "unrelated", "unsafe_restricted_only"]) as unknown as string[]
    });
    expect((response.parserToSellableRepairPacket as {
      candidates: Array<{ dryRunDecision: string; requiresSourceCorroboration: boolean; provenanceHash: string; nextBuyerSearches: string[]; noLeak: boolean }>;
    ...
    expect((response.parserToSellableRepairPacket as { rejectedRepairs: Array<{ blockedReason: string }> }).rejectedRepairs.map((row) => row.blockedReason)).toEqual(expect.arrayContaining([
      "stale_report",
      "alias_collision",
    ...
    expect((response.parserToSellableRepairPacket as {
      rejectedRepairs: Array<{ projectedRows: number; doesNotCountToward100Floor: boolean; noLeak: boolean }>;
    }).rejectedRepairs.every((row) => row.projectedRows === 0 && row.doesNotCountToward100Floor && row.noLeak)).toBe(true);
    expect((response.parserToSellableRepairPacket as { ownerHandoffs: Array<{ owner: string }> }).ownerHandoffs.map((row) => row.owner)).toEqual(expect.arrayContaining(["agent_03", "agent_04", "agent_05", "agent_07", "agent_08", "agent_10"]));
    expect((response.parserToSellableRepairPacket as { noLeakBoundary: Record<string, boolean> }).noLeakBoundary).toMatchObject({
    ...
    const graphPublicUnlockQueue = (response.graphPublicCorroborationPivotPacket as { paidRowUnlockQueue: { counts: { admitted_by_parser: number; ready_for_parser: number; ready_for_current_admission: number; ready_for_parser_admission: number; needs_public_source: number; contradicted: number; contradicted_or_alias_hold: number; stale: number; stale_recheck: number; unsafe_or_restricted: number; rowsCountTowardFloorNow: number; rowsReadyAfterParserAdmission: number }; parserAdmissionHandoff: Array<{ actor: string; victimOrTarget: string; sourceFamily: string; freshnessAgeDays: number; contradictionState: string; provenanceHash: string; buyerReason: string; expectedPaidRowLiftAfterParserAdmission: number; programDbPriority: { gapContribution: number; findingLikely: boolean; sourceProvenanceOnlyRisk: string; preferredParserAction: string; admissionBlocker: string }; programDcPriority: { gapContribution: number; findingLikely: boolean; sourceProvenanceOnlyRisk: string; preferredParserAction: string; admissionBlocker: string; sourceFamilyDiversityLift: number; corroborationStrength: string; freshnessRisk: string }; programDdPriority: { gapContribution: number; findingLikely: boolean; sourceProvenanceOnlyRisk: string; preferredParserAction: string; admissionBlocker: string; sourceFamilyDiversityLift: number; corroborationStrength: string; contradictionRisk: string; freshnessRisk: string; buyerVisibleValue: string; noLeakProof: string; nextPivot: string }; programDePriority: { expectedCurrentRowLift: number; confidenceLift: number; freshnessLift: number; sourceFamilyLift: number; contradictionRisk: string; sourceProvenanceOnlyRisk: string; buyerVisibleNextPivot: string; gateContribution: string; noLeakProof: string; admissionBlocker: string }; programFgPriority: { whyCorroborationMatters: string; buyerActionEnabled: string; confidenceDelta: number; freshnessDelta: number; sourceFamilyDelta: number; contradictionRisk: string; parserAdmissionReason: string; nextParserSlice: string; noLeakProof: string; admissionBlocker: string }; admissionState: string; countsTowardFloorNow: boolean; noLeak: boolean }>; ready_for_current_admission: Array<{ actor: string; countsTowardFloorNow: boolean; noLeak: boolean }>; ready_for_parser_admission: Array<{ expectedRowsUnlockedAfterParserAdmission: number; countsTowardFloorNow: boolean; proofUrlHash: string; noLeak: boolean }>; needs_public_source: Array<{ sourceClass: string; countsTowardFloorNow: boolean; noLeak: boolean }>; contradicted_or_alias_hold: Array<{ countsTowardFloorNow: boolean; noLeak: boolean }>; stale_recheck: Array<{ countsTowardFloorNow: boolean; noLeak: boolean }>; programDbRejectionBuckets: Record<string, number | boolean>; programDcRejectionBuckets: Record<string, number | boolean>; programDdRejectionBuckets: Record<string, number | boolean>; programDeRejectionBuckets: Record<string, number | boolean>; graphOnlyCountsTowardPaidFloorNow: boolean; noLeak: boolean } }).paidRowUnlockQueue;
    expect(graphPublicUnlockQueue.counts).toEqual({ admitted_by_parser: 0, ready_for_parser: 1000, ready_for_current_admission: 1000, ready_for_parser_admission: 14, needs_public_source: 6, contradicted: 6, contradicted_or_alias_hold: 6, stale: 4, stale_recheck: 4, unsafe_or_restricted: 0, rowsCountTowardFloorNow: 0, rowsReadyAfterParserAdmission: 25 });
    expect(graphPublicUnlockQueue.parserAdmissionHandoff).toHaveLength(1000);
    expect(graphPublicUnlockQueue.ready_for_current_admission).toHaveLength(1000);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.every((row) => row.actor.length > 0 && row.victimOrTarget.length > 0 && row.sourceFamily.length > 0 && row.freshnessAgeDays >= 0 && row.contradictionState.length > 0 && row.provenanceHash.length > 0 && row.buyerReason.length > 0 && row.expectedPaidRowLiftAfterParserAdmission > 0 && row.programDbPriority.gapContribution > 0 && row.programDbPriority.admissionBlocker === "none" && row.programDcPriority.gapContribution > 0 && row.programDcPriority.admissionBlocker === "none" && row.programDcPriority.sourceFamilyDiversityLift > 0 && row.programDcPriority.corroborationStrength.length > 0 && row.programDcPriority.freshnessRisk.length > 0 && row.programDdPriority.gapContribution > 0 && row.programDdPriority.admissionBlocker === "none" && row.programDdPriority.sourceFamilyDiversityLift > 0 && row.programDdPriority.corroborationStrength.length > 0 && row.programDdPriority.contradictionRisk.length > 0 && row.programDdPriority.freshnessRisk.length > 0 && row.programDdPriority.buyerVisibleValue.length > 0 && row.programDdPriority.noLeakProof === "hash_only_public_or_metadata_reference" && row.programDdPriority.nextPivot.length > 0 && row.programDePriority.confidenceLift >= 0 && row.programDePriority.freshnessLift >= 0 && row.programDePriority.sourceFamilyLift >= 0 && row.programDePriority.contradictionRisk.length > 0 && row.programDePriority.sourceProvenanceOnlyRisk.length > 0 && row.programDePriority.buyerVisibleNextPivot.length > 0 && row.programDePriority.gateContribution.length > 0 && row.programDePriority.noLeakProof === "hash_only_public_or_metadata_reference" && row.programDePriority.admissionBlocker.length > 0 && row.programFgPriority.whyCorroborationMatters.length > 0 && row.programFgPriority.buyerActionEnabled.length > 0 && row.programFgPriority.confidenceDelta > 0 && row.programFgPriority.freshnessDelta > 0 && row.programFgPriority.sourceFamilyDelta > 0 && row.programFgPriority.contradictionRisk.length > 0 && row.programFgPriority.parserAdmissionReason.length > 0 && row.programFgPriority.nextParserSlice.length > 0 && row.programFgPriority.noLeakProof === "hash_only_public_or_metadata_reference" && row.programFgPriority.admissionBlocker === "none" && row.admissionState === "ready_for_parser" && row.countsTowardFloorNow === false && row.noLeak)).toBe(true);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.filter((row) => row.programDdPriority.findingLikely).length).toBeGreaterThanOrEqual(350);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.filter((row) => row.programDePriority.admissionBlocker === "none" && row.programDePriority.expectedCurrentRowLift > 0).length).toBeGreaterThanOrEqual(150);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.filter((row) => row.programDePriority.gateContribution === "current750").length).toBeGreaterThanOrEqual(150);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.filter((row) => row.programFgPriority.nextParserSlice === "current1000_alias_victim_ttp").length).toBeGreaterThanOrEqual(100);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.filter((row) => row.programFgPriority.nextParserSlice === "current1000_source_family_freshness").length).toBeGreaterThanOrEqual(150);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.filter((row) => row.programFgPriority.nextParserSlice === "current1000_metadata_public_support").length).toBeGreaterThanOrEqual(50);
    ...
    expect(graphPublicUnlockQueue.contradicted_or_alias_hold).toHaveLength(6);
    expect(graphPublicUnlockQueue.stale_recheck).toHaveLength(4);
  ...
  test("publishes enterprise contract index for CTI integration without unsafe example leaks", async () => {
    ...
    const apifyStoreReadiness = contract.apifyStoreReadiness as {
      ...
      hundredRowConversionProgress: {
        ...
        acceptedRepairProjection: {
          ...
          graphOnlyRowsCountTowardProductionFloor: boolean;
          proofSizedRunsCountTowardProductionReadiness: boolean;
          caveatOnlyRunsCountTowardProductionReadiness: boolean;
        };
    ...
    expect(apifyStoreReadiness.hundredRowConversionProgress).toMatchObject({
      schemaVersion: "ti.apify_100_row_conversion_proof.v1",
      routeVisibleOn: expect.arrayContaining(["/v1/contracts#apifyStoreReadiness", "Apify OUTPUT", "/v1/ops/product-slo"]),
      currentRun: {
        proofRunId: "OThlfd0uzSCNnedAO",
        proofDatasetId: "LSen2fYtwFTtOr7vK",
        proofDecision: "shape_safety_proof",
        ...
        exactBlockers: expect.arrayContaining([
    ...
    expect(apifyStoreReadiness.hundredRowConversionProgress.acceptedRepairProjection.projectedSellableRowsFromAcceptedRepairs).toBeGreaterThan(0);
  ...
  test("returns non-mutating evidence replay-plan and cutover-report DTOs without sensitive bodies or object keys", async () => {
    ...
    expect(cutoverReport.readModelCutover).toMatchObject({
      ...
      actorDatasetSourceGapSuppressionFeedback: {
        ...
        counts: {
          sourceFamilyGaps: expect.any(Number),
          staleRowsSuppressed: expect.any(Number),
          contextRowsHeld: expect.any(Number),
          billableRowsUnaffected: expect.any(Number)
        },
        suppressionPolicy: {
        ...
        sourceFamilyFeedbackRows: expect.arrayContaining([
          expect.objectContaining({
            currentDatasetDecision: "not_billable_coverage_gap",
            suppressionReason: "missing_source_family",
            requiredBeforePromotion: expect.any(Array),
            noLeak: true
          })
        ]),
        staleSuppressionRows: expect.any(Array),
        restrictedContextRows: expect.any(Array),
        noLeakGuarantees: {
          restrictedRowsMetadataOnly: true,
          rawBodiesExposed: false,
          objectKeysExposed: false,
      ...
      actorDatasetSourceGapConsumerQueue: {
        ...
        counts: {
          totalQueueItems: expect.any(Number),
          agent01SourceActivationItems: expect.any(Number),
          agent04PublicChannelItems: expect.any(Number),
          agent05RestrictedMetadataItems: expect.any(Number),
          agent07ExtractionQualityItems: expect.any(Number),
          staleRefreshItems: expect.any(Number),
          restrictedCorroborationItems: expect.any(Number)
        },
        queueRows: expect.arrayContaining([
          expect.objectContaining({
            ownerQueue: expect.any(String),
            queueAction: expect.any(String),
