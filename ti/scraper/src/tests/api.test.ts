...
describe("api v1", () => {
  ...
  test("exposes live product SLO dashboard for Agent 10 revenue monitoring", async () => {
    ...
    const graphPublicUnlockQueue = (response.graphPublicCorroborationPivotPacket as { paidRowUnlockQueue: { counts: { admitted_by_parser: number; ready_for_parser: number; ready_for_parser_admission: number; needs_public_source: number; contradicted: number; stale: number; unsafe_or_restricted: number; rowsCountTowardFloorNow: number; rowsReadyAfterParserAdmission: number }; parserAdmissionHandoff: Array<{ actor: string; victimOrTarget: string; sourceFamily: string; freshnessAgeDays: number; contradictionState: string; provenanceHash: string; buyerReason: string; expectedPaidRowLiftAfterParserAdmission: number; admissionState: string; countsTowardFloorNow: boolean; noLeak: boolean }>; ready_for_parser_admission: Array<{ expectedRowsUnlockedAfterParserAdmission: number; countsTowardFloorNow: boolean; proofUrlHash: string; noLeak: boolean }>; needs_public_source: Array<{ sourceClass: string; countsTowardFloorNow: boolean; noLeak: boolean }>; graphOnlyCountsTowardPaidFloorNow: boolean; noLeak: boolean } }).paidRowUnlockQueue;
    expect(graphPublicUnlockQueue.counts).toEqual({ admitted_by_parser: 0, ready_for_parser: 40, ready_for_parser_admission: 14, needs_public_source: 6, contradicted: 6, stale: 4, unsafe_or_restricted: 0, rowsCountTowardFloorNow: 0, rowsReadyAfterParserAdmission: 25 });
    expect(graphPublicUnlockQueue.parserAdmissionHandoff).toHaveLength(40);
    expect(graphPublicUnlockQueue.parserAdmissionHandoff.every((row) => row.actor.length > 0 && row.victimOrTarget.length > 0 && row.sourceFamily.length > 0 && row.freshnessAgeDays >= 0 && row.contradictionState.length > 0 && row.provenanceHash.length > 0 && row.buyerReason.length > 0 && row.expectedPaidRowLiftAfterParserAdmission > 0 && row.admissionState === "ready_for_parser" && row.countsTowardFloorNow === false && row.noLeak)).toBe(true);
