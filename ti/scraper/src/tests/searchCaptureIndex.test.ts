...
describe("search capture index", () => {
  ...
  test("indexes retained historical actor evidence", () => {
    ...
    store.saveCapture(fixtureCapture({ id: "cap_apt29_history", sourceId: "src_apt29_history", body: undefined, publishedAt: new Date(Date.now() - 180 * 86_400_000).toISOString(), collectedAt: new Date().toISOString(), metadata: { safeExcerpt: "Amazon disrupted an APT29 watering hole campaign targeting diplomatic organizations with credential phishing and malware." } }));
