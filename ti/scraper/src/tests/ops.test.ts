...
describe("ops controls", () => {
  ...
  test("builds live product SLO dashboard with null revenue unknowns and append-only daily snapshots", async () => {
    ...
    expect(dashboard.graphPublicCorroborationPivotPacket.hostedDefaultPublicCorroborationLift).toMatchObject({
    ...
    expect(dashboard.graphPublicCorroborationPivotPacket.hostedDefaultPublicCorroborationLift.acceptedPublicCorroborationRows.every((row) => row.buyerVisibleMetricImproved !== "none" && row.parserHandoff.length > 0 && row.provenanceHash.length > 0 && row.countsTowardPaidPromotionNow === false && row.noLeak)).toBe(true);
