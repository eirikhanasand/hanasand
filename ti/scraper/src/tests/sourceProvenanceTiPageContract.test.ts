...
import {
  ...
  TI_SOURCE_PROVENANCE_SOURCE_PACK_FIXTURE_ACTION_ALERT_DEDUPE_PACKET_SCHEMA_VERSION,
  TI_SOURCE_PROVENANCE_SOURCE_PACK_FIXTURE_ACTION_ALERT_REBUILD_PACKET_SCHEMA_VERSION,
  ...
  buildSourceProvenanceSourcePackFixtureActionAlertDedupePacket,
  buildSourceProvenanceSourcePackFixtureActionAlertRebuildPacket,
...
describe("source provenance TI page contract", () => {
  ...
  test("packages operator remediation actions into execution readiness rows", () => {
    const aptGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "APT29",
      aliases: ["APT29", "Nobelium"],
      sourceFamily: "actor_page",
      sourceId: "src_actor_page_apt29_execution",
      captureId: "cap_actor_page_apt29_execution",
      contentHash: "hash_actor_page_apt29_execution",
      provenance: "Actor page fixture gives APT29 execution coverage.",
      relationship: "actor_activity"
    });
    const ransomwareGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "Akira",
      aliases: ["Akira"],
      sourceFamily: "public_advisory",
      sourceId: "src_public_advisory_akira_execution",
      captureId: "cap_public_advisory_akira_execution",
      contentHash: "hash_public_advisory_akira_execution",
      provenance: "Public advisory fixture gives Akira execution coverage.",
      relationship: "targeting"
    });
    const catalog = buildSourceProvenanceSourcePackFixtureCatalogPacket({
      growthPackets: [aptGrowthPacket, ransomwareGrowthPacket],
    ...
    const handoff = buildSourceProvenanceSourcePackFixtureAlertReadinessPacket({
      catalog,
      generatedAt: "2026-06-29T15:01:00.000Z"
    });
    const dedupe = buildSourceProvenanceSourcePackFixtureAlertDedupePacket({
      alertReadiness: handoff,
      generatedAt: "2026-06-29T15:02:00.000Z"
    });
    const drilldown = buildSourceProvenanceSourcePackFixtureHealthDrilldownPacket({
      dedupe,
    ...
    const intelligence = buildSourceProvenanceSourcePackFixtureIntelligencePacket({
      drilldown,
      generatedAt: "2026-06-29T15:04:00.000Z"
    });
    const remediation = buildSourceProvenanceSourcePackFixtureOperatorRemediationPacket({
      intelligence,
      generatedAt: "2026-06-29T15:05:00.000Z"
    });
    const execution = buildSourceProvenanceSourcePackFixtureActionExecutionPacket({
      remediation,
      generatedAt: "2026-06-29T15:06:00.000Z"
    });
    ...
    const retryRow = execution.rows.find((row) => row.actor === "APT29" && row.action === "retry_parser");
    const policyRow = execution.rows.find((row) => row.actor === "Akira" && row.action === "request_policy_review");
    const watchlistRow = execution.rows.find((row) => row.actor === "Akira" && row.action === "materialize_watchlist_terms");
    const inspectionRow = execution.rows.find((row) => row.actor === "APT29" && row.action === "inspect_source_health");
    ...
    expect(policyRow).toMatchObject({
      actor: "Akira",
      sourceFamily: "darkweb_metadata",
      executionState: "policy_review_required",
      typedFailureReason: {
        code: "policy_review_required",
        ownerLane: "policy",
        field: "sourceFamily",
      ...
      downstreamReadiness: expect.objectContaining({ publicTI: false, alertGeneration: false })
    });
    expect(watchlistRow).toMatchObject({
      actor: "Akira",
      sourceFamily: "public_advisory",
    ...
    expect(watchlistRow?.typedFailureReason).toBeUndefined();
    expect(inspectionRow).toMatchObject({
      actor: "APT29",
      executionState: "inspection_required",
      typedFailureReason: {
    ...
    expect(execution.consumers).toEqual(expect.arrayContaining([
      expect.objectContaining({ consumer: "publicTI", ready: true, requiredFields: expect.arrayContaining(["rows[].publicTiRoute", "rows[].executionState", "rows[].typedFailureReason"]) }),
      expect.objectContaining({ consumer: "alertGeneration", ready: true, requiredFields: expect.arrayContaining(["rows[].downstreamReadiness", "rows[].provenance"]) }),
      expect.objectContaining({ consumer: "dashboard", ready: true, requiredFields: expect.arrayContaining(["summary", "rows[].priority", "rows[].typedFailureReason"]) }),
      expect.objectContaining({ consumer: "sourceOps", ready: true, requiredFields: expect.arrayContaining(["rows[].ownerLane", "rows[].canExecuteDryRun", "rows[].retry"]) }),
      expect.objectContaining({ consumer: "support", ready: true, requiredFields: expect.arrayContaining(["rows[].publicTiRoute", "rows[].typedFailureReason", "safeOutput"]) }),
      expect.objectContaining({ consumer: "integration", ready: true, requiredFields: expect.arrayContaining(["sourcePackFixtureOperatorRemediationPacketId", "rows[].rowId", "safeOutput"]) })
    ]));
    expect(execution.payloadShape).toEqual(expect.arrayContaining([
      "rows[].action",
      "rows[].executionState",
  ...
  test("builds source fixture action drilldown filters for operator and support consumers", () => {
    const aptGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "APT29",
      aliases: ["APT29", "Nobelium"],
      sourceFamily: "actor_page",
      sourceId: "src_actor_page_apt29_action_drilldown",
      captureId: "cap_actor_page_apt29_action_drilldown",
      contentHash: "hash_actor_page_apt29_action_drilldown",
      provenance: "Actor page fixture gives APT29 action drilldown coverage.",
      relationship: "actor_activity"
    });
    const ransomwareGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "Akira",
      aliases: ["Akira"],
      sourceFamily: "public_advisory",
      sourceId: "src_public_advisory_akira_action_drilldown",
      captureId: "cap_public_advisory_akira_action_drilldown",
      contentHash: "hash_public_advisory_akira_action_drilldown",
      provenance: "Public advisory fixture gives Akira action drilldown coverage.",
      relationship: "targeting"
    });
    const catalog = buildSourceProvenanceSourcePackFixtureCatalogPacket({
      growthPackets: [aptGrowthPacket, ransomwareGrowthPacket],
      generatedAt: "2026-06-29T15:20:00.000Z"
    });
    const handoff = buildSourceProvenanceSourcePackFixtureAlertReadinessPacket({
      catalog,
      generatedAt: "2026-06-29T15:21:00.000Z"
    });
    const dedupe = buildSourceProvenanceSourcePackFixtureAlertDedupePacket({
      alertReadiness: handoff,
      generatedAt: "2026-06-29T15:22:00.000Z"
    });
    const drilldown = buildSourceProvenanceSourcePackFixtureHealthDrilldownPacket({
      dedupe,
      generatedAt: "2026-06-29T15:23:00.000Z"
    });
    const intelligence = buildSourceProvenanceSourcePackFixtureIntelligencePacket({
      drilldown,
      generatedAt: "2026-06-29T15:24:00.000Z"
    });
    const remediation = buildSourceProvenanceSourcePackFixtureOperatorRemediationPacket({
      intelligence,
      generatedAt: "2026-06-29T15:25:00.000Z"
    });
    const execution = buildSourceProvenanceSourcePackFixtureActionExecutionPacket({
      remediation,
      generatedAt: "2026-06-29T15:26:00.000Z"
    });
    const audit = buildSourceProvenanceSourcePackFixtureActionAuditPacket({
      execution,
      generatedAt: "2026-06-29T15:27:00.000Z"
    });
    const actionDrilldown = buildSourceProvenanceSourcePackFixtureActionDrilldownPacket({
      audit,
      generatedAt: "2026-06-29T15:28:00.000Z"
    });

    expect(actionDrilldown).toMatchObject({
      ...
      summary: {
        ...
        actors: expect.arrayContaining(["APT29", "Akira"]),
        sourceFamilies: expect.arrayContaining(["actor_page", "public_advisory", "telegram_public", "darkweb_metadata"]),
        parserStates: expect.arrayContaining(["healthy", "retry_scheduled", "policy_blocked", "inspection_required"]),
        healthStates: expect.arrayContaining(["active", "degraded", "blocked"]),
        activationStates: expect.arrayContaining(["ready", "retry_waiting", "policy_review_required", "inspect_required"]),
        failureCodes: expect.arrayContaining(["retry_window_pending", "policy_review_required", "source_health_inspection_required"]),
        nextRetryAt: "2026-06-29T12:37:00.000Z"
      },
      safeOutput: {
        rawTargetsExposed: false,
    ...
    const retryRow = actionDrilldown.drilldownRows.find((row) => row.actor === "APT29" && row.activationState === "retry_waiting");
    const policyRow = actionDrilldown.drilldownRows.find((row) => row.actor === "Akira" && row.activationState === "policy_review_required");
    const readyRow = actionDrilldown.drilldownRows.find((row) => row.actor === "Akira" && row.activationState === "ready");
    const inspectionRow = actionDrilldown.drilldownRows.find((row) => row.actor === "APT29" && row.activationState === "inspect_required");
    ...
    expect(retryRow).toMatchObject({
      ...
      activationState: "retry_waiting",
      affectedConsumers: expect.arrayContaining(["dashboard", "sourceOps", "support"]),
      failure: {
        code: "retry_window_pending",
        ownerLane: "parser",
        field: "retry.nextRetryAt",
    ...
    expect(policyRow).toMatchObject({
      actor: "Akira",
      sourceFamily: "darkweb_metadata",
      parserState: "policy_blocked",
      healthState: "blocked",
      activationState: "policy_review_required",
      failure: {
        code: "policy_review_required",
        ownerLane: "policy",
        field: "sourceFamily",
    ...
    expect(readyRow).toMatchObject({
      ...
      affectedConsumers: expect.arrayContaining(["publicTI", "alertGeneration"])
    });
    expect(readyRow?.failure).toBeUndefined();
    expect(inspectionRow).toMatchObject({
      actor: "APT29",
      parserState: "inspection_required",
      healthState: "degraded",
      activationState: "inspect_required",
      affectedConsumers: expect.arrayContaining(["publicTI", "dashboard", "support"]),
      failure: {
        code: "source_health_inspection_required",
        ownerLane: "source",
        field: "route.body",
    ...
    expect(actionDrilldown.filters).toEqual(expect.arrayContaining([
      expect.objectContaining({
        kind: "failure_code",
        value: "retry_window_pending",
        count: 2,
        operatorAction: expect.objectContaining({
          action: "retry_parser",
          route: expect.objectContaining({ path: "/v1/dwm/source-requests", liveNetworkFetch: false })
      ...
      expect.objectContaining({
        ...
        count: 2,
        operatorAction: expect.objectContaining({ route: expect.objectContaining({ liveNetworkFetch: false }) })
      }),
      expect.objectContaining({
        kind: "affected_consumer",
        value: "alertGeneration",
        count: 2,
        operatorAction: expect.objectContaining({ action: "materialize_watchlist_terms" })
      })
    ]));
    expect(actionDrilldown.consumers).toEqual(expect.arrayContaining([
      expect.objectContaining({ consumer: "publicTI", ready: true, requiredFields: expect.arrayContaining(["drilldownRows[].publicTiRoute", "drilldownRows[].healthState", "drilldownRows[].failure"]) }),
      expect.objectContaining({ consumer: "alertGeneration", ready: true, requiredFields: expect.arrayContaining(["drilldownRows[].activationState", "drilldownRows[].provenance"]) }),
      expect.objectContaining({ consumer: "dashboard", ready: true, requiredFields: expect.arrayContaining(["filters", "summary", "drilldownRows[].parserState"]) }),
      expect.objectContaining({ consumer: "sourceOps", ready: true, requiredFields: expect.arrayContaining(["filters[].operatorAction", "drilldownRows[].retry"]) }),
      expect.objectContaining({ consumer: "support", ready: true, requiredFields: expect.arrayContaining(["drilldownRows[].publicTiRoute", "drilldownRows[].failure", "safeOutput"]) }),
      expect.objectContaining({ consumer: "integration", ready: true, requiredFields: expect.arrayContaining(["sourcePackFixtureActionAuditPacketId", "drilldownRows[].rowId", "safeOutput"]) })
    ]));
    expect(actionDrilldown.payloadShape).toEqual(expect.arrayContaining([
  ...
  test("dedupes source fixture alert bridge rows before alert rebuild consumers", () => {
    const aptGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "APT29",
      aliases: ["APT29", "Nobelium"],
      sourceFamily: "actor_page",
      sourceId: "src_actor_page_apt29_action_alert_dedupe",
      captureId: "cap_actor_page_apt29_action_alert_dedupe",
      contentHash: "hash_actor_page_apt29_action_alert_dedupe",
      provenance: "Actor page fixture gives APT29 action alert dedupe coverage.",
      relationship: "actor_activity"
    });
    const ransomwareGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "Akira",
      aliases: ["Akira"],
      sourceFamily: "public_advisory",
      sourceId: "src_public_advisory_akira_action_alert_dedupe",
      captureId: "cap_public_advisory_akira_action_alert_dedupe",
      contentHash: "hash_public_advisory_akira_action_alert_dedupe",
      provenance: "Public advisory fixture gives Akira action alert dedupe coverage.",
      relationship: "targeting"
    });
    const catalog = buildSourceProvenanceSourcePackFixtureCatalogPacket({
      growthPackets: [aptGrowthPacket, ransomwareGrowthPacket],
      generatedAt: "2026-06-29T15:40:00.000Z"
    });
    const handoff = buildSourceProvenanceSourcePackFixtureAlertReadinessPacket({
      catalog,
      generatedAt: "2026-06-29T15:41:00.000Z"
    });
    const dedupe = buildSourceProvenanceSourcePackFixtureAlertDedupePacket({
      alertReadiness: handoff,
      generatedAt: "2026-06-29T15:42:00.000Z"
    });
    const health = buildSourceProvenanceSourcePackFixtureHealthDrilldownPacket({
      dedupe,
      generatedAt: "2026-06-29T15:43:00.000Z"
    });
    const intelligence = buildSourceProvenanceSourcePackFixtureIntelligencePacket({
      drilldown: health,
      generatedAt: "2026-06-29T15:44:00.000Z"
    });
    const remediation = buildSourceProvenanceSourcePackFixtureOperatorRemediationPacket({
      intelligence,
      generatedAt: "2026-06-29T15:45:00.000Z"
    });
    const execution = buildSourceProvenanceSourcePackFixtureActionExecutionPacket({
      remediation,
      generatedAt: "2026-06-29T15:46:00.000Z"
    });
    const audit = buildSourceProvenanceSourcePackFixtureActionAuditPacket({
      execution,
      generatedAt: "2026-06-29T15:47:00.000Z"
    });
    const actionDrilldown = buildSourceProvenanceSourcePackFixtureActionDrilldownPacket({
      audit,
      generatedAt: "2026-06-29T15:48:00.000Z"
    });
    const alertBridge = buildSourceProvenanceSourcePackFixtureActionAlertBridgePacket({
      drilldown: actionDrilldown,
      generatedAt: "2026-06-29T15:49:00.000Z"
    });
    const canonicalCandidate = alertBridge.rows.find((row) => row.actor === "Akira" && row.coverageState === "alert_ready");
    expect(canonicalCandidate).toBeDefined();
    const duplicatedBridge = {
      ...alertBridge,
      rows: canonicalCandidate ? [...alertBridge.rows, { ...canonicalCandidate, rowId: `${canonicalCandidate.rowId}_duplicate` }] : alertBridge.rows
    };
    const actionDedupe = buildSourceProvenanceSourcePackFixtureActionAlertDedupePacket({
      alertBridge: duplicatedBridge,
      generatedAt: "2026-06-29T15:50:00.000Z"
    });

    expect(actionDedupe).toMatchObject({
      schemaVersion: TI_SOURCE_PROVENANCE_SOURCE_PACK_FIXTURE_ACTION_ALERT_DEDUPE_PACKET_SCHEMA_VERSION,
      ok: true,
      status: "partial",
      tenantId: "tenant_acme",
      organizationId: "org_acme",
      sourcePackFixtureActionAlertBridgePacketId: alertBridge.id,
      summary: {
        rowCount: 11,
        canonicalRows: 2,
        duplicateRows: 1,
        ...
        sourceFamilies: expect.arrayContaining(["actor_page", "public_advisory", "telegram_public", "darkweb_metadata"]),
        blockerCodes: expect.arrayContaining(["retry_window_pending", "policy_review_required", "source_health_inspection_required", "source_not_alert_ready"]),
        dedupeStates: expect.arrayContaining(["canonical", "duplicate", "held"]),
        nextRetryAt: "2026-06-29T12:37:00.000Z"
      },
      safeOutput: {
        rawTargetsExposed: false,
    ...
    const canonicalRow = actionDedupe.rows.find((row) => row.actor === "Akira" && row.dedupeState === "canonical");
    const duplicateRow = actionDedupe.rows.find((row) => row.actor === "Akira" && row.dedupeState === "duplicate");
    const heldRetryRow = actionDedupe.rows.find((row) => row.actor === "APT29" && row.sourceFamily === "telegram_public");
    const heldPolicyRow = actionDedupe.rows.find((row) => row.actor === "Akira" && row.sourceFamily === "darkweb_metadata");
    ...
    expect(duplicateRow).toMatchObject({
      actor: "Akira",
      dedupeState: "duplicate",
      duplicateOf: canonicalRow?.rowId,
      dedupeKey: canonicalRow?.dedupeKey,
      action: {
        action: "suppress_duplicate",
        route: expect.objectContaining({ method: "GET", path: "/v1/dwm/source-requests", liveNetworkFetch: false })
      }
    });
    expect(heldRetryRow).toMatchObject({
      actor: "APT29",
      sourceFamily: "telegram_public",
      dedupeState: "held",
      alertReadiness: "blocked",
      blockerCodes: expect.arrayContaining(["retry_window_pending", "source_not_alert_ready"]),
      action: expect.objectContaining({ action: "review_blockers" })
    });
    expect(heldPolicyRow).toMatchObject({
      actor: "Akira",
      sourceFamily: "darkweb_metadata",
      dedupeState: "held",
      alertReadiness: "blocked",
      blockerCodes: expect.arrayContaining(["policy_review_required", "source_not_alert_ready"])
    });

    expect(actionDedupe.consumers).toEqual(expect.arrayContaining([
      expect.objectContaining({ consumer: "publicTI", ready: true, requiredFields: expect.arrayContaining(["rows[].publicTiRoute", "rows[].dedupeState", "rows[].blockerCodes"]) }),
      expect.objectContaining({ consumer: "alertGeneration", ready: true, requiredFields: expect.arrayContaining(["rows[].dedupeKey", "rows[].dedupeState", "rows[].action", "rows[].provenance"]) }),
      expect.objectContaining({ consumer: "dashboard", ready: true, requiredFields: expect.arrayContaining(["summary", "rows[].dedupeState", "rows[].coverageState"]) }),
      expect.objectContaining({ consumer: "sourceOps", ready: true, requiredFields: expect.arrayContaining(["rows[].freshness", "rows[].blockerCodes", "rows[].action"]) }),
      expect.objectContaining({ consumer: "support", ready: true, requiredFields: expect.arrayContaining(["rows[].publicTiRoute", "rows[].blockerCodes", "safeOutput"]) }),
      expect.objectContaining({ consumer: "integration", ready: true, requiredFields: expect.arrayContaining(["sourcePackFixtureActionAlertBridgePacketId", "rows[].rowId", "safeOutput"]) })
    ]));
    expect(actionDedupe.payloadShape).toEqual(expect.arrayContaining([
      "rows[].dedupeKey",
      "rows[].dedupeState",
      "rows[].duplicateOf",
      "rows[].alertReadiness",
    ...
    expect(JSON.stringify(actionDedupe)).not.toContain("rawText");
    expect(JSON.stringify(actionDedupe)).not.toContain("password");
  });

  test("builds source fixture alert rebuild requests from deduped alert bridge rows", () => {
    const aptGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "APT29",
      aliases: ["APT29", "Nobelium"],
      sourceFamily: "actor_page",
      sourceId: "src_actor_page_apt29_action_alert_rebuild",
      captureId: "cap_actor_page_apt29_action_alert_rebuild",
      contentHash: "hash_actor_page_apt29_action_alert_rebuild",
      provenance: "Actor page fixture gives APT29 action alert rebuild coverage.",
      relationship: "actor_activity"
    });
    const ransomwareGrowthPacket = buildActorFixtureGrowthPacket({
      actor: "Akira",
      aliases: ["Akira"],
      sourceFamily: "public_advisory",
      sourceId: "src_public_advisory_akira_action_alert_rebuild",
      captureId: "cap_public_advisory_akira_action_alert_rebuild",
      contentHash: "hash_public_advisory_akira_action_alert_rebuild",
      provenance: "Public advisory fixture gives Akira action alert rebuild coverage.",
      relationship: "targeting"
    });
    const catalog = buildSourceProvenanceSourcePackFixtureCatalogPacket({
      growthPackets: [aptGrowthPacket, ransomwareGrowthPacket],
      generatedAt: "2026-06-29T15:51:00.000Z"
    });
    const handoff = buildSourceProvenanceSourcePackFixtureAlertReadinessPacket({
      catalog,
      generatedAt: "2026-06-29T15:52:00.000Z"
    });
    const dedupe = buildSourceProvenanceSourcePackFixtureAlertDedupePacket({
      alertReadiness: handoff,
      generatedAt: "2026-06-29T15:53:00.000Z"
    });
    const health = buildSourceProvenanceSourcePackFixtureHealthDrilldownPacket({
      dedupe,
      generatedAt: "2026-06-29T15:54:00.000Z"
    });
    const intelligence = buildSourceProvenanceSourcePackFixtureIntelligencePacket({
      drilldown: health,
      generatedAt: "2026-06-29T15:55:00.000Z"
    });
    const remediation = buildSourceProvenanceSourcePackFixtureOperatorRemediationPacket({
      intelligence,
      generatedAt: "2026-06-29T15:56:00.000Z"
    });
    const execution = buildSourceProvenanceSourcePackFixtureActionExecutionPacket({
      remediation,
      generatedAt: "2026-06-29T15:57:00.000Z"
    });
    const audit = buildSourceProvenanceSourcePackFixtureActionAuditPacket({
      execution,
      generatedAt: "2026-06-29T15:58:00.000Z"
    });
    const actionDrilldown = buildSourceProvenanceSourcePackFixtureActionDrilldownPacket({
      audit,
      generatedAt: "2026-06-29T15:59:00.000Z"
    });
    const alertBridge = buildSourceProvenanceSourcePackFixtureActionAlertBridgePacket({
      drilldown: actionDrilldown,
    ...
    const actionDedupe = buildSourceProvenanceSourcePackFixtureActionAlertDedupePacket({
      alertBridge: duplicatedBridge,
      generatedAt: "2026-06-29T16:01:00.000Z"
    });
    const rebuild = buildSourceProvenanceSourcePackFixtureActionAlertRebuildPacket({
      dedupe: actionDedupe,
      generatedAt: "2026-06-29T16:02:00.000Z"
    });

    expect(rebuild).toMatchObject({
      schemaVersion: TI_SOURCE_PROVENANCE_SOURCE_PACK_FIXTURE_ACTION_ALERT_REBUILD_PACKET_SCHEMA_VERSION,
      ok: true,
      status: "partial",
      tenantId: "tenant_acme",
      organizationId: "org_acme",
      sourcePackFixtureActionAlertDedupePacketId: actionDedupe.id,
      summary: {
        requestCount: 11,
        readyRequests: 2,
        blockedRequests: 8,
        ...
        actors: expect.arrayContaining(["APT29", "Akira"]),
        sourceFamilies: expect.arrayContaining(["actor_page", "public_advisory", "telegram_public", "darkweb_metadata"]),
        blockerCodes: expect.arrayContaining(["retry_window_pending", "policy_review_required", "source_health_inspection_required", "source_not_alert_ready", "duplicate_source_action"]),
        matchableFields: expect.arrayContaining(["actor", "source_family", "public_ti_route", "provenance_hash", "watchlist_term", "indicator", "ttp", "actor_alias", "source_freshness"]),
        nextRetryAt: "2026-06-29T12:37:00.000Z"
      },
      safeOutput: {
    ...
    const readyRequest = rebuild.requests.find((request) => request.actor === "Akira" && request.requestState === "ready");
    const suppressedRequest = rebuild.requests.find((request) => request.actor === "Akira" && request.requestState === "suppressed");
    const retryBlockedRequest = rebuild.requests.find((request) => request.actor === "APT29" && request.sourceFamily === "telegram_public");
    const policyBlockedRequest = rebuild.requests.find((request) => request.actor === "Akira" && request.sourceFamily === "darkweb_metadata");
    ...
    expect(readyRequest).toMatchObject({
      ...
      sourceFamily: "public_advisory",
      requestState: "ready",
      matchableFields: expect.arrayContaining(["watchlist_term", "indicator", "ttp"]),
      blockers: [],
      route: expect.objectContaining({
        method: "POST",
        path: "/v1/dwm/source-requests",
    ...
    expect(readyRequest?.alertGeneratorKey).toMatch(/^ti_source_provenance_source_pack_fixture_action_alert_generator_/);
    expect(suppressedRequest).toMatchObject({
      actor: "Akira",
      requestState: "suppressed",
      dedupeKey: readyRequest?.dedupeKey,
      blockers: expect.arrayContaining([
        expect.objectContaining({ code: "duplicate_source_action", ownerLane: "alert", field: "dedupeKey", nextAction: "suppress_duplicate" })
      ]),
      route: expect.objectContaining({
        method: "GET",
        path: "/v1/dwm/source-requests",
    ...
    expect(retryBlockedRequest).toMatchObject({
      actor: "APT29",
      sourceFamily: "telegram_public",
      ...
      blockers: expect.arrayContaining([
        expect.objectContaining({ code: "retry_window_pending", ownerLane: "parser", field: "freshness.nextRetryAt" }),
        expect.objectContaining({ code: "source_not_alert_ready", ownerLane: "alert", field: "alertReadiness" })
      ])
    });
    expect(policyBlockedRequest).toMatchObject({
      actor: "Akira",
      sourceFamily: "darkweb_metadata",
      requestState: "blocked",
      blockers: expect.arrayContaining([
    ...
    expect(rebuild.consumers).toEqual(expect.arrayContaining([
      expect.objectContaining({ consumer: "publicTI", ready: true, requiredFields: expect.arrayContaining(["requests[].publicTiRoute", "requests[].requestState", "requests[].blockers"]) }),
      expect.objectContaining({ consumer: "alertGeneration", ready: true, requiredFields: expect.arrayContaining(["requests[].requestState", "requests[].alertGeneratorKey", "requests[].dedupeKey", "requests[].provenance"]) }),
      expect.objectContaining({ consumer: "dashboard", ready: true, requiredFields: expect.arrayContaining(["summary", "requests[].requestState", "requests[].blockers"]) }),
      expect.objectContaining({ consumer: "sourceOps", ready: true, requiredFields: expect.arrayContaining(["requests[].freshness", "requests[].route", "requests[].blockers"]) }),
      expect.objectContaining({ consumer: "support", ready: true, requiredFields: expect.arrayContaining(["requests[].publicTiRoute", "requests[].blockers", "safeOutput"]) }),
      expect.objectContaining({ consumer: "integration", ready: true, requiredFields: expect.arrayContaining(["sourcePackFixtureActionAlertDedupePacketId", "requests[].requestId", "safeOutput"]) })
    ]));
    expect(rebuild.payloadShape).toEqual(expect.arrayContaining([
      "requests[].requestState",
      "requests[].dedupeKey",
      "requests[].alertGeneratorKey",
      "requests[].matchableFields",
      "requests[].blockers",
      "requests[].route",
      "requests[].provenance",
      "summary"
    ]));
    expect(JSON.stringify(rebuild)).not.toContain("rawText");
    expect(JSON.stringify(rebuild)).not.toContain("password");
  });
