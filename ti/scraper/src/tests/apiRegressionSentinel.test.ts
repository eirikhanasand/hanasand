...
describe("api regression sentinel", () => {
  ...
  test("publishes the case handoff action receipt contract for workflow consumers", async () => {
    ...
    expect(receiptSurface).toMatchObject({
      ...
      recordFields: expect.arrayContaining(["receiptId", "caseId", "alertId", "actionId", "auditEventId", "workflowEventId", "idempotencyKey", "dedupeKey", "captureIds", "sourceIds", "contentHashes", "webhookDeliveryId", "webhookDestinationId", "endpointHash", "payloadHash", "sourceFamily", "sourceHandoffReadiness", "nextAnalystActions"]),
      blockerCodes: expect.arrayContaining(["case_not_found", "missing_case_alert", "handoff_action_not_ready", "case_read_only_member", "missing_webhook_destination", "missing_webhook_dry_run_receipt", "missing_alert_source_handoff_readiness", "public_ti_handoff_not_ready"])
    });
