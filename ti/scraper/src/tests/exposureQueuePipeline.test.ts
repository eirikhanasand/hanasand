...
describe("DWM exposure queue pipeline", () => {
  ...
  test("rejects dark-web and Telegram manual intake without retained collector evidence", async () => {
    for (const sourceFamily of ["darkweb_metadata", "telegram_public"]) {
      const store = new InMemoryScraperStore();
      const response = await handleApiRequest(authenticatedRequest("http://local/v1/dwm/exposure-claims/ingest", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ items: [{ actor: "Akira", company: "Contoso", sourceFamily }] })
      }), testOptions(store));
  ...
  test("promotes collected scraper findings into the exposure queue automatically", async () => {
    ...
    const collected = {
      ...
      collectedAt,
      publishedAt,
      metadata: { adapter: "public_advisory", reportTimestamps: [{ role: "publisher", timestamp: publishedAt, sourceId: "src_public_news", referenceUrl: "https://www.ransomware.live/id/RmFicmlrYW0gTWFudWZhY3R1cmluZ0Bha2lyYQ==", evidencePath: "feed.entry.publishedAt", extractionMethod: "source_field" }] }
    };
    ...
    expect(store.listExtractedEntities().some((entity: any) => entity.type === "publication_strategy" && entity.value === "public victim listing")).toBe(false);
