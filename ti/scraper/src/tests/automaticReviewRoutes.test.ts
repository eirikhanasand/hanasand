...
describe("automatic Hanasand AI intelligence review", () => {
  ...
  test("fairly interleaves oldest due claims and incidents without starving retries", async () => {
    const store = seededClaimStore();
    for (let index = 0; index < 7; index++) {
      seedClaim(store, `claim_${index}`, `APT29 targeted tenant ${index}.`);
      store.saveClaimEvidence(claimEvidence(`evidence_${index}`, `claim_${index}`, "capture_source_a", "source_a", 0.8));
    }
    ...
    const retry = tasks.find((task: any) => task.subject.id === "claim_actor");
    store.saveAnalystMetadataReviewTask({ ...retry, state: "retrying", attempt: 1, queuedAt: "2026-07-22T09:00:00.000Z", nextAttemptAt: firstAt, lastError: "Hanasand AI returned HTTP 503" });
    ...
    expect(requests.find((request) => request.subject.id === "claim_actor")?.retryCorrection).toBeUndefined();
