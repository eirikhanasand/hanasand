...
describe("scheduled API collection runs", () => {
  test("requires explicit production approval for automatic evaluation", () => {
  ...
  test("executes a queued run and exposes its captured results", async () => {
    ...
    store.replaceActorIdentityCatalog({
      ...
      identities: [{
        ...
        domains: ["enterprise-attack"], contributors: ["Daniyal Naeem, BT Security", "Matt Brenton, Zurich Insurance Group", "Katie Nickels, Red Canary", "Joe Gumke, U.S. Bank", "Liran Ravich, CardinalOps", "Vicky Ray, RayvenX"], referenceSources: [{ name: "MITRE ATT&CK", url: "https://attack.mitre.org/groups/G0016" }],
