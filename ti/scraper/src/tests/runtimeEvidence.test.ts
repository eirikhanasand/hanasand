...
} from "../api/evidenceDtos.ts";
import { InMemoryObjectEvidenceStore } from "../storage/memoryStore.ts";
...
describe("runtime evidence persistence", () => {
  test("persists public-channel runtime promotion into replayable evidence deltas", () => {
    ...
    store.savePlan({
      ...
      tasks: [task("src_runtime_q", "tenant_q")],
      reviewRequired: [],
      rejected: [],
      audit: []
    });
    ...
    const runtime = buildTelegramPublicRuntimeCollection({
      source: src,
      task: task("src_runtime_q", "tenant_q"),
      result: {
        items: [
          collectedTelegramItem("src_runtime_q", 3001, "APT29 used CVE-2026-9999 against victim: Fjord Energy AS"),
          collectedTelegramItem("src_runtime_q", 3002, "APT29 edited public-channel context", { editDate: "2026-01-01T00:04:00.000Z" }),
          collectedTelegramItem("src_runtime_q", 3003, "", { deleted: true })
        ],
        discovered: [],
        warnings: []
      },
    ...
    const persisted = persistTelegramPublicRuntimeEvidence(store, runtime, {
      tenantId: "tenant_q",
      runId: "run_q",
      query: "APT29",
    ...
    expect(persisted.safeOutput).toEqual({
    ...
    expect(timeline.find((delta) => delta.kind === "expired")?.metadata).toMatchObject({
    ...
    const report = buildEvidenceCutoverReportDto(store, objectStore, "APT29", {
      tenantId: "tenant_q",
      runId: "run_q",
    ...
    expect(persisted.snapshot.metadata.connector).toMatchObject({
      ...
      deltas: {
        newMessageIds: [3001],
        editedMessageIds: [3002],
        deletedOrUnavailableMessageIds: [3003]
      }
    });
    expect(store.getSource("src_runtime_q")?.metadata?.telegramPublicConnector).toMatchObject({
      sourceHealthPatch: {
        sourceId: "src_runtime_q",
...
function source(input: Partial<SourceRecord> = {}): SourceRecord {
  return {
    id: input.id ?? "src_runtime_q",
    tenantId: input.tenantId,
    name: input.name ?? "Public Telegram Runtime Channel",
    ...
    legalNotes: "Public channel reviewed for CTI collection through official Telegram APIs.",
...
function collectedTelegramItem(sourceId: string, id: number, text: string, metadata: Record<string, unknown> = {}): CollectedItem {
  ...
  return {
    ...
    metadata: {
      ...
      urlMentions: text.includes("CVE-2026-9999") ? ["https://reports.example/apt29"] : [],
      media: { retention: "metadata_only", items: [] },
      extractionHandoff: {
        messageText: text,
        actorAliases: text.includes("APT29") ? ["APT29"] : [],
        cves: text.includes("CVE-2026-9999") ? ["CVE-2026-9999"] : [],
        victims: text.includes("Fjord Energy AS") ? ["Fjord Energy AS"] : [],
