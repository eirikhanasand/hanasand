...
describe("api v1", () => {
  test("executes approved public canary activation collection and exposes operator view", async () => {
    ...
    const canaryFetch = async () =>
      new Response(
        `       <rss><channel><item>         <title>APT42 phishing campaign and CVE-2026-11111 exploitation</title>         <link>https://example.test/apt42-campaign</link>         <description>APT42 used malware in a phishing campaign targeting energy sector victims. Indicator 198.51.100.44 observed.</description>         <pubDate>${publishedAt}</pubDate>       </item></channel></rss>     `,
        { status: 200, headers: { "content-type": "application/rss+xml" } },
      );
    const options = { store, frontier, objectStore, canaryFetch };
    const activationBlocked = await body(
      await handleApiRequest(
        api("/v1/sources/canary-activation", { method: "POST", body: "{}" }),
        options,
      ),
    );
    expect((activationBlocked.error as { code: string }).code).toBe(
      "approval_required",
    );
    const activation = await body(
      await handleApiRequest(
    ...
    expect((activation.activation as { activated: unknown[] }).activated.length)
      .toBeGreaterThanOrEqual(8);
    expect(store.listSources().every((item) => item.status === "active")).toBe(
      true,
    );
    ...
    expect(operatorView.schedulerHealth).toMatchObject({
      errorRate: 0,
      promotionYield: 1,
      duplicateRate: 0,
      retryScheduledCount: 0,
    ...
    expect(
      operatorView.publicAnswerReadiness.find((item) => item.query === "APT42")
        ?.captureCount,
    ).toBe(1);
    ...
    const search = await body(
      await handleApiRequest(api("/v1/intel/search?q=APT42"), options),
    );
    expect(search.status).toMatch(/ready|partial/);
    expect((search.publicTiAnswer as { evidenceLedgerReferences: unknown[] }).evidenceLedgerReferences.length).toBeGreaterThan(0);
    expect(JSON.stringify(search.actorProfile)).toContain("APT42");
    const pauseBlocked = await body(
      await handleApiRequest(
        api("/v1/sources/canary-pause", { method: "POST", body: "{}" }),
        options,
      ),
    );
    expect((pauseBlocked.error as { code: string }).code).toBe(
      "approval_required",
    );
    const pause = await body(
      await handleApiRequest(
        api("/v1/sources/canary-pause", {
          ...
          body: JSON.stringify({
            operatorApproval: true,
            approvedBy: "analyst-1",
            generatedAt: "2026-05-24T10:02:00.000Z",
          }),
        }),
        options,
      ),
    );
    expect((pause.pause as { paused: unknown[] }).paused.length)
      .toBeGreaterThanOrEqual(8);
    expect(
