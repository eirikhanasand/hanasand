...
describe("api v1", () => {
  test("publishes canary soak health across repeated portfolio-only cycles", async () => {
    ...
    const canaryFetch = async (url: string) => {
      ...
      return new Response(
        `         <rss><channel><item>           <title>${title}</title>           <link>https://example.test/canary-soak/${
          encodeURIComponent(title)
        }</link>           <description>${description}</description>           <pubDate>Sun, 24 May 2026 10:30:00 GMT</pubDate>         </item></channel></rss>       `,
      ...
      await handleApiRequest(
        api("/v1/sources/canary-activation", {
          ...
          body: JSON.stringify({
            ...
            approvedBy: "soak-proof",
            generatedAt: "2026-05-24T10:20:00.000Z",
          }),
        }),
        options,
      ),
      ...
      await handleApiRequest(
        api("/v1/ops/canary/run", {
          ...
          body: JSON.stringify({
            ...
            maxSources: 1,
            maxTasks: 1,
            generatedAt: "2026-05-24T10:21:00.000Z",
          }),
        }),
        options,
      ),
      ...
      await handleApiRequest(
        api("/v1/ops/canary/run", {
          ...
          body: JSON.stringify({
            ...
            maxSources: 2,
            maxTasks: 1,
            generatedAt: "2026-05-24T10:22:00.000Z",
          }),
        }),
        options,
      ),
    );
    const soak = await body(
      await handleApiRequest(
        api(
          "/v1/ops/canary/soak?minCycles=2&generatedAt=2026-05-24T10:22:00.000Z",
        ),
        options,
      ),
    ) as CanarySoakResponseForTest;
    ...
    const productionSoak = await body(
      await handleApiRequest(
        api(
          "/v1/ops/canary/soak?minCycles=2&requireNativeLiveHttp=true&generatedAt=2026-05-24T10:22:00.000Z",
        ),
        options,
      ),
    ) as CanarySoakResponseForTest;
    ...
    expect(productionSoak.soak.blockers).toContain(
    ...
    expect(soak.soak.proofCommands).toContain(
      "bun run check:canary-proof-path",
    );
    expect(store.listCaptures().some((capture) => capture.sourceId === "src_non_canary_soak")).toBe(true);
  });
});
