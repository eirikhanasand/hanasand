...
describe("dwm alert repository", () => {
  test("rebuilds tenant/org alerts from watchlist and fixture Telegram/darkweb captures with SQL-shaped persistence", () => {
    ...
    expect(generationPlan.candidates[0].suppressedDuplicateCaptureRefs).toEqual([expect.objectContaining({
      ...
      duplicateIdentity: "telegram_public:hash-repo-tg-acme"
    })]);
  ...
  test("consumes org watchlist export delta terms into persisted alerts without wiping workflow receipts", () => {
    ...
    expect(receiptRows.get("webhook_delivery")).toMatchObject({
      ...
      scopeFields: expect.arrayContaining(["alerts.sourceHandoffReadiness.duplicateEvidenceSuppression", "alerts.sourceHandoffReadiness.webhookConsumer.selectedWebhookDestinationId"]),
    ...
    expect(receiptRows.get("public_ti")).toMatchObject({
      ...
      scopeFields: expect.arrayContaining(["alerts.sourceHandoffReadiness.duplicateEvidenceSuppression", "alerts.sourceHandoffReadiness.publicTiConsumer.alertGenerationRefCount"]),
  ...
  test("API rebuild and list expose generated alerts in product-ready shape", async () => {
    ...
    expect(list.alerts[0].customerReadiness.caseWebhookReplayReadiness).toMatchObject({
      ...
      ready: false,
      route: expect.stringContaining("/v1/cases/"),
      casePath: list.alerts[0].casePath,
      selectedCaptureIds: ["cap_repo_tg_acme", "cap_repo_tg_acme_followup"],
      readinessRefs: {
        caseHandoffReady: true,
        deliveryReady: false,
        alertReplayReady: false,
        workflowReady: true,
    ...
    expect(list.alertQueueVisibility.consumerContract.stableFields).toContain("alerts[].customerReadiness.caseWebhookReplayReadiness");
