...
function newTask(index: ReviewIndex, subject: AutomaticReviewTask["subject"], at: string, modelVersion: string): AutomaticReviewTask {
  ...
  const sourceRevision = source ? `${automaticSourceReviewIdentity(source).sha256}:${records[0]?.id ?? "no-evidence"}` : "";
...
function governDecision(decision: AutomaticReviewDecision, assertion: Record<string, unknown>, evidence: GovernedEvidence[], identities: ActorIdentityRecord[]) {
  ...
  if (literalEvidenceIds && !literalIdentifierGrounded(assertion, evidence, literalEvidenceIds, negative)) {
    ...
    const reason = negative
      ? "The cited governed evidence does not contain the asserted literal or a proposition-linked alternate identifier"
...
function literalIdentifierGrounded(assertion: Record<string, unknown>, evidence: GovernedEvidence[], evidenceIds: string[], allowRelatedLiteral = false) {
  ...
  return evidenceIds.every((id) => {
    ...
    return identifiers.some((identifier) => identifier.toLocaleLowerCase("en-US") === literal)
      || (allowRelatedLiteral && identifiers.length > 0 && propositionLinked(value, item.capture.safeExcerpt, pattern!));
  });
}
