...
function fairDueTasks(tasks: AutomaticReviewTask[], limit: number) {
  const byAge = (left: AutomaticReviewTask, right: AutomaticReviewTask) => Date.parse(left.queuedAt) - Date.parse(right.queuedAt) || left.id.localeCompare(right.id);
  const queues = ["incident", "claim"].map((type) => tasks.filter((task) => task.subject.type === type).sort(byAge));
