...
function automaticReviewPrompt(request: Record<string, unknown> & { promptVersion: string }, retryCorrection?: string) {
  return [
    ...
    "For source subjects, supported means the retained parser output itself is relevant operational threat intelligence and coherently extracted rather than navigation, boilerplate, marketing, malformed markup, or an unrelated security-themed page. For lawful metadata-only dark-web victim listings, coherently extracted victim organization names are operational threat intelligence even when the bounded listing does not include IOCs or attack vectors; do not mark the source uncertain solely for lacking those fields. Cite the retained output. Use invalid with reject for evidenced irrelevant or malformed output, uncertain with mark_needs_review for insufficient bounded output, and always leave actorAttribution empty. A source decision is review evidence only; it never represents collection success, health, or retained captures.",
