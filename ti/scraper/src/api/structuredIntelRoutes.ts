...
function validEvaluationLabelType(value: unknown): boolean {
  ...
  return /^(?:actor|victim|date|source|ttp|impact|dataset|leak_type|indicator|incident|cve|malware|country|sector|publication_strategy|publicity_tactic|channel_type|victim_pressure_tactic|extortion_type|monetization_path|buyer_seller_communication|intermediary_communication|profitability_signal)(?:_extraction)?$/.test(value.trim());
}
