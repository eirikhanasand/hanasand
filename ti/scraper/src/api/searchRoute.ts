...
function actorIdentity(store: any, tenantId: string | undefined, query: string) {
  ...
  return { matched: Boolean(catalogCandidates.length || dictionary || matchedProfiles.length || observedActorMatched), terms, normalizedTerms: new Set(terms.map(normalizeActorName)), catalogCandidates, catalogAmbiguous: catalogResolution.ambiguous };
}
