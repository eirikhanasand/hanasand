...
function buildApifyStoreReadinessContract(input: {
  ...
  const apifyRealSellableSampleRowForContract = (
    id: string,
    actorOrGroup: string,
    claimType: string,
  ...
  const marketplaceConversionRealRowSamplePack = {
    ...
    targetSellableRows: 100,
    sampleRows: [
      apifyRealSellableSampleRowForContract("real_apt29_identity", "APT29", "campaign", "government/cloud targets", ["government", "cloud services", "United States"], "identity targeting activity", ["T1078 valid accounts", "cloud account abuse", "APT29 recent activity"], "current", 0.9, ["clear_web", "public_channel"]),
      apifyRealSellableSampleRowForContract("real_volt_lotl", "Volt Typhoon", "infrastructure_activity", "critical infrastructure operators", ["critical infrastructure", "United States"], "living-off-the-land intrusion notes", ["living-off-the-land", "network discovery", "critical infrastructure targeting"], "recent", 0.84, ["government_advisory", "vendor_report"]),
      apifyRealSellableSampleRowForContract("real_scattered_spider_social", "Scattered Spider", "campaign", "telecom/helpdesk targets", ["telecommunications", "United States"], "social-engineering activity against identity support", ["phishing for information", "helpdesk social engineering", "sector:telecom"], "current", 0.88, ["rss_security_blog", "vendor_report"]),
      apifyRealSellableSampleRowForContract("real_clop_campaign", "Clop", "vulnerability_exploitation", "managed file-transfer customers", ["information technology", "global"], "campaign impact and exploited product context", ["public-facing application exploitation", "campaign:MOVEit", "victim claim"], "recent", 0.87, ["cert_advisory", "vendor_report"])
    ],
  ...
  const hostedDefaultPublicCorroborationLift = {
    ...
    acceptedPublicCorroborationRows: [
      { class: "single_source", hostedBaselineDecision: "included_with_caveat", expectedRowsUnlockedAfterParserAdmission: 26, buyerVisibleMetricImproved: "source_family_diversity", publicSourceFamily: "vendor_report", actorSpecificClaim: "Current vendor reporting corroborates hosted APT29, APT42, LockBit, Akira, Clop, and Play actor rows with second-family support.", freshness: "current_public_source", parserHandoff: "Cross-family public corroboration converts single-source hosted rows into parser-ready evidence.", provenanceHash: "sha256:program-fh-hosted-public-corroboration-single-source-120", countsTowardPaidPromotionNow: false, noLeak: true },
      { class: "stale_timestamp", hostedBaselineDecision: "included_with_caveat", expectedRowsUnlockedAfterParserAdmission: 21, buyerVisibleMetricImproved: "freshness", publicSourceFamily: "government_advisory", actorSpecificClaim: "Fresh public advisories refresh hosted APT28, Volt Typhoon, Sandworm, Lazarus, and Black Basta activity rows that were stale-only.", freshness: "current_public_source", parserHandoff: "Fresh public advisory timestamps replace stale latest-activity caveats for hosted rows.", provenanceHash: "sha256:program-fh-hosted-public-corroboration-stale-timestamp-120", countsTowardPaidPromotionNow: false, noLeak: true },
      { class: "missing_sector_country", hostedBaselineDecision: "hold", expectedRowsUnlockedAfterParserAdmission: 20, buyerVisibleMetricImproved: "sector_country", publicSourceFamily: "victim_notice", actorSpecificClaim: "Public victim notices add safe sector and country context for hosted Akira, Clop, RansomHub, Qilin, and BianLian rows.", freshness: "recent_public_source", parserHandoff: "Victim or target context adds sector/country fields that buyers can filter and act on.", provenanceHash: "sha256:program-fh-hosted-public-corroboration-sector-country-120", countsTowardPaidPromotionNow: false, noLeak: true },
      { class: "missing_ttp_tool", hostedBaselineDecision: "included_with_caveat", expectedRowsUnlockedAfterParserAdmission: 18, buyerVisibleMetricImproved: "ttp_tool", publicSourceFamily: "cert_advisory", actorSpecificClaim: "CERT and advisory reporting ties hosted APT42, Turla, FIN7, Scattered Spider, and ransomware rows to concrete TTP/tool pivots.", freshness: "current_public_source", parserHandoff: "Procedure and tool corroboration gives Agent 03 a concrete TTP admission path.", provenanceHash: "sha256:program-fh-hosted-public-corroboration-ttp-tool-120", countsTowardPaidPromotionNow: false, noLeak: true },
      { class: "missing_buyer_action", hostedBaselineDecision: "hold", expectedRowsUnlockedAfterParserAdmission: 18, buyerVisibleMetricImproved: "buyer_action", publicSourceFamily: "public_report", actorSpecificClaim: "Public reports convert hosted caveated actor and ransomware leads into next-search, monitor, or enrichment actions.", freshness: "recent_public_source", parserHandoff: "Public reporting is rewritten into a next-search or monitoring action instead of generic context.", provenanceHash: "sha256:program-fh-hosted-public-corroboration-buyer-action-120", countsTowardPaidPromotionNow: false, noLeak: true },
      { class: "missing_confidence_reason", hostedBaselineDecision: "included_with_caveat", expectedRowsUnlockedAfterParserAdmission: 17, buyerVisibleMetricImproved: "confidence_reason", publicSourceFamily: "security_blog", actorSpecificClaim: "Security-blog corroboration explains confidence changes for hosted APT, ransomware, and public-channel rows without exposing unsafe material.", freshness: "recent_public_source", parserHandoff: "Corroborating source-family and timestamp detail explains why confidence should increase.", provenanceHash: "sha256:program-fh-hosted-public-corroboration-confidence-120", countsTowardPaidPromotionNow: false, noLeak: true }
    ],
