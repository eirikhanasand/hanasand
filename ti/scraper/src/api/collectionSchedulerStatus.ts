...
export async function collectionSchedulerStatus(options: ApiServerOptions, lastControlAction?: Record<string, unknown>, tenantId?: string, page: { limit?: number; cursor?: number } = {}) {
  ...
  return json({
    ...
    sourceCoverage: {
      ...
      everUsefulSourceCount: activeSources.filter((source: any) => observationsBySource.get(source.id)?.some((row: any) => row.useful === true && Number(row.captureCount ?? 0) > 0)).length,
      ...
      usefulWithin24hSourceCount: activeSources.filter((source: any) => observationsBySource.get(source.id)?.some((row: any) => row.useful === true && Number(row.captureCount ?? 0) > 0 && withinDailyWindow(row.checkedAt, generatedAt))).length,
