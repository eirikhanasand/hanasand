...
export async function collectionSchedulerStatus(options: ApiServerOptions, lastControlAction?: Record<string, unknown>, tenantId?: string, page: { limit?: number; cursor?: number } = {}) {
  ...
  return json({
    ...
    sourceCoverage: {
      ...
      everUsefulSourceCount: activeSources.filter((source: any) => Boolean(qualificationBySource.get(source.id)?.lastUsefulAt)).length,
      ...
      usefulWithin24hSourceCount: activeSources.filter((source: any) => withinDailyWindow(qualificationBySource.get(source.id)?.lastUsefulAt, generatedAt)).length,
