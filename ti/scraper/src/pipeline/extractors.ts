...
const MALWARE = ["emotet", "trickbot", "cobalt strike", "qakbot", "plugx", "sliver", "carbanak", "mimikatz", "icedid", "rclone", "anydesk", "snake", "powgoop", "wellmess", "ngrok", "cloudflared", "meshagent", "screenconnect"];
const RANSOMWARE_FAMILIES = ["akira", "lockbit", "alphv", "blackcat", "black cat", "clop"];
const TECHNIQUES = ["phishing", "credential dumping", "valid accounts", "lateral movement", "remote desktop protocol", "powershell", "command and control", "exfiltration", "persistence", "spearphishing", "data exfiltration", "data theft", "sms phishing", "living off the land", "exploit", "intrusion"];
const SECTORS = ["healthcare", "government", "energy", "finance", "financial services", "telecommunications", "education", "manufacturing", "retail"];
const COUNTRIES = ["united states", "norway", "ukraine", "russia", "china", "germany", "france", "united kingdom", "japan", "south korea"];
...
export function extractEntities(text: string, context: ExtractionContext): ExtractedEntity[] {
  ...
  for (const record of ACTOR_ALIAS_RECORDS) for (const alias of record.aliases) { const index = lower.indexOf(alias); if (index >= 0) { rows.push({ ...ent("actor", record.canonical, text.slice(index, index + alias.length), context, index, index + alias.length, record.confidence), aliases: record.aliases }); break; } }
  for (const [type, hints, confidence, transform] of [["malware", MALWARE, 0.72, same], ["sector", SECTORS, 0.54, same], ["country", COUNTRIES, 0.54, title], ["ttp", TECHNIQUES, 0.72, same]] as const) for (const hint of hints) { const index = lower.indexOf(hint); if (index >= 0) rows.push(ent(type, transform(hint), text.slice(index, index + hint.length), context, index, index + hint.length, confidence)); }
  if (/\b(?:ransomware|leak|claimed|victim|extortion|data theft)\b/i.test(text)) for (const family of RANSOMWARE_FAMILIES) { const index = lower.indexOf(family); if (index >= 0) rows.push(ent("ransomware_family", title(family), text.slice(index, index + family.length), context, index, index + family.length, 0.76)); }
...
function ent(type: ExtractedEntity["type"], value: string, rawValue: string, context: ExtractionContext, start: number, end: number, confidence: number): ExtractedEntity { return { type, value: clean(value), rawValue, normalizedValue: clean(value), confidence, provenance: [prov(context, start, end, rawValue)], reviewReasons: confidence < 0.65 ? ["weak entity signal"] : [] }; }
...
function clean(value: string) { return normalizeWhitespace(value).split(/\s+(?:and|then)\s+(?:posted|listed|published|shared|used|exploited|claimed)\b/i)[0]?.split(/\.\s+(?:First seen|Last seen|Published|Observed|The campaign)\b/i)[0]?.split(/\s+in\s+the\s+(?:healthcare|government|energy|finance|financial services|telecommunications|education|manufacturing|retail)\s+sector\b/i)[0]?.split(/\s+(?:with|using|on)\s+(?:Cobalt|PlugX|QakBot|Emotet|TrickBot|Sliver|AnyDesk|Rclone|20\d{2}-\d{2}-\d{2})\b/i)[0]?.replace(/[.,;:]+$/, "").trim() ?? ""; }
