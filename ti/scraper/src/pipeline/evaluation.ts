...
const scoreCategory = (fixtures: any[], category: string) => { const expected = fixtures.flatMap((f) => f.expected).filter((v) => v.startsWith(`${category}:`)).length; const actual = fixtures.flatMap((f) => f.actual).filter((v) => v.startsWith(`${category}:`)).length; const matched = fixtures.flatMap((f) => f.matched).filter((v) => v.startsWith(`${category}:`)).length; const precision = ratio(matched, actual), recall = ratio(matched, expected); return { category, precision, recall, f1: precision + recall ? Number(((2 * precision * recall) / (precision + recall)).toFixed(3)) : 0 }; };
