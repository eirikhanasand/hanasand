...
export function checkDeployHygiene(repoRoot = resolve("../../..")): DeployHygieneReport {
  ...
  const checks: DeployHygieneCheck[] = [
    ...
    check("backup.object_continuity", /TI_BACKUP_PRIOR_OBJECT_LEDGER/.test(backupWrapper) && /object_continuity=/.test(backupScript) && /immutable evidence object changed between backups/.test(backupScript), "successive archives reject changed bytes for the same immutable object identity"),
