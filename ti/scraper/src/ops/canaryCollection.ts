...
export async function runLeasedTask(options: any, runId: string, generatedAt: string, fetcher: any, mode: string, maxBytes: number, counters: any, latestCaptureIds: string[], errors: any[], completeEvaluationCaptures: any[] = []) {
  ...
  try {
    ...
    const itemUseful = (collected: any) => Boolean(task.planning?.watchlistDiscovery
      ...
      || isSellableIntelText({ text: collected.rawText, title: collected.title, sourceId: collected.sourceId, sourceName: source.name, adapter: collected.metadata?.adapter, publishedAt: collected.publishedAt, collectedAt: collected.collectedAt, now: generatedAt, maxAgeDays: task.planning?.actorEnrichment ? Number.POSITIVE_INFINITY : sourceActivityWindowDays(source) }));
