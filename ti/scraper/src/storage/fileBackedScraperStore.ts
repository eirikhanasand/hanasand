...
export class FileBackedScraperStore extends InMemoryScraperStore {
  ...
  override saveActorEnrichmentRun(run: any): any { return this.saved(() => super.saveActorEnrichmentRun(run)); }
  ...
  private load(snapshot: Partial<FileBackedScraperSnapshot>): void {
    ...
    for (const run of snapshot.actorEnrichmentRuns ?? []) super.saveActorEnrichmentRun(run);
    ...
    for (const catalog of snapshot.actorIdentityCatalogs ?? []) this.hydrateActorIdentityCatalogSnapshot(catalog);
...
const extraSaves: Record<string, (store: FileBackedScraperStore, row: any) => void> = {
  ...
  timelinessRecords: (s, r) => s.hydrateTimelinessSnapshot(r),
  ...
  ,actorEnrichmentRuns: (s, r) => InMemoryScraperStore.prototype.saveActorEnrichmentRun.call(s, r)
};
