...
export function startScraperRuntime() {
  ...
  const store = new FileBackedScraperStore({ snapshotPath: paths.evidenceMetadataPath });
...
function startupSeed() {
  return { name: "Example security RSS seed", type: "rss", url: "https://example.com/feed.xml", accessMethod: "public_http", status: "paused", risk: "low", trustScore: 0.5, crawlFrequencySeconds: 3600, legalNotes: "Placeholder seed. Replace with approved public security RSS sources before collection." };
}
