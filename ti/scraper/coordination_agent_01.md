...
- Latest verification is green: `bun run check`, `bun test src/tests/sourceSeeds.test.ts`, focused source atlas API coverage, `bun test src/tests/api.test.ts`, `bun test src/tests/schedulerProduction.test.ts`, `bun test src/tests/ops.test.ts`, `bun run check:route-inventory`, `bun run check:contract-index`, `bun run check:api-regression`, and full `bun test` (529 pass).
