...
export async function fetchRansomwareLiveRss(query: string): Promise<TiSearchResponse | undefined> {
  const response = await fetch(FEED_URL, { headers: { "user-agent": "hanasand-ti-apify-actor/0.8 metadata-only" } }).catch(() => undefined);
  if (!response?.ok) return undefined;
  const items = parseNewsRss(await response.text(), 160).filter((item) => matches(query, item.title));
  return items.length ? rssResponse(query, items.slice(0, 80)) : undefined;
}

function rssResponse(query: string, items: ReturnType<typeof parseNewsRss>): TiSearchResponse {
  ...
  return {
    ...
    recentActivity: dated.map(({ item, iso }, index) => ({ date: iso, title: item.title, detail: `${item.source} metadata-only victim-claim signal for ${query}.`, confidence: 0.66, sourceIds: [`rwlive_${index}`], url: item.link, claimType: "victim_claim", victimName: victim(item.title), firstReportedAt: iso, lastReportedAt: iso, publisherCount: 1, impact: "public ransomware victim claim metadata" })),
    sources: dated.map(({ item }, index) => ({ id: `rwlive_${index}`, name: item.title, type: "captured_public_source", provenance: "Ransomware.live public RSS metadata", url: item.link })),
