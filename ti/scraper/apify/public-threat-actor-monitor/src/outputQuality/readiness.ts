...
export const PRODUCTION_SELLABLE_ROW_FLOOR = 2000;
...
export function monetizationReadinessForRows(rows: MarketplaceRow[], quality: ReturnType<typeof paidRowQualitySummary>) {
  ...
  const blockers = [
    quality.sellable < PRODUCTION_SELLABLE_ROW_FLOOR ? "sellable_rows_below_production_floor" : null,
  ...
  return {
    ...
    nextRevenueAction: blockers.includes("sellable_rows_below_paid_traffic_floor")
      ? `add or repair live sources until at least ${PRODUCTION_SELLABLE_ROW_FLOOR} output rows are chargeable findings and at least 25 percent of rows are sellable`
