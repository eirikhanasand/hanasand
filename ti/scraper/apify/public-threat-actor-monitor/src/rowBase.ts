...
import { relationshipInsightFields } from "./relationshipSupport.ts";
import { analysisFacetsFor, coverageGapSummary, coverageGapTitle, qualityFields } from "./rowQuality.ts";
import { schedulerFields, sourceCoverageProductFields } from "./rowScheduler.ts";

export function baseRow(response: TiSearchResponse, generatedAt: string, lastSeen: string): MarketplaceRow {
...
export function coverageGapRows(response: TiSearchResponse, generatedAt: string, lastSeen: string): MarketplaceRow[] {
