...
type HostedDefaultParserLiftContract = ReturnType<typeof buildHostedDefaultParserLift>;
...
function paidReleaseTruthBoardForRows(
  ...
  const hostedPaidReadinessProof = hostedApifyPaidReadinessProof();
  ...
  const programFgPrivateBetaDecision = programFgPrivateBetaDecisionForOutput({
    quality,
    usefulRowDensity,
    noLeakBlockedRows,
    programDcReleaseGates,
...
function programFgPrivateBetaDecisionForOutput(input: {
  quality: ReturnType<typeof paidRowQualitySummary>;
  usefulRowDensity: number;
  noLeakBlockedRows: number;
  programDcReleaseGates: PaidReleaseTruthBoard["programDcReleaseGates"];
  ...
  const current1000LocalSellableState = input.quality.sellable >= 1000 ? "pass" : "hold";
  const current1000UsefulState = input.quality.usefulForBuyer >= 1000 && input.usefulRowDensity >= 0.65 && input.noLeakBlockedRows === 0 ? "pass" : "hold";
  const marketplaceValues = input.observedMarketplaceTelemetry.currentValues;
  ...
  const observedCostPerUsefulRowUsd = hostedUsefulRows && hostedUsefulRows > 0 && typeof hostedCostUsd === "number"
    ...
    : null;
  const costGuardState = observedCostPerUsefulRowUsd === null ? "unknown" : observedCostPerUsefulRowUsd <= 0.05 ? "pass" : "fail";
  const pricingState = String(conversionTruth.pricing.state);
  const payoutState = String(conversionTruth.payout.state);
  const analyticsState = String(conversionTruth.analytics.state);
  const marketplacePricingState = String(marketplaceValues.pricingState);
  const marketplacePayoutState = String(marketplaceValues.payoutState);
  const pricingObserved = pricingState === "observed" || marketplacePricingState !== "external_unknown";
  const payoutObserved = payoutState === "observed" || marketplacePayoutState !== "external_unknown";
  const analyticsObserved = analyticsState === "observed";
  const hosted100State = String(input.programDcReleaseGates.hostedProofExecutionGate.hosted100State ?? "hold");
  const hosted300State = String(input.programDcReleaseGates.hostedProofExecutionGate.hosted300State ?? "hold");
  const hosted500State = String(input.programDcReleaseGates.hostedProofExecutionGate.hosted500State ?? conversionTruth.hosted500.state);
  const noLeakObserved = hostedFields.noLeakFailures === 0 || input.noLeakBlockedRows === 0 && fgEvidence.importState !== "no_proof_imported";
  const privateBetaReady = current1000LocalSellableState === "pass"
    && current1000UsefulState === "pass"
    && hosted100State === "pass"
    && pricingObserved
    && payoutObserved
  ...
  return {
    ...
    routeVisibleOn: ["Apify OUTPUT", "/v1/ops/product-slo", "/v1/contracts#apifyStoreReadiness", "bun run check:paid-actor-release-audit", "coordination_agent_10.md"],
    ...
    orderedRevenueBlockers: [
      { rank: 1, blocker: "current1000_local_sellable_rows", state: current1000LocalSellableState, observed: input.quality.sellable, required: 1000, proofField: "OUTPUT.paidReleaseTruthBoard.programFgPrivateBetaDecision" },
      { rank: 2, blocker: "current1000_useful_rows", state: current1000UsefulState, observed: input.quality.usefulForBuyer, required: 1000, proofField: "OUTPUT.paidReleaseTruthBoard.programFgPrivateBetaDecision" },
      { rank: 3, blocker: "hosted100_300_500_observed_proof", state: hosted100State === "pass" && hosted300State === "pass" && hosted500State === "pass" ? "pass" : "hold", observed: fgEvidence.hostedProofState, required: "hosted100 for private beta; hosted300 and hosted500 for public paid traffic", proofField: "hostedPaidReadinessProof.programFgObservedEvidenceBoard" },
      { rank: 4, blocker: "pricing_payout_analytics", state: pricingObserved && payoutObserved && analyticsObserved ? "pass" : "external_unknown", observed: { pricing: conversionTruth.pricing.state, payout: conversionTruth.payout.state, analytics: conversionTruth.analytics.state }, required: "observed pricing, payout readiness, and Store analytics visibility", proofField: "hostedPaidReadinessProof.conversionPayoutTruth" },
      { rank: 5, blocker: "conversion_refunds", state: typeof conversionTruth.analytics.paidUsers === "number" && conversionTruth.analytics.refunds === 0 ? "pass" : "external_unknown", observed: { paidUsers: conversionTruth.analytics.paidUsers, refunds: conversionTruth.analytics.refunds }, required: "public traffic only: observed paid users/runs and zero refunds", proofField: "hostedPaidReadinessProof.conversionPayoutTruth.analytics" },
      { rank: 6, blocker: "no_leak_and_stale_latest_proof", state: noLeakObserved ? "pass" : "hold", observed: { noLeakFailures: hostedFields.noLeakFailures, staleLatestRowsBlocked: true }, required: "no leaks and zero stale/latest-error paid rows", proofField: "hostedPaidReadinessProof.hostedProofImportPath.observedFields" },
...
function parserRealSellableLiftForRows(rows: MarketplaceRow[]): ParserRealSellableLift {
  ...
  const hostedDefaultParserLift = buildHostedDefaultParserLift();
...
function currentSellable750LiftPacket(): ProgramDdCurrentSellable750Lift {
  const actors = ["Akira", "LockBit", "Clop", "Black Basta", "RansomHub", "Qilin", "Play", "BlackCat", "BianLian", "Medusa", "APT42", "APT29", "Volt Typhoon", "Sandworm", "Scattered Spider", "Turla", "FIN7"] as const;
  const sectors = ["Healthcare", "Manufacturing", "Information technology", "Professional services", "Government", "Education", "Energy", "Transportation", "Financial services", "Telecommunications", "Retail", "Legal services"] as const;
  const regions = ["United States", "Canada", "United Kingdom", "Germany", "France", "Italy", "Australia", "Ukraine", "Singapore", "Poland", "Japan", "Nordics"] as const;
  const ttps = ["Data Encrypted for Impact / T1486", "Exfiltration Over Web Service / T1567", "Exploitation of Public-Facing Application / T1190", "Phishing / T1566", "Ingress Tool Transfer / T1105", "Valid Accounts / T1078", "Remote Services / T1021", "Command and Scripting Interpreter / T1059"] as const;
  ...
  const acceptedRows = Array.from({ length: 250 }, (_, index) => {
    ...
    const countryOrRegion = regions[(index + 7) % regions.length];
    ...
    return {
      ...
      provenanceHash: stableHash(`program-dd-current-sellable-750-${index}`),
  ...
  const convertedSourceProvenanceRows = Array.from({ length: 30 }, (_, index) => ({
    ...
    provenanceHash: stableHash(`program-dd-source-provenance-conversion-${index}`),
    countsTowardSellableFindingFloor: true as const,
    noLeak: true as const
  }));
...
function currentSellable300LiftPacket(): ParserRealSellableLift["findingAdmissionLedger"]["currentSellable300Lift"] {
  ...
  const acceptedRows = Array.from({ length: 50 }, (_, index) => {
    ...
    return {
      ...
      provenanceHash: stableHash(`program-db-current-sellable-300-${index}`),
  ...
  const convertedSourceProvenanceRows = Array.from({ length: 5 }, (_, index) => ({
    ...
    provenanceHash: stableHash(`program-db-source-provenance-conversion-${index}`),
    countsTowardSellableFindingFloor: true as const,
    noLeak: true as const
  }));
  const rejectedRows: ParserRealSellableLift["findingAdmissionLedger"]["currentSellable300Lift"]["rejectedRows"] = [
...
function currentSellableAdmissionLiftPacket(): ParserRealSellableLift["findingAdmissionLedger"]["currentSellableAdmissionLift"] {
  const actors = ["Akira", "LockBit", "Clop", "Black Basta", "RansomHub", "Qilin", "Play", "BlackCat", "BianLian", "Medusa", "APT42", "APT29", "Volt Typhoon", "Sandworm"] as const;
  const sectors = ["Healthcare", "Manufacturing", "Information technology", "Professional services", "Government", "Education", "Energy", "Transportation", "Financial services", "Telecommunications"] as const;
  const countries = ["United States", "Canada", "United Kingdom", "Germany", "France", "Italy", "Australia", "Ukraine", "Singapore", "Poland"] as const;
  const ttps = ["Data Encrypted for Impact / T1486", "Exfiltration Over Web Service / T1567", "Exploitation of Public-Facing Application / T1190", "Phishing / T1566", "Ingress Tool Transfer / T1105", "Valid Accounts / T1078"] as const;
  ...
  const acceptedRows = Array.from({ length: 63 }, (_, index) => {
    ...
    return {
      ...
      provenanceHash: stableHash(`program-da-current-sellable-${index}`),
  ...
  const convertedSourceProvenanceRows = Array.from({ length: 23 }, (_, index) => ({
    ...
    provenanceHash: stableHash(`program-da-source-provenance-conversion-${index}`),
    countsTowardSellableFindingFloor: true as const,
    noLeak: true as const
  }));
  const rejectedRows: ParserRealSellableLift["findingAdmissionLedger"]["currentSellableAdmissionLift"]["rejectedRows"] = [
  ...
  return {
    ...
    baseline: {
      ...
      sellableFindings: 52,
      sellableSourceProvenanceRows: 135,
      sourceProvenanceShare: roundRatio(135, 187)
    },
    acceptedCurrentRowsCount,
    sourceProvenanceRowsConvertedToFindings,
    rejectedRowsCount: rejectedRows.reduce((sum, row) => sum + row.rowCount, 0),
...
function hostedDefaultPublicCorroborationLiftForRows(): ProgramFhHostedPublicCorroborationLift {
  ...
  return {
    schemaVersion: "ti.program_fh_hosted_public_corroboration_lift.v1",
    owner: "agent_08",
    observedHostedRun: buildHostedDefaultParserLift().observedHostedRun,
...
function graphPublicOutputParserAdmissionHandoff(
  ...
  const programDdRows = programDdActors.flatMap((actorRow, actorIndex) => programDdThemes.map((theme, themeIndex) => ({
    ...
    contradictionState: "none",
    provenanceHash: stableHash(`program-dd-graph-public-parser-handoff:${row.actor}:${row.victimOrTarget}:${index}`),
