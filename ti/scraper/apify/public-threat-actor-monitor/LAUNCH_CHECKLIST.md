...
- Before buying traffic, verify `revenueConversionChecklist`, `pricingProof`, `hundredRowConversionProof`, `marketplaceConversionRealRowSamplePack`, `first100BuyerPreview`, `paidReleaseTruthBoard.buyerPaidReleaseVerdict`, and 12 `buyerSampleRows` are visible in `/v1/contracts#apifyStoreReadiness`, `/v1/ops/product-slo.apifyLaunchExperiment`, and Actor `OUTPUT`. Telemetry and payout fields must remain `null`/`unknown` or `external_unknown` until copied from Apify analytics or billing.
