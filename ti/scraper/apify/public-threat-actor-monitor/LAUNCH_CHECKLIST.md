...
- Treat `hostedPaidReadinessProof.latestHostedProof` as a single-query shape/safety proof only. It does not count toward the 100-row paid floor. The paid floor requires a hosted 100-name default run with observed dataset item count, sellable rows, sellable finding count, caveated rows, average buyer value, runtime, memory, usage cost, payout state, pricing model, Store views, runs, unique users, paid users, refunds, and last verified timestamp.
