...
- Before buying traffic, verify `revenueConversionChecklist`, `pricingProof`, `hundredRowConversionProof`, `marketplaceConversionRealRowSamplePack`, `first100BuyerPreview`, `paidReleaseTruthBoard.buyerPaidReleaseVerdict`, `paidReleaseTruthBoard.hostedPaidReadinessProof`, and 12 `buyerSampleRows` are visible in `/v1/contracts#apifyStoreReadiness`, `/v1/ops/product-slo.apifyLaunchExperiment`, and Actor `OUTPUT`. Telemetry and payout fields must remain `null`/`unknown` or `external_unknown` until copied from Apify analytics or billing.
...
- Treat `paidReleaseTruthBoard.buyerPaidReleaseVerdict` as the public paid-release summary. It must say `hold_paid_traffic`, keep `publicListingState` at `draft_copy_ready_not_promoted`, explain observed row blockers in buyer language, and keep marketplace telemetry, payout readiness, and pricing state `external_unknown` until observed.
- Treat `hostedPaidReadinessProof.latestHostedProof` as a historical single-query shape/safety proof only. It does not count toward the 100-row paid floor. The paid floor requires a hosted 100-name default run with observed dataset item count, sellable rows, sellable finding count, caveated rows, average buyer value, runtime, memory, usage cost, no-leak result, and Program CP `secondBatchAudit`; payout state, pricing model, Store views, runs, unique users, paid users, refunds, and last verified timestamp stay external until copied from Apify.
