...
The hosted paid readiness proof is nested at `OUTPUT.paidReleaseTruthBoard.hostedPaidReadinessProof`. It records the old hosted shape/safety run `OThlfd0uzSCNnedAO` separately from the local 100-name proof and keeps both out of paid promotion until a hosted 100-name default run records run id, dataset id, dataset item count, sellable count, sellable finding count, caveated count, average buyer value, runtime, memory, usage cost, payout state, pricing model, Store views, runs, unique users, paid users, refunds, and last verified timestamp. Unknown fields stay `null` or `external_unknown`.
