...
const rows = outputRecord.rows as Array<Record<string, any>>;
...
function assertDailyCollection(run: Record<string, any>) {
  ...
  assert(run.preset === "100-name-default-watchlist", "daily collection preset");
...
function assertReadiness(readiness: Record<string, any>) {
  ...
  assert(readiness.minimumProductionSellableRows === 100, "readiness floor");
  assert(Number(readiness.sellableRows) >= 100, "readiness sellable rows");
  assert(readiness.status === "ready_for_paid_traffic", "ready at 100 rows");
}
