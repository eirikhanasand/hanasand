...
`searchableSourceMetadataPromotionConsumerReplay` is the dry-run bridge from promotion-gate audit rows into the Actor/public-answer consumer path as `ti.evidence_searchable_source_metadata_promotion_consumer_replay.v1`. Direct public rows produce `actor_dataset_candidate_ready` receipts, but production writes still stay disabled until a real repository is enabled. Restricted/dark metadata rows produce `await_public_support_replay` receipts until completed public-support evidence replays through the read model; receipts contain only ids, promotion state, required evidence, buyer-visible field names, blockers, and no-leak flags.

`searchableSourceMetadataPublicSupportReplayReceiptLedger` is the pending public-support receipt ledger as `ti.evidence_searchable_source_metadata_public_support_replay_receipt_ledger.v1`. It enumerates every restricted/dark metadata candidate that needs public report, public-channel, advisory/vendor, or freshness corroboration before promotion-gate replay can turn it into an Actor/public-answer candidate. Until Agent 01/04/07 support receipts arrive, rows remain `awaiting_public_support_receipts`, production promotion stays disabled, and raw public-support bodies are not accepted.

`searchableSourceMetadataPublicSupportReplayReceiptRepository` is the disabled Postgres audit boundary as `ti.evidence_searchable_source_metadata_public_support_replay_receipt_repository.v1`. It maps receipt ledgers to `evidence_searchable_source_public_support_replay_runs` and `evidence_searchable_source_public_support_replay_receipts`, then holds persistence behind `TI_SEARCHABLE_SOURCE_METADATA_PUBLIC_SUPPORT_REPLAY_RECEIPT_REPOSITORY_ENABLED`. The status exposes accepted/held receipt counts, completed/pending state, blocked Actor-row count, and replay readiness while persisting zero rows, mutating zero queues, promoting zero Actor rows, and writing zero public-answer cache entries.
