...
`GraphLiveSearchUpdateDto` is the compact incremental graph contract for responsive `/ti` search. It appears as `graph.liveUpdate`, `graph.runtime.liveUpdate`, and `readiness.runtime.liveUpdate` on `/v1/intel/search.graph`, `/v1/graph/query`, `/v1/graph/review-plan`, and `/v1/exports/stix`. API/frontends should use its `deltaCounts`, `scenarioCoverage`, `deltaStream`, `nextPollSeconds: 3`, and `cursorField: "graph.deltas[].cursor"` to poll live clear-web/public-channel graph deltas. Its policies are stable: weak discovery gives pivots/caveats only, public-channel-only hints are held until corroborated or reviewed, restricted evidence is held context with no public fact promotion, default STIX export is reviewed/promoted only, and TAXII remains descriptor-only with no server claim. `deltaStream.fixtures` is the Agent 07/09/10 compatibility matrix for review holds, ledger holds, public answer impacts, cursor state, release-gate decisions, graph rollback, and STIX eligibility transitions.
...
`/v1/intel/search.graph`, `/v1/graph/query`, and `/v1/exports/stix` also expose a compact `runtime` block. API/frontends should use this block for release-gate checks because it normalizes relationship confidence, ledger IDs, review state, freshness, export holds, `exportSla`, `liveUpdate`, and `publicFactPolicy` across public answer facts and STIX export readiness.

`runtime.backendContract` is the backend-neutral graph repository contract for future persistence. It does not require a live Neo4j or SQL graph backend today; it freezes the operations and required fields those backends must preserve so API consumers can keep using the same graph DTOs while storage moves out of memory.
...
ATT&CK/STIX hardening is part of the frozen mapping contract. Consumers should preserve `x_ti_*` custom properties, treat deprecated or revoked ATT&CK IDs as review holds, and keep TAXII integration code pointed at descriptor metadata only until a real TAXII server is coordinated.
