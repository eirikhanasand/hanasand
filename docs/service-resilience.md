...
The primary Compose worker uses `LOG_CATCHUP_BATCH_LIMIT=1000` and `LOG_CATCHUP_INTERVAL_MS=50` to continue draining pending work without an unnecessary five-second delay. Pass these environment variables to `deploy-inspur-api-worker.py` to change the preserved worker settings during a release; invalid values fail before deployment, and rollback restores the previous values. This controls scheduling, not a guaranteed processing rate.
...
For a short measured trial without restarting the worker, atomically write JSON containing numeric `limit` (1–1000) and `expiresAt` (Unix milliseconds, at most ten minutes ahead) to `/home/hanasand/resilience/status/log-catchup.json`. Optional `intervalMs` (50–5000) controls the start-to-start cadence of busy passes, including processing time, with a minimum 50 ms pause after completion; idle or failed passes back off for five seconds. The baseline cadence is `LOG_CATCHUP_INTERVAL_MS` (default 5000). Passes never overlap, and shutdown waits for the current pass to finish. The existing read-only mount exposes the file as `/resilience/log-catchup.json`; the worker rereads it each processing tick. Missing or expired files restore both validated environment settings, so a trial expires even if its controller stops. Invalid configuration is reported as a processing error. Keep the baseline settings at the intended rollback values, monitor queue age and replica health, and remove the expired trial file after verification. Measure newly processed events and incoming writes, not just cursor movement: cursors can pass logs already handled by the priority queue.
