...
The primary retains up to 16 GB of WAL per slot (`max_slot_wal_keep_size`), increased from 2 GB after the OVH replica fell behind. `wal_compression=zstd` reduces replication traffic; the primary and replica images support it, and `fsync` and `full_page_writes` stay enabled. Replica checks open an HA case when the slot is missing, inactive, or more than 1 MB behind. Lost WAL stays recorded through failed samples and clears only after that replica catches up. Restore into a new volume and verify the backup before replacing the failed replica; retain the old volume until recovery is confirmed.
