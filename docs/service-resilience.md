...
For a short measured trial without restarting the worker, atomically write JSON containing numeric `limit` (1–1000) and `expiresAt` (Unix milliseconds, at most ten minutes ahead) to `/home/hanasand/resilience/status/log-catchup.json`. The existing read-only mount exposes it as `/resilience/log-catchup.json`; the worker rereads it each processing tick. Missing or expired files use the validated environment limit, so a trial expires even if its controller stops. Invalid configuration is reported as a processing error. Keep the baseline limit at the intended rollback value, monitor queue age and replica health, and remove the expired trial file after verification.
