...
The primary Compose definition reserves a maximum 2 GiB `/dev/shm` for parallel PostgreSQL queries, within its existing 96 GiB memory allowance. Docker's default 64 MiB caused `53100` shared-memory allocation failures. This setting takes effect only when the database container is recreated; apply it during a coordinated, approved database maintenance window, preserving the data volume and replication configuration. It does not change PostgreSQL durability or `shared_buffers`.
