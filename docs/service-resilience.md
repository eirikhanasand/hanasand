...
OVH replication uses its own compressed SSH connection, `hanasand-tunnel-replication`, through loopback port 38503. `isolated-tunnels.py authorize` permits this port on the existing restricted key; `start` creates the connection without restarting other tunnels. After a restore finishes, point the standby's `primary_conninfo` at port 38503 and reload PostgreSQL. Never interrupt the old replication connection while a backup is still using it. Application queries remain on their separate, uncompressed connection.
