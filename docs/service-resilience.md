...
- Backup verification shares Inspur's disks with the live database. Both isolated restore stages are limited to 20 MiB/s reads and 10 MiB/s writes per physical disk, with 16 GiB memory and no swap. The verification database uses a 4 GiB cache. Recovery can take up to two hours (`BACKUP_VERIFY_RECOVERY_TIMEOUT_SECONDS`); an exited database fails immediately. A backup is verified only after its manifest, WAL replay and actual read/write check pass. These limits do not change the live databases.
