...
Support stays blocked on a secondary recovery site. Its HTTP and WebSocket routes open automatically when fresh monitor state selects that site for the frontend or API and the database is writable. Failback closes support connections again; stale status and read-only recovery keep support blocked. The restricted application role has access only to the support tables needed for conversations, feedback and one-use connection tickets. If no AI model is available on the active recovery site, the saved conversation enters the human queue. This does not start AI or background workers on OVH or promote a database.
