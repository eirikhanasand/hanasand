...
External mail travels through Inspur Stalwart → the Inspur SSH connector → OVH Stalwart → recipient MX. This applies to the three approved senders: `eirik@hanasand.com`, `support@hanasand.com`, and `noreply@hanasand.com`. Other senders retain their existing routing. Local mailbox delivery remains local. Inspur’s network blocks internet SMTP, so incoming mail also enters through OVH port 25 → HAProxy → an encrypted reverse SSH connection → Inspur Stalwart. The PROXY protocol preserves the original sender IP for spam checks and rate limits.
...
- STARTTLS and certificate validation are required on the relay route. Both the upstream server and private relay enforce sender ownership. The relay principal accepts only the three addresses in `RELAY_SENDERS`; expanding this list requires authorization. Queue health principals have read-only queue permissions.
