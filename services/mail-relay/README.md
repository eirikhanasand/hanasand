...
External mail travels through Inspur Stalwart → the Inspur SSH connector → OVH Stalwart → recipient MX. This applies to personal, support, sales and system mail. Local mailbox delivery remains local. Inspur’s network blocks internet SMTP, so incoming mail also enters through OVH port 25 → HAProxy → an encrypted reverse SSH connection → Inspur Stalwart. The PROXY protocol preserves the original sender IP for spam checks and rate limits.
...
- STARTTLS and certificate validation are required on the relay route. The private relay principal can authenticate/send on behalf of the upstream mail server, which enforces sender ownership. Other principals retain sender-address checks. Queue health principals have read-only queue permissions.
