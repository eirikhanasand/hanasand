...
- Secrets live under `~/mail-relay`, mode 0700, with secret files mode 0600; `~/hanasand/mail/mail-relay` is a Git-tracked symlink to that private runtime directory. OVH SMTP storage is durable in `data`; never remove this directory or the existing unrelated mail service during redeployment.
