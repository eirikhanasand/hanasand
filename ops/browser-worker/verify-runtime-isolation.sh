...
printf '%s\n' "$env" | while IFS='=' read -r name _; do
    case "$name" in
        NODE_ENV|PORT|HOME|BUN_INSTALL|BUN_INSTALL_BIN|BUN_RUNTIME_TRANSPILER_CACHE_PATH|BROWSER_SANDBOX_WORKER_ONLY|BROWSER_SANDBOX_SKIP_RUN_DB|BROWSER_SANDBOX_CHROMIUM_SANDBOX|BROWSER_SANDBOX_MAX_SESSIONS|BROWSER_SANDBOX_PREWARM|ONION_SESSION_PROXY|PATH|HOSTNAME) ;;
