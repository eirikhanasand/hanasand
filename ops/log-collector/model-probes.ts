...
export function ownsModelListener(pid: string, port: number, proc = '/proc'): boolean {
  if (!/^[1-9]\d*$/.test(pid) || !Number.isInteger(port) || port < 1 || port > 65535) return false;
  try {
...
export function enrichModelProbe(log: LogEvent, cursor: string, options: {
  ...
  if (Object.keys(log.metadata).sort().join(',') !== 'collector,pid,unit,user'
    || log.metadata.collector !== 'journal') return log;
  ...
  try {
    const key = verificationKey(options.keyFile || process.env.MODEL_PROBE_VERIFICATION_FILE || '/home/hanasand/hanasand/ops/runtime/probe-verification.json');
    if (!key) return log;
    ...
    return verifyModelDiscoveryEvidence(enriched, key) ? enriched : log;
  } catch { return log; }
}
