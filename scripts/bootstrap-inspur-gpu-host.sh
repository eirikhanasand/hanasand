...
if [ "$MODEL_BACKEND" = "vllm" ]; then
  ...
  MODEL_PROFILE="qwen3-coder-30b-a3b-vllm"
else
...
Environment=HANASAND_VLLM_TENSOR_PARALLEL_SIZE=${EXPECTED_GPUS}
Environment=HANASAND_VLLM_MODEL_REPO=Qwen/Qwen3-Coder-30B-A3B-Instruct
Environment=HANASAND_VLLM_SERVED_MODEL_NAME=hanasand
Environment=HANASAND_VLLM_DTYPE=float16
Environment=HANASAND_VLLM_MAX_MODEL_LEN=65536
Environment=HANASAND_VLLM_GPU_MEMORY_UTILIZATION=0.86
Environment=HANASAND_VLLM_MAX_NUM_SEQS=2
Environment=HANASAND_VLLM_MAX_NUM_BATCHED_TOKENS=2048
Environment=HANASAND_VLLM_ENFORCE_EAGER=1
Environment=HANASAND_VLLM_ENABLE_PREFIX_CACHING=0
Environment=HANASAND_MODEL_CONTEXT_MAX_TOKENS=65536
