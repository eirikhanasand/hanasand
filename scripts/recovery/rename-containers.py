    ...
    with os.fdopen(descriptor, 'r+') as lock:
        ...
        try:
            ...
            for filename in ('replace-recovery.py', 'upgrade-routers.py'):
                ...
                text = re.sub(r"hanasand-recovery-[a-z0-9-]+", lambda match: simple_name(match[0]), originals[path])
                text = text.replace("'hanasand-recovery-proxy-'+str(index)", "'hanasand-proxy-'+str(index + 1)")
