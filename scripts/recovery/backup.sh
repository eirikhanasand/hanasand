...
record_result() {
 ...
 if "$memory_stage"; then
  docker rm -f hanasand-recovery-backup >/dev/null 2>&1 || true
...
if [ "$available_disk_kib" -lt 134217728 ]; then
 ...
 stage_volume=hanasand-recovery-backup-$stamp
...
docker run --rm --name hanasand-recovery-backup --network host --cpus 1 --memory "$backup_memory" --memory-swap "$backup_memory" -v "$archive:/backup" "$image" \
