...
image = 'hanasand-resilience-monitor:' + release
...
for key in settings:
    if key.startswith('RESILIENCE_') or key == 'HANASAND_RELEASE_COMMIT':
        command += ['-e', key]
command += [image]
...
try:
    ...
    for attempt in range(45):
        try:
            with urllib.request.urlopen('http://127.0.0.1:' + settings.get('RESILIENCE_PORT', '19901') + '/status', timeout=3) as response:
