...
if kind == 'frontend': settings.update(HOSTNAME='127.0.0.1', FRONTEND_AUTH_API='http://127.0.0.1:28082/api', FRONTEND_INTERNAL_API='http://127.0.0.1:28082/api', TI_SCRAPER_API_BASE='http://127.0.0.1:18097', RESILIENCE_STATUS_URL='http://127.0.0.1:19901/status')
...
for port in ports:
    ...
    if kind != 'auth':
        for mount in original['Mounts']:
            if mount['Destination'] == '/resilience': continue
